...
4. **State remapping**：结构变更（如把"按章"重切成"按逻辑段"）时，生成 `old_kp→new_kp` 映射，**非破坏性**：原始 `attempts` 不动，只更新/新增 `kp_id` 指向或写 `kp_remap` 表；`review_queue`/`insights`/错题按映射迁移；掌握度按合并时间线重算（复用 `recomputeReviewFromAttempts` 思路）。
