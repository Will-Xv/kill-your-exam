...
- **手写长图切片（2026-08，`lib/handSplit.js`）**：拉得很长的手写图又窄又高，多模态整体降采样会糊。`h > w*1.6` 时额外生成切片（片高≈`w*1.1`，最多 12 片）。★**两种切法逐切口各自判断、混用**：先用 `inkPerRow` 逐行统计墨量，在目标切点 ±25% 窗口内找 ≥6px 的**全空白行**（`findBlankCut`）→ 从缝里下刀，干净无重复；该处找不到缝才**重叠**（`OV=min(h*6%, w*15%)`，≥24px），保证跨切口笔画至少在一张里完整。**整页原图始终第一位**，切片按序跟随（`handwriting-p{i}of{n}.png`）——用整页看结构、用切片看字迹，**由模型自己挑或对照着看**，代码不替它决定。判卷提示（练习 `answer` / 模考 `submit`）检测到 `handwriting-p*` 会说明"同一份作答、相邻切片可能重叠、别重复计分"。附件上限 `attachParts` 4→10；三处提交（练习/模考/竞技场）均改为 `[...其它.slice(0,3), 整页, ...切片].slice(0,10)`。
