...
export default function DynamicForm({ title, fields = [], onSubmit, onCancel, busy }) {
  ...
  const missing = fields.some((f) => f.required && (vals[f.key] == null || vals[f.key] === "" || (Array.isArray(vals[f.key]) && !vals[f.key].length)));
  ...
  return (
    <div className="rounded-2xl border border-[#e4d5af] bg-[#fbf6e9] p-4 text-sm text-[#2f2413] shadow-sm">
          ...
          <div key={f.key}>
            ...
            {f.type === "select" ? (
              ...
              <div className="space-y-1">{(f.options || []).map((o) => (
                <label key={o.value} className="flex cursor-pointer items-center gap-2"><input type="radio" checked={vals[f.key] === o.value} onChange={() => set(f.key, o.value)} /> {o.label || o.value}</label>
              ))}</div>
            ) : f.type === "checkbox" ? (
      ...
      <div className="mt-3 flex justify-end gap-2">
        ...
        <button onClick={() => onSubmit(vals)} disabled={busy || missing} className="rounded-full bg-[#2f2413] px-4 py-1.5 text-xs font-semibold text-white hover:opacity-90 disabled:opacity-50">{busy ? t("处理中…") : t("提交")}</button>
      </div>
    </div>
  );
}
