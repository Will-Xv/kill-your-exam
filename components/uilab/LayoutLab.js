...
function Toolbar({ S }) {
  const _plc = placement.useItems();
  const canPub = !!_plc.canPublish;
  const t = useT();
    ...
    <div className={`fixed z-[60] flex max-w-[94vw] gap-2 ${colDir} ${posCls}`}>
      ...
      {!editing ? (
        <div className="flex flex-wrap items-center gap-2">
          ...
          {active && canPub && <button className={btn + " bg-[#9e140c] text-white hover:opacity-90"} onClick={async () => { if (await confirmDialog(t("发布为默认后,所有用户的首页都会用这套布局。确定发布?"))) lab.publishDefault(); }}>🌐 {t("发布为默认")}</button>}
          {canPub && S.publishedDefault && <button className={btn + " bg-[#f6efdc] text-[#9e140c] ring-1 ring-[#e4d5af] hover:brightness-95"} onClick={async () => { if (await confirmDialog(t("取消发布默认布局?所有用户会恢复原始首页。"))) lab.unpublishDefault(); }}>{t("取消发布")}</button>}
        </div>
      ) : (
        <div className="flex max-w-[94vw] flex-wrap items-center gap-1.5 rounded-2xl border border-[#e4d5af] bg-[#f6efdc] p-2 shadow-xl">
          ...
          <button className={btn + " bg-[#2f2413] text-[#f6efdd]"} onClick={async () => { const n = await promptDialog(t("给这套布局起个名字:"), { defaultValue: active ? active.name : t("我的布局") }); if (n && n.trim()) lab.savePreset(n.trim()); }}>💾 {t("另存为")}</button>
          ...
          {canPub && <button className={btn + " bg-[#9e140c] text-white hover:opacity-90"} onClick={async () => { if (await confirmDialog(t("发布为默认后,所有用户的首页都会用这套布局。确定发布?"))) lab.publishDefault(); }}>🌐 {t("发布为默认")}</button>}
          <button className={btn + " bg-[#f6efdc] text-[#3d2b10] ring-1 ring-[#e4d5af]"} onClick={async () => { if (await confirmDialog(t("储存并覆盖当前页面?\n\n「确定」= 保存本次修改并覆盖当前生效的布局。\n「取消」= 不保存。"))) { lab.saveOverwriteCurrent(); } else if (await confirmDialog(t("不保存,直接退出编辑吗?\n(「取消」可继续编辑)"))) { lab.exitEdit(); } }}>✓ {t("完成")}</button>
        </div>
      )}
