  ...
  return (
    <>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 md:top-0 md:bottom-auto">
        <div className="mx-auto flex max-w-3xl items-center justify-around gap-1 border-t border-[#e4d5af] bg-[#f5eed6]/92 px-1 py-1.5 backdrop-blur-xl md:mt-3 md:justify-center md:gap-1 md:rounded-full md:border md:border-[#e4d5af] md:px-2 md:shadow-lg">
          {primary.map((it) => (
            <Link key={it.href} href={it.href}
              className={`flex flex-1 flex-col items-center gap-0.5 rounded-2xl px-2 py-1.5 text-[11px] font-medium transition md:flex-none md:flex-row md:gap-1.5 md:px-4 md:py-2 md:text-sm ${active(it.href) ? "text-[#6b4a25] md:bg-[#efe0bd]" : "text-[#7a6a45] hover:text-[#2f2413]"}`}>
              <span className="text-lg md:text-base">{it.icon}</span><span>{t(it.label)}</span>
            </Link>
          ))}
          <button onClick={() => setOpen(!open)}
            className={`flex flex-1 flex-col items-center gap-0.5 rounded-2xl px-2 py-1.5 text-[11px] font-medium transition md:flex-none md:flex-row md:gap-1.5 md:px-4 md:py-2 md:text-sm ${open ? "text-[#6b4a25] md:bg-[#efe0bd]" : "text-[#7a6a45] hover:text-[#2f2413]"}`}>
