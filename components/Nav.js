...
export default function Nav() {
  ...
  const p = onHome ? (layout && layout["__nav"]) : null;
  ...
  const drag = (fn) => (e) => {
    ...
    const m = (ev) => fn(ev.clientX - sx, ev.clientY - sy, p0);
    const up = () => { window.removeEventListener("pointermove", m); window.removeEventListener("pointerup", up); };
    window.addEventListener("pointermove", m); window.addEventListener("pointerup", up);
  };
  const mvN = drag((dx, dy, p0) => lab.setPos("__nav", { x: Math.round(p0.x + dx), y: Math.round(p0.y + dy) }));
  const eL = drag((dx, _d, p0) => { const w = Math.max(160, Math.round(p0.w - dx / (p0.s || 1))); lab.setPos("__nav", { x: Math.round(p0.x + (p0.w - w)), w }); });
  const eR = drag((dx, _d, p0) => lab.setPos("__nav", { w: Math.max(160, Math.round(p0.w + dx / (p0.s || 1))) }));
  const sc = drag((dx, _d, p0) => lab.setPos("__nav", { s: Math.max(0.5, Math.min(2, +((p0.s || 1) + dx / (p0.w || 400)).toFixed(3))) }));
  ...
  return (
    <>
      ...
      <nav className={`fixed bottom-0 left-0 right-0 z-50 md:top-0 md:bottom-auto ${dockLeft && !p ? "md:pr-[460px] lg:pr-[500px]" : ""}`}>
        <div ref={navRef} style={navStyle} className="mx-auto flex max-w-3xl items-center justify-around gap-1 border-t border-[#e4d5af] bg-[#f6efdc]/95 px-1 py-1.5 backdrop-blur-xl md:mt-3 md:justify-center md:gap-1 md:rounded-full md:border md:border-[#e4d5af] md:px-2 md:shadow-lg">
          {primary.map((it) => (
