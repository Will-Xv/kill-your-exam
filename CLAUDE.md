...
- **多考试时主动提示建分组(2026-07,Will)**:主页(HomeClient 顶部)+ 追杀计划(/exams 顶部)在【用户有≥2门顶层考试且≥2门未分组】时主动弹提示"要不要建成一组,今日任务统一管理更方便(数据不合并)",三选一:【好,建一组】(POST /api/exam-group group_all→createGroup 把所有未分组顶层考试装进"我的考试"组)/【暂不】(本次隐藏,下次再提)/【不再提醒】(setSetting exam_group_prompt_dismissed:<uid>=1 永久)。组件 components/ExamGroupPrompt。i18n 全语言。
