...
## ★ 元规则:砖头【宁可如实报失败,绝不静默兜底】(2026-07 v12 复盘,Will 拍板)

v12 报告里三条最严重的"杀手嘴上说了、实际没做"——查到底**根子是同一个**:出错时代码倾向"装作没事继续走",工具返回假成功,act 执行日志如实记"成功",杀手于是如实转述"已改好"。**杀手一直是诚实的,它只是忠实转述了工具给它的假消息**。所以这类问题**加提示词、加日志都堵不住**,必须在砖头里修。

已修的三个实例(引以为戒):
- `planWithAI`(dailyPlan):`catch { out = 默认值 }` 吞掉 AI 超时 → 套默认结果照样写库 → `return ok:true + "已按你的需求重排"`。主人要的"先辩论"被丢弃,却报成功。
- `assign_practical_task`:`Promise.all(... .catch(() => null))` 吞掉每道失败 → 全失败 count=0 仍 `ok:true`,提示语还写"已一次布置 0 道实践作业"。
- `checkInSyllabus` 守门:`unsure` 和判定出错都静默放行 → 越界内容照样落地。

**今后写/改砖头一律遵守**:
1. AI 调用失败/超时 → **返回 `ok:false` + 如实的 note**,并且**不要写库**(别用一份降级结果覆盖主人原有的状态)。
2. 批量操作 → 分别统计成功/失败,**全失败必须 ok:false**;部分失败要在返回里点名哪几项失败,并要求汇报时说明。
3. 判定类守门 → "拿不准"和"判不出来"**都不算通过**,要拦下来问主人(Will:"咋有问题都放行啊")。
4. 任何会调 AI 的长流程 → **必须有硬超时**。Railway 是持久进程,心跳会一直刷新 `updated_at`,看门狗只在心跳停了才收尸——没超时就等于让主人对着不动的"思考中"无限期干等。
5. 判断"是不是 bug"时:**先怀疑工具返回了假成功,再怀疑模型**。

## ★ 元规则:功能【退役/删除】时,必须把所有引用一并清干净(2026-07 v12 复盘,Will 拍板)

上面那条元规则只覆盖了「新增/改动」,**「删除/退役」这个方向一直没人管**,结果同一天栽了两次:

- `build_study_plan` 2026-07-19 就退役了,`appGuide` 里却还有 **4 处在教杀手用它** —— 杀手照着调,就是调一个不存在的工具。
- `app/onboarding`(手动建考试页)、`app/collector`(采集页)早已删除,`FEATURES.md` 里却仍把它们当**现有功能**在描述(连章节标题都还挂着 `app/onboarding`)。

**今后退役/删除任何功能(工具、砖头、页面、入口),必须同一次改动里做完**:

1. **代码**:删实现,并检查还有谁在调它。
2. **`lib/appGuide.js`**:把所有教杀手使用它的段落删掉或改写成现行做法。**这一条最容易漏、后果也最严重**——提示词留着,杀手就会真的去调那个不存在的工具。
3. **`FEATURES.md`**:它是「**现有功能**」文档。把该功能的描述删掉,或明确标注已下线及替代方案。带日期的变更记录(如「取消更多☰(2026-07-19)」)属于历史,保留不动。
4. **`CLAUDE.md`**:历史条目**不要删**(那是变更史),但要在提到它的地方**加一句「⚠️ 已于 X 月 X 日退役」**,免得以后有人读到旧记录把它当现役复活;同时作废所有依赖它的待办。
5. **退役注释写清「为什么」**:像 `lib/bricks/schedule.js` 里 build_study_plan 那样写明退役原因,别只写「已退役」——否则以后会有人凭直觉把它加回来。

**排查提示**:怀疑某个功能是否已退役,用 `grep -rn "<名字>" --include=*.js lib app | grep -v appGuide` —— 如果只剩 appGuide 提到它,那它就已经不是真工具了。
...
## 提示词引号检查(2026-07·反复踩)
- 工具描述/砖头 title/scopeGuard 等【中文提示词是双引号字符串】,文中写了【英文直引号 "】就会截断字符串、build 失败(反复踩,如 read_material 描述、ask_user_form 描述)。中文引用一律用「」『』或全角 "",【不要用英文直引号】。
...
- **⚠️ 考试分组【已退役 2026-07】**:后改为「所有考试默认算一个组、靠首页 chips 切换」,不再有手动建分组。砖头 `exam_group_*`、`lib/examGroups.js`、`components/ExamGroupPrompt.js`、`/api/exam-group` 全部删除,`db.js` 的 `exam_groups`/`exam_group_members` 建表语句也已移除(老账号残留空表无害)。下面这条是当初的设计记录,保留供历史参考:
...
- **建考试自动布局别删有用栏目(2026-07,Will)**:autoAdjustExamUi 判错——非艺术类考试删了拍照搜题(quizupload)和实践作业(tasks,而该考试已有实践作业!),却留着没用的表演回放(performances)。修:①提示改成按功能是否真用得上、给具体判据(performances 是最易错项,非表演一律 false;tasks 只要会动手做就 true);②硬规则覆盖 AI:非表演类(exam_type!=='performance')【强制保留 quizupload】、已有 practical_tasks 的【强制保留 tasks】、非表演类【强制收起 performances】;quizupload 纳入自动整理。③appGuide 给杀手主动调布局也加护栏:非表演类别删拍照搜题/实践作业(尤其已有作业时),表演回放只给表演类。
...
- **改行为后要回头扫一遍 appGuide 有没有"旧说法"**:Will 提醒"免得杀手还以为某些砖头是旧功能"。这次真扫出了硬矛盾——`set_reminder` 那条还写着「到点会推送通知+进收件箱」「没开也没关系,提醒会在收件箱」,和新规则(提醒只走推送、不进收件箱)正面打架;另外还有一段旧的 `customize_daily_plan` 说明残留在四层路由后面。**加新规则时别只顾着往下追加,要 grep 老段落里的同一件事有没有相反的写法。**

### 导航栏「我的」(2026-07-19,Will)
- 需求原话:导航栏「更多」旁边加「我的」,**可在导航栏里挪动但不能挪出去或删掉**;收件箱/设置/反馈/回档并进去;随后又把 模拟考/屠杀准备 挪进「更多功能」、**「更多」整个删掉**,管理员/开发者项并进「我的」,最后 **你的全部杀技也进「我的」**、表演回放留在「更多功能」。
- 实现要点:`mine` 是**菜单型 item**(无 `href`、有 `menu`),Nav 里遇到 `it.menu` 渲染成按钮而不是 `Link`;固定靠 `placementCore.PINNED` + `applyMove` 守卫 + `ui_remove_feature` 拒绝。
- **存量数据要迁移,别只在渲染时兜**:Will 明确说"老用户放在更多里面的旧的直接放在更多功能里面就好了" ⇒ 在 `lib/db.js` 做一次性 `more→morefeatures` 改写,而不是在每个渲染处 merge。
- **手动定制 UI 也要一起改**:Will 专门提醒过。去掉「更多菜单」列、**不要加「我的」列**(内容固定),并把「我的」里的项从面板里整体排除。

### 周计划 / 建考试(2026-07-19,Will)
- **Will 的判断很关键**:"如果原来的是按学习进程把知识点铺开的话,好像跟原来的今日任务逻辑也对不上吧?新的无论怎么改,一定要跟现有的今日任务逻辑对得上。" ⇒ 核实后确认 `currentDailyItems` 与 `dayPlan` **互不引用**,是两套平行系统。**以后新增任何"计划/进度"视图,先确认它和今日任务是不是同一个引擎算的。**
- 表达方式:**不要做"预计哪天学哪个"的推算**(Will 明确否掉),要讲**客观截止约束**——"这天之前要学完哪些"。
- **共享文案模块** `lib/dailyLabels.js`:首页与周计划共用,防止两边各写一套又跑偏。
- **手动建考试整体下线**:`app/onboarding` + `app/api/onboarding` 删除;导引结束直接进空首页;各处入口改成 `openKiller()`。旧的未建完考试由杀手补完。
- ★**别在主人没开口时自己弹表单打断他**(Will:"如果有正在进行的对话也不能冲突")。

### 反复踩的两类坑(2026-07-19,已第 4 次)
- **公式原样露出来**:凡是【会显示知识点/任务/计划步骤标题】的地方,都要走 `<MD inline>`(标题里可能带 `$...$`)。已修过:首页今日任务、周计划每日条目、学习页速练卡片(那处是**先 slice 截断再渲染**把 `$...$` 切断,要用 `safeCut`)、**模拟考蓝图页的按知识点题目分布**、首页作业/逾期标题、杀手计划步骤。**加新的标题展示位时先想一下这条。**
- **词典缺键 = 静默露中文**:`"确定"`、`"表演"` 都是**8 个词典一个都没有**,所以整页英文里只有它是中文。加任何界面文案,8 个词典一起加;排查这类问题直接 `grep -c '"某词":' lib/translations.js`,结果是 0 就是它。

### "AI 编的数据被当成事实"这一类(2026-07-19)
- **典型**:考试时长。`exams` 没有时长字段 ⇒ 建考试时 AI 在档案里写了句"30 分钟 quiz" ⇒ 蓝图读到后说"基于**指定的** 30 分钟"并据此定题量。**AI 自己的假设,一层层传下去就变成了"用户指定的事实"。**
- **通用解法**:凡是【会影响下游决策的关键事实】(时长、题量、考试形式、分值…),要么变成**一等数据字段 + 由主人/官方来源填**,要么**明确标成未知**;【绝不能让 AI 编一个然后被下游当真】。判断方法:看到 AI 输出里出现"基于指定的 X""按照给定的 Y",就要回头查 X/Y 到底是谁给的——很可能是它自己上一轮编的。

### 选择题公式渲染成乱码(2026-07-20):不是解析/渲染 bug,是提示词坑
- 现象:选项里 $\lim_{h\to 0}\frac{...}{h}$ 渲染成 lim_{hto0}frac...(反斜杠全没)。
- 排查:实测 repairJsonLatex(单/双反斜杠都保留)和 MD 渲染管线(带反斜杠进=带反斜杠出)【都是好的】;全库也没有剥反斜杠的 sanitizer。
- 真根因:出题提示词写着"数学公式用 $...$ 包裹,【不要裸露反斜杠命令】"——本意是"别让命令裸在 $ 外",但模型读成了"别用反斜杠",于是在 $...$ 里写成 lim/frac/to 裸词。⇒ 数据里本就没有反斜杠。
- 修:三处出题提示词改成"必须用标准 LaTeX 且【务必带反斜杠】,例:$\lim_{h\to 0}$、$\frac{a}{b}$、$\sqrt{x}$、$\theta$;绝不要写成 lim/frac/to 裸词"。
- 教训:**"渲染坏了"先别默认是渲染 bug**——很可能是【喂进来的数据本身就是坏的】,而数据坏很可能是【提示词把模型带偏了】。排查顺序:先确认数据对不对(解析/存储),再看渲染。

## 2026-07 v12/v13 复测批 + 资料上传/超大PDF重构(Will 逐条)
反复出现的元病:**"同一件事只改了一半"**——一处改了、同类的另一处没跟上。本批多个 bug 都是这个(H8 计数、reorder、切考试漏刷 mastery、聊天条数统计、上传块与后端护栏)。改一处务必全库搜同类。

- **杀手汇报"说了没做"三连(根子相同)**:出错时代码静默兜底、工具返回假成功、act日志如实记成功、杀手如实转述 → 杀手一直诚实,是**工具在撒谎**。planWithAI 吞异常回退、assign_practical_task `.catch(()=>null)` 全失败仍 ok:true、checkInSyllabus unsure 静默放行。→ 元规则:砖头宁可如实报失败、绝不静默兜底(见上文元规则区)。
- **H8 穿越计数**:写入端 attempts 用 nowStamp()(带偏移)、读取端用 date('now')(真实钟)→两边对不上。修 /api/daily + homeData.js(首页统计块是另一条路径,漏了)。app/page.js 补 setReqUser。
- **P5-10 reorder**:planWithAI 超时静默回退成默认计划仍报成功;且没有"撤销今日任务"的砖头(daily_plans 纳入 checkpoint)。reorder 加**写完自我核验**(读回实际顺序,不是意图)。
- **read_material 工具(新)**:填"有文件却读不出→杀手编借口/私自联网"的缺口。底层 rag.materialFilePart(File API)+generate。★元规则红线:达不到主人指定要求就上报、停下,严禁私自联网/编假技术借口/拿非要的内容顶替(但真技术问题必须如实说,别因怕犯规不敢报)。
- **不再每轮盲喂资料**:runLoop 删掉 materialParts(max:3)(白烧token+只喂最新3份→syllabus看不到)。改按需 read_material。听力音频走出题路径(generators)的独立 materialParts,不受影响。
- **超时/出错重发不从零重来**:chat_runs 加 resume_contents_json,出错时存已跑完的工具进度,重发接着用(不限时间,消费一次即清)。
- **合并考试先问结构**:exam_merge 决定门以前错绑在"两门都有日期且不同"上→没日期就闷头合并。改成 mode(merge/family)前置、与日期解绑。
- **选项弹窗**:ask_user_form 的 radio 支持 allowOther(自填)、checkbox 多选也加;通用规则=行动中"几个明确选项挑一个"的抉择一律用 radio 弹、别纯文字列。
- **竞技场去游戏化**:严肃学习工具、无"玩法/趣味"。先判断学习方法(哲学→辩论等),竞技场只是某些方法(debate/socratic)的载体;create_custom_mode/generate_custom_modes 措辞从"玩法/创意"改"学习方式/教学有效性"。只有主人明确要"变有趣"才走趣味(appGuide 第20条)。建考试后主动按【时间线+怎么学最合适+仅学习也判断考核形式】定制,临考冲刺去掉"学新知识"。
- **分块上传(Will 点子)**:>8MB 前端 File.slice 切 80MB/块逐块传→后端 append 临时文件→fs.rename 入库(零内存)。文件任意大小可存。护栏单块≤96MB、总量≤2GB。
- **超大PDF拆页读(lib/pdfIndex.js)**:>45MB PDF 入库后台 indexBigPdf(splitPdfBySize≤16MB整页→readImage生成检索要点+页码存chunks);read_material 遇 isHugePdf 走 readBigPdf(retrieve命中页段→extractPdfPages只抽相关页→只读那几页)。**不引入 pdf-parse**(Will:半吊子文字会误导;当初正是因此删的文字索引)。三种"块"别混:上传80MB(搬进服务器)/索引片16MB(喂Gemini)/Gemini读~50MB·1000页(Google限)。
- **上传/超大文件的 OOM**:Railway Hobby 每副本 8GB,小用户量下 OOM 基本不会发生(之前说"拖垮全站"是过度描述)。上传并发内存兜底=6GB 在途预算(非救急,便宜的防御)。
- **服务端错误 i18n**:lib/i18nServer.js `estr(lang,zh)`——认证路由服务端翻好再发(前端无需改),登录/注册走前端 t()。全站扫了 30 条中文 error。
- **提示词引号检查**:node --check【不可靠】(JS正则/除号歧义,整文件里会漏"双引号被中文提前截断");改用 scripts/lint-quotes.mjs 词法扫描,`npm run check` 调用,改完提示词先跑再 build。
