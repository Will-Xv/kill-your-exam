  ...
  return (
    <div className="flex min-h-[80vh] items-center justify-center relative">
      <select value={lang} onChange={(e) => setLang(e.target.value)}
        className="absolute top-0 right-0 rounded-lg border border-stone-300 bg-white px-2 py-1 text-sm text-stone-600">
        {LANGS.map(([code, label]) => <option key={code} value={code}>{label}</option>)}
      </select>
      <form onSubmit={submit} className="card w-full max-w-sm">
        <div className="text-center mb-4">
          <div className="text-4xl mb-2">📘</div>
          <h1 className="text-xl font-bold">{t("AI 备考助手")}</h1>
        </div>
        <div className="mb-4 flex rounded-xl bg-stone-100 p-1 text-sm">
          <button type="button" onClick={() => setTab("login")} className={`flex-1 rounded-lg py-2 ${tab === "login" ? "bg-white shadow font-medium" : "text-stone-500"}`}>{t("登录")}</button>
          <button type="button" onClick={() => setTab("register")} className={`flex-1 rounded-lg py-2 ${tab === "register" ? "bg-white shadow font-medium" : "text-stone-500"}`}>{t("注册")}</button>
        </div>
        <div className="space-y-3">
          ...
          <button className="btn w-full" disabled={busy || !username || !password}>{tab === "login" ? t("登录") : t("注册并进入")}</button>
        </div>
        <p className="text-xs text-stone-400 mt-4 text-center">{t("登录一次,这台设备一年内免登录")}</p>
      </form>
    </div>
  );
}
