...
export async function POST(req) {
  try {
    ...
    if (spread && Array.isArray(kpIds) && kpIds.length) {
      ...
      // 【只复用主人自己的题】小测要考"这个点最该考的那道",题库里攒的 AI 旧题是以前随机出的,不拿来充数;
      // 真题、必考原题、以及从他上传的文件里抄进题库的原题可以直接抽。freshFrom 用来放行"刚为这个点生成的新题"。
    ...
    const genMore = async (opts = {}) => {
      ...
      const mparts = await hitMediaParts(exam.id, hits);   // 【只挂命中的图/音频】文字要点走 hits(RAG);不再无差别附最多 6 份整份原件
