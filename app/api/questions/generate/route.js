...
【防泄题】组内不得答案泄露、不要高度相似。${lessons ? "\n【避免已知毛病】\n" + lessons : ""}` + (mparts.length ? "\n考生资料库中的图片/音频原件已作为附件提供,可据此出题。" : "") + langRule;
    ...
    if (reuse && results.length > 0 && need > 0) {
      ...
      return Response.json({ questions: results.slice(0, count).map(pub), kp: { id: kp.id, title: kp.title }, note: null });
    }

    // 题库没有现成题:这次必须现出(需要等 AI),顺带多存几道供下次秒开。
    if (need > 0) {
      const rows = await genMore();
      for (const q of rows) { if (results.length >= count) break; results.push(q); }
    }
