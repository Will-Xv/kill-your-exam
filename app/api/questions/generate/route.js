...
【防泄题】组内不得答案泄露、不要高度相似。${lessons ? "\n【避免已知毛病】\n" + lessons : ""}` + (mparts.length ? "\n考生资料库中的图片/音频原件已作为附件提供,可据此出题。" : "") + langRule;
      ...
      for (const q of (out.questions || [])) {
        ...
        if (q.qtype === "perform") {
          ...
          const body = JSON.stringify({ stem: q.stem, captureType: cap, mediaMaterialId, analyzeAudio: aa, countdownSec: p.countdownSec || 3, autoStopAfterMediaSec: p.autoStopAfterMediaSec || 7, maxDurationSec: 300, rubric: p.rubric || [], instructions: (p.instructions || "") + (autoMusic ? " 【配乐说明】所给音乐是系统自动附带的练习配乐,风格不一定完全贴合本考试,重点是跟上它的节奏与情绪表达;正式考试请用你自备的曲目,也可在「补充资料」上传替换。" : "") });
