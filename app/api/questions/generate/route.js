...
const genSchema = { type: "object", properties: { questions: { type: "array", items: { type: "object", properties: {
  ...
  options: { type: "array", items: { type: "string" } }, answer: { type: "string" }, explanation: { type: "string" }, difficulty: { type: "integer" }, audioId: { type: "integer" },
...
【防泄题】组内不得答案泄露、不要高度相似。${lessons ? "\n【避免已知毛病】\n" + lessons : ""}` + (mparts.length ? "\n考生资料库中的图片/音频原件已作为附件提供,可据此出题。" : "") + langRule;
      ...
      for (const q of (online.found || [])) {
        ...
        const info = insQ.run(exam.id, kp.id, q.qtype, JSON.stringify({ stem: q.stem, options: q.options || [], audioId: q.audioId || null }),
