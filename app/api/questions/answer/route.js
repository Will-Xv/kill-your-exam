  ...
  try {
    ...
    if (q.qtype === "short") {
      ...
      const gradeSchema = { type: "object", properties: { score: { type: "integer" }, feedback: { type: "string" },
        crossKp: { type: "array", description: "考生答案里【顺带】清楚体现出对【别的知识点】(不是本题知识点)的深刻理解或明显薄弱时列出;kpId 只能取自下面清单;要确凿才填,没有就空数组。", items: { type: "object", properties: { kpId: { type: "integer" }, kind: { type: "string", enum: ["understanding", "gap"] }, insight: { type: "string" } }, required: ["kpId", "kind"] } }
...
如果这份答案里【顺带】清楚体现出考生对【别的知识点】(不是本题知识点)的深刻理解或明显薄弱,请在 crossKp 里列出(kind=understanding/gap),kpId 只能取自下面清单,要确凿才填、宁缺毋滥,没有就空数组。本题知识点id=${q.kp_id || 0}(不要放进 crossKp)。可引用的知识点清单:\n${kpListStr}` + langInstruction(user.lang);
