...
import { isChampion, pickSticker } from "@/lib/leaderboard";
...
export async function GET() {
  ...
  const p = rows[0];
  return Response.json({ count: rows.length, canTaunt: isChampion(u.id),
    item: p ? { id: p.id, kind: p.kind, fromName: p.from_name, fromUser: p.from_user, sticker: p.sticker } : null });
}
...
export async function POST(req) {
  ...
  if (action === "send") {
    if (!isChampion(u.id)) return forbidden();
  ...
  if (action === "resolve") {
    ...
    } else if (reply === "retaunt" && row.kind === "disdain" && isChampion(u.id)) {
