...
export async function POST(req) {
  ...
  if (!user) return unauthorized();
    ...
    const mparts = await hitMediaParts(exam.id, hits, 2);   // 【只挂命中的图/音频】文字要点走上面的 RAG 检索,不再无差别附整份原件
...
${learnerKpContext(kpId) ? "【这位考生在此知识点上的历史(据此有的放矢:他之前的误区要重点讲清、他已掌握的可略讲)】\n" + learnerKpContext(kpId) + "\n" : ""}
${exam.exam_type === "performance" ? "这是一门艺术/表演/技能类考试,内容不涉及数学。【严禁】使用任何 LaTeX 或数学公式环境($$、$、\\begin{cases}、\\text{}、\\frac 等一律不要),时间、比例、步骤等直接用普通文字写(如 0–15 秒、放大到约 1.2 倍)。" : "仅在确有数学公式时才用 LaTeX 并用 $...$ 或 $$...$$ 包裹(不要裸露反斜杠命令);时间、比例、步骤等普通信息一律用普通文字,不要硬套公式。"}输出 Markdown,结构:## 核心概念 / ## 考点解析 / ## 易错点 / ## 一个例子。【排版·重要】每个标题(##/###)、每个块公式($$...$$)、每个列表项都必须【单独成行】,块与块之间空一行——【绝不要】把标题、块公式、列表和正文挤在同一行。语言平实,面向有行业经验的成年考生,不要废话。(标题也要翻译成目标语言)。如果资料语言与输出语言不同,专业术语可保留资料原文并附翻译,其余内容不要混合语言。` + (mparts.length ? `\n(考生资料库中的图片/PDF/音频原件已作为附件提供,请直接查看/听取——【重点看与「${kp.title}」相关的那些页和示意图/图表】,据此讲解,涉及图就把图讲清楚)` : "") + langInstruction(user.lang);
