...
const OP_LABEL = {
  ...
  exam_merge: "合并考试", ui_layout: "界面布局改动",
};
...
export default function Checkpoints() {
  ...
  async function rollback(id, label, kind) {
    if (!await confirmDialog(t("确定回档撤销这次「{x}」?这会把相关考试还原到该操作【执行前】的状态,当前之后的改动会被覆盖。").replace("{x}", label))) return;
    setBusy(kind + "-" + id); setMsg("");
    try {
      const d = await fetch("/api/checkpoints", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "rollback", checkpointId: id, kind }) }).then((r) => r.json());
      if (d.ok) { setMsg(t("已回档 ✓ 相关考试已还原。")); await load(); }
      else setMsg((d.error || t("回档失败")) + "");
    } catch { setMsg(t("回档失败")); }
    setBusy(0);
  }
  async function redo(id, kind) {
    setBusy(kind + "-" + id); setMsg("");
    try {
      const d = await fetch("/api/checkpoints", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "redo", checkpointId: id, kind }) }).then((r) => r.json());
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 pb-24 pt-6 md:pt-20">
      ...
      <p className="mt-2 text-sm text-[#8a7a54]">{t("对知识树/考试结构的每一次大改(重建、合并、复制、挂父、开汇总等)都自动留了一个还原点。想撤就点「撤销」;撤销后又反悔,点「重做」恢复到撤销前——每条记录都能撤销↔重做反复来回。默认保留最近 40 次、最多 60 天,更早的自动清掉。")}{" "}{t("界面布局的改动(挪栏目、隐藏、导航停靠、首页布局等)也一并记录在这里,同样能撤销/重做。")}</p>
        ...
        : (
          <>
            <div className="mt-4 space-y-2">
              {list.map((c) => (
                <div key={c.kind + "-" + c.id} className={`card flex items-center justify-between gap-3 py-3 ${c.undone ? "opacity-60" : ""}`}>
                  ...
                  {c.undone
                    ? (c.redoable
                        ? <button className="btn-ghost shrink-0 text-sm" disabled={busy === c.kind + "-" + c.id} onClick={() => redo(c.id, c.kind)}>{busy === c.kind + "-" + c.id ? t("重做中…") : "↪️ " + t("重做")}</button>
                        : <span className="shrink-0 text-xs text-slate-400">{t("已撤销")}</span>)
                    : <button className="btn-ghost shrink-0 text-sm" disabled={busy === c.kind + "-" + c.id} onClick={() => rollback(c.id, t(OP_LABEL[c.op] || c.op) + (c.names && c.names.length ? " · " + c.names.join(", ") : ""), c.kind)}>{busy === c.kind + "-" + c.id ? t("回档中…") : "↩️ " + t("撤销")}</button>}
                </div>
              ))}
            </div>
