...
export default function Collector() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-black">🧲 {t("浏览器采集 / Agent")}</h1>
      <p className="text-slate-500">{t("用 Chrome 扩展,把已登录的学习网站内容采集进资料库,甚至让 agent 自动翻页采集。全程不接触你的密码。")}</p>
      ...
      <div className="card">
        ...
        <ol className="list-decimal space-y-2 pl-5 text-sm text-slate-600">
          ...
          <li>{t("在学习网站登录后:用「手动采集」抓当前页,或切到「🤖 Agent 自动」填个目标让它自己翻页采集。")}</li>
        </ol>
      </div>

      <div className="card border-amber-200 bg-amber-50">
        <h2 className="font-bold text-amber-900">🛡️ {t("安全说明")}</h2>
        <p className="mt-1 text-sm text-amber-800">{t("Agent 只会读取内容、点击翻页/章节类链接,系统强制禁止点击任何「提交/购买/支付/删除/退出」按钮,绝不填表、不花钱、不改你的账户。首次建议在纯阅读页面上试用并留意日志。")}</p>
      </div>
    </div>
  );
}
