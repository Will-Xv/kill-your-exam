...
export default function Chat() {
  ...
  const suggestions = [t("帮我看看我现在学得怎么样"), t("我觉得有一章我已经很熟了,想少花时间"), t("帮我搜一下这门考试最新的公告")];
  return (
    <div className="flex flex-col md:mt-14" style={{ height: "calc(100dvh - 130px)" }}>
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pb-3">
        ...
        {busy && <div className="max-w-[85%] rounded-2xl px-4 py-2.5 text-sm bg-white border border-stone-200 text-stone-400 animate-pulse">{t("正在思考(可能需要查资料/改文档,请稍候)…")}</div>}
        <div ref={bottom} />
      </div>
      <div className="flex gap-2 pt-2">
        <input className="input flex-1" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder={t("说说你的想法…")} />
        <button className="btn" onClick={() => send()} disabled={busy || !input.trim()}>{t("发送")}</button>
      </div>
    </div>
  );
}
