...
export default function Materials() {
  ...
  async function del(id) {
    if (!confirm("确定删除这份资料?相关检索内容也会移除。")) return;
  ...
  return (
    <div className="space-y-4 md:mt-14">
      ...
      <div className="card space-y-2">
        ...
        {files.length > 0 && <button className="btn w-full" onClick={upload} disabled={busy}>上传 {files.length} 个文件</button>}
        {log && <p className="text-sm text-emerald-700 animate-pulse">{log}</p>}
        <p className="text-xs text-stone-400">支持 PDF、Word、文本、图片(手机拍照即可)。扫描版 PDF 请转成图片上传。</p>
      </div>
      {checklist.length > 0 && (
        <div className="card space-y-1">
          <h2 className="font-semibold text-sm mb-1">资料收集清单({done}/{checklist.length})</h2>
          {checklist.map((c, i) => (
          ...
          <div key={m.id} className="card flex items-center justify-between py-3">
            <div>
              ...
              <p className="text-xs text-stone-500">
                {m.status === "ready" && `✓ 已入库(${m.chunk_count} 段)`}
                {m.status === "processing" && "⏳ 处理中"}
                {m.status === "failed" && <span className="text-red-600">✗ {m.error}</span>}
              </p>
            </div>
            <button className="text-stone-400 hover:text-red-600 text-sm" onClick={() => del(m.id)}>删除</button>
          </div>
        ))}
        {!list.length && <p className="text-center text-stone-400 text-sm py-6">还没有资料。资料是 AI 讲对、出对题的地基。</p>}
      </div>
    </div>
  );
}
