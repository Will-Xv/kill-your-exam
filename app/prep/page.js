    ...
    <div className="space-y-4">
      ...
      {Object.entries(byCat).map(([cat, items]) => (
        <div key={cat} className="card">
          ...
          <ul className="space-y-1.5">{items.map((x, i) => <li key={i} className="flex gap-2 text-sm"><span className="text-amber-500">✓</span>{x}</li>)}</ul>
        </div>
      ))}
            ...
            {(prep.knowledgeCheck.questions || []).map((q, i) => {
              ...
              return (
                <div key={i} className="rounded-xl bg-white border border-slate-100 p-3">
                  <p className="text-xs text-slate-400">{q.topic} · <span className="text-amber-600">{RE[q.reason] || q.reason}</span></p>
