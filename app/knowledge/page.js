...
export default function Knowledge() {
  ...
  return (
    <div className="space-y-4 md:mt-14">
      <h1 className="text-2xl font-bold">{t("掌握度矩阵")}</h1>
      <div className="card flex justify-around text-center text-sm">
        {Object.entries(LEVEL).map(([k, [label, color]]) => (
          <div key={k}><div className={`mx-auto h-3 w-3 rounded-full ${color} mb-1`} /><b>{count(k)}</b> {t(label)}</div>
        ))}
      </div>
          ...
          <div className="space-y-1">
            {kps.map((kp) => (
              <a key={kp.id} href={`/study?kp=${kp.id}`} className="flex items-center justify-between rounded-lg px-2 py-1.5 text-sm hover:bg-stone-50">
                ...
                <span className="text-xs text-stone-400">{kp.attempts ? `${kp.accuracy}% · ${kp.attempts}` : "—"}</span>
              </a>
            ))}
          </div>
        </div>
      ))}
      <p className="text-xs text-stone-400">{t("掌握度按作答记录规则计算(近期作答权重更高)。🟢有资料 🟡部分 ⚪无资料。")}</p>
    </div>
  );
}
