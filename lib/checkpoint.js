...
export function snapshot(userId, examIds, { op = "op", label = "", runId = null } = {}) {
  ...
  // review_queue 通过 question 归属这些考试
...
export function restore(checkpointId, userId) {
  ...
  const ids = JSON.parse(cp.exam_ids);
  const inSql = "(" + ids.map(Number).join(",") + ")";
  const tx = db.transaction(() => {
    // 先删 review_queue(依赖 question),再删各表
  ...
  tx();
  db.prepare("UPDATE checkpoints SET undone=1 WHERE id=?").run(cp.id);
...
export function listCheckpoints(userId, limit = 20) {
  return db.prepare("SELECT id, run_id, exam_ids, op, label, created_at, undone FROM checkpoints WHERE user_id=? ORDER BY id DESC LIMIT ?").all(userId, limit)
    .map((c) => ({ ...c, examIds: JSON.parse(c.exam_ids || "[]") }));
}
// 最近一个还没撤销的检查点(“撤销刚才那次”)
