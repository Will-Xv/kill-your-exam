...
export function applyToExam() {
  const p = S.working || placementNow();
  try { fetch("/api/ui-items", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ placement: p, scope: "exam" }) }).then(() => set({ examPlacement: clone(p), working: null })); } catch {}
  persist();
}
// 重置当前考试的布局覆盖(回到全局默认)。
