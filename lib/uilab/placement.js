...
import { WHERES, defaultPlacement, clone, applyMove } from "@/lib/uilab/placementCore";
export { WHERES, defaultPlacement };
...
export function initClient() {
  ...
  emit();
  fetch("/api/ui-items").then((r) => (r.ok ? r.json() : null)).then((d) => { if (d) set({ publishedDefault: d.placement || null, examPlacement: d.examPlacement || null }); }).catch(() => {});
}
