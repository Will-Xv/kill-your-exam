...
let S = { enabled: false, isDesktop: false, editing: false, activeId: null, presets: [], working: null, natural: null, lastReverted: null, guides: [], publishedDefault: null, past: [], future: [], loaded: false };
const subs = new Set();
...
function appliedLayout() {
  if (S.editing || !S.isDesktop) return null;
  const p = activePreset();
  if (S.enabled && p) return p.layout;      // 开发者本地在用的布局优先(仅自己可见)
  if (S.publishedDefault) return S.publishedDefault; // 已发布的全站默认(所有用户)
  return null;
}
export function hasHomeLayout() { return (S.editing && S.enabled && S.isDesktop) || !!appliedLayout(); }
...
export function enterEdit(seeds) { const p = activePreset(); const nat = seeds || {}; const working = p ? { ...clone(nat), ...clone(p.layout) } : clone(nat); set({ editing: true, natural: working, working, past: [], future: [] }); }
