...
let S = { enabled: false, isDesktop: false, editing: false, activeId: null, presets: [], working: null, natural: null, lastReverted: null, guides: [], drop: null, publishedDefault: null, examLayout: null, past: [], future: [], loaded: false };
const subs = new Set();
...
export function initClient() {
  ...
  emit();
  fetch("/api/ui-layout").then((r) => r.ok ? r.json() : null).then((d) => { if (d) { S = { ...S, publishedDefault: d.layout || null, examLayout: d.examLayout || null }; emit(); } }).catch(() => {});
}
export function refreshLayoutServer() { if (typeof window === "undefined") return; fetch("/api/ui-layout").then((r) => r.ok ? r.json() : null).then((d) => { if (d) set({ publishedDefault: d.layout || null, examLayout: d.examLayout || null }); }).catch(() => {}); }
...
function appliedLayout() {
  ...
  if (S.examLayout) return S.examLayout;              // 本考试布局(杀手/手动设)最优先 —— 和 per-exam 放置表一致的层级
  const p = activePreset();
  if (S.enabled && p) return p.layout;               // 其次:开发者本地在用的预设
  if (S.publishedDefault) return S.publishedDefault;  // 再次:已发布的全站默认
  return null;
}
...
export function enterEdit(ids) {
  ids = ids || [];
  const p = activePreset();
  const base = (p && p.layout && p.layout.v === 2) ? p.layout : null;
  let working;
  if (base) working = normalizeLayout(base, ids);
  else {
    const hasKiller = ids.includes("__killer");
    const content = ids.filter((x) => x !== "__killer");
    working = normalizeLayout(hasKiller
      ? { v: 2, template: "lr", zones: { a: content, b: ["__killer"] } }
      : { v: 2, template: "single", zones: { a: content } }, ids);
  }
  set({ editing: true, working, natural: clone(working), past: [], future: [] });
}
export function exitEdit() { if (S.editing) set({ editing: false }); }
// 点「完成」时:把当前编辑保存并覆盖当前生效的布局(在用预设→覆盖预设;否则→覆盖/发布默认),再退出。
export function saveOverwriteCurrent() {
  if (S.working) {
    const p = activePreset();
    if (p) overwriteActive();
    else publishDefault();
  }
  exitEdit();
}
...
export function toggleScrollbar() { if (!S.working) return; pushHistory(); S.working = { ...S.working, scrollbar: !S.working.scrollbar }; emit(); }
