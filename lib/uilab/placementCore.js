...
export const DESKTOP_DEFAULT = [
  ...
  { item: "arena", where: "morefeatures", order: 4 }, { item: "tasks", where: "morefeatures", order: 5 },
...
export function normalizePlacement(pl, allItemIds, refPl) {
  ...
  for (const bp of ["desktop", "mobile"]) {
    ...
    for (const id of allItemIds) {
      ...
      const maxOrder = arr.filter((e) => e.where === where).reduce((m, e) => Math.max(m, e.order), -1);
      arr.push({ item: id, where, order: maxOrder + 1 });
    }
  }
  return out;
}
