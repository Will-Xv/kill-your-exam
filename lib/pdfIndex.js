...
// 建索引时【顺手】问一句这份文件里有没有成套的现成题目 —— 反正原件已经读了一遍,这一问几乎不要钱。
// 只有答 yes 的,后面才值得再花一次钱把题抄进题库(见 lib/paperExtract.js);讲义/笔记零浪费。
const HAS_Q_ASK = `\n最后【另起一行】单独输出一行:HAS_QUESTIONS: yes 或 HAS_QUESTIONS: no —— 这份文件里是不是含有【成套的现成题目】(试卷、习题册、练习题、带编号的题目列表都算 yes;纯讲义、课本正文、笔记、大纲算 no)。这一行不属于上面的内容,不要混进去。`;
function takeHasQ(out, materialId) {
  const txt = String(out || "");
  const m = /HAS_QUESTIONS\s*[:：]\s*(yes|no)/i.exec(txt);
  if (m) { try { db.prepare("UPDATE materials SET has_questions=? WHERE id=?").run(/yes/i.test(m[1]) ? 1 : 0, materialId); } catch {} }
  return txt.replace(/^.*HAS_QUESTIONS\s*[:：].*$/gim, "").trim();
}
  ...
  for (const pc of pieces) {
    ...
    try {
      summary = await readImage(pc.buffer, "application/pdf",
        `这是一份大文档的第 ${pc.startPage}–${pc.endPage} 页。请用${lang && lang !== "zh" ? "该文档所用的语言" : "中文"}列出这几页的【关键主题、术语、涉及的表格/图/条款编号】,做成一段便于以后【检索定位】的要点(要包含足以判断"这几页讲什么"的关键词;别逐字抄全文,也别编页面上没有的内容)。` + (made === 0 ? HAS_Q_ASK : ""));
    } catch { summary = ""; }
    const txt = (made === 0 ? takeHasQ(summary, materialId) : String(summary || "").trim());
...
export async function indexPdfOutline(materialId, examId, lang) {
  ...
  return await saveOutline(takeHasQ(out, materialId), materialId, examId);
}
...
export async function indexImageMaterial(materialId, examId, mime, lang) {
  ...
  try {
    txt = await readImage(buf, mime || "image/png",
      ...
      (lang && lang !== "zh" ? "用图片所用的语言。" : "用中文。") + `别编造图上没有的东西;只输出内容本身。` + HAS_Q_ASK);
  } catch { return 0; }
  txt = takeHasQ(txt, materialId);
