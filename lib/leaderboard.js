...
// 排行榜完整数据(含 canTaunt/me),供 API 路由与首页服务端渲染共用,保证两边一致。
export function leaderboardPayload(u) {
  const lb = leaderboard();
  const maps = rankMaps();
  ...
  return { weekly: deco(lb.weekly), total: deco(lb.total), champion: lb.champion, totalChampion, me: { id: u.id, username: u.username } };
}
