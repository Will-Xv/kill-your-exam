...
export async function autoAdjustExamUi(exam, user, dossierText) {
  try {
    ...
    for (const fid of ["mock", "prep", "performances", "tasks"]) {
      ...
      for (const bp of ["desktop", "mobile"]) {
        ...
        else if (fid === "tasks") pl = applyMove(pl, bp, fid, "morefeatures"); // 相关的实践作业默认是隐藏的→凸显到更多功能
        // mock/prep/performances 保留时留在默认位置,不挪
      }
    }
