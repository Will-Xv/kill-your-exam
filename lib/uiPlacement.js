...
// —— 让 UI 布局改动也进「回档(后悔药)」页(P2-17):按用户列出、可任意点还原/重做/清空。ui_events 已存 before/after/skey,直接复用。
export function listUiCheckpoints(userId) {
  ensureEvents();
  return db.prepare("SELECT id, exam_id, summary, undone, before_json, after_json, created_at FROM ui_events WHERE user_id=? ORDER BY id DESC LIMIT 40").all(userId).map((e) => {
    const nm = (() => { try { return db.prepare("SELECT name FROM exams WHERE id=?").get(e.exam_id)?.name || null; } catch { return null; } })();
    return { id: e.id, examId: e.exam_id, examName: nm, summary: e.summary || "", undone: !!e.undone, created_at: e.created_at, redoable: !!e.undone && (e.after_json != null) };
  });
}
export function restoreUiEvent(id, userId) {
  ensureEvents();
  const ev = db.prepare("SELECT * FROM ui_events WHERE id=? AND user_id=?").get(Number(id), userId);
  if (!ev) throw new Error("ui event not found");
  const sk = ev.skey || key(ev.exam_id);
  if (ev.before_json) setSetting(sk, ev.before_json); else { try { db.prepare("DELETE FROM settings WHERE key=?").run(sk); } catch {} }
  db.prepare("UPDATE ui_events SET undone=1 WHERE id=?").run(ev.id);
  return { summary: ev.summary || "" };
}
export function redoUiEvent(id, userId) {
  ensureEvents();
  const ev = db.prepare("SELECT * FROM ui_events WHERE id=? AND user_id=?").get(Number(id), userId);
  if (!ev) throw new Error("ui event not found");
  if (!ev.undone) throw new Error("nothing to redo");
  const sk = ev.skey || key(ev.exam_id);
  if (ev.after_json) setSetting(sk, ev.after_json); else { try { db.prepare("DELETE FROM settings WHERE key=?").run(sk); } catch {} }
  db.prepare("UPDATE ui_events SET undone=0 WHERE id=?").run(ev.id);
  return { summary: ev.summary || "" };
}
export function clearUiEvents(userId) { ensureEvents(); try { return db.prepare("DELETE FROM ui_events WHERE user_id=?").run(userId).changes; } catch { return 0; } }
...
export async function autoAdjustExamUi(exam, user, dossierText) {
  try {
    ...
    // 只留一条【数据事实】兜底(不是按类型的硬规则):这门考试【已经真有实践作业】,就别把「实践作业」栏藏了。其余全按 AI(它现在知道每个栏目干什么)。
