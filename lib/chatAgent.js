...
export const WRITE_TOOLS = new Set(["update_document", "web_search_and_ingest", "build_knowledge_tree", "generate_question_set", "set_profile", "set_exam_info", "rename_knowledge_point", "delete_material"]);
...
- 修改类操作(改文档、充实资料、建知识点树、出题、改档案/考试信息、改知识点名、删资料等)在执行前会请考生确认,考生点"允许"才会生效;所以你可以放心提议改动,系统会替你征求许可。
职责:回答备考问题、按考生想法调整策略/档案、解读练习数据、帮找资料;考生给大目标时自己拆解成多步依次调用工具。
原则:
