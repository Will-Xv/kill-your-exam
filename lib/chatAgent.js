...
export const WRITE_TOOLS = new Set(["web_search_and_ingest", "delete_material", "delete_knowledge_point", "clear_questions", "browser_task", "build_knowledge_tree"]);
...
export const functionDeclarations = [
  ...
  { name: "build_knowledge_tree", description: "重新生成整个知识点树(会删掉现有知识点重建,属于危险操作,会弹确认)。【必须先问主人】旧的做题记录和掌握度怎么处理,再据此设 retain:keep=把旧记录按语义迁移到新知识点(默认、最稳);summarize=把旧表现浓缩成观察挂到新知识点、清掉原始做题记录;none=清掉记录与观察干净重来(题库保留)。不要替主人擅自决定 retain。", parameters: { type: "object", properties: { retain: { type: "string", enum: ["keep", "summarize", "none"] } } } },
...
export async function execTool(name, args, exam, user) {
  switch (name) {
    ...
    case "build_knowledge_tree": { const r = await rebuildKnowledgeTree(exam, user.lang, args.retain || "keep"); const mm = { keep: "完全保留旧记录", summarize: "旧表现已浓缩为观察", none: "已清空旧记录" }[r.mode] || r.mode; return { ok: true, note: `已重建知识点树:${r.chapters} 章 ${r.points} 点(${mm})` }; }
...
async function makePlan(ask, user) {
  const toolList = functionDeclarations.map((f) => `- ${f.name}: ${(f.description || "").slice(0, 60)}`).join("\n");
  ...
  const prompt = `你是「杀手」的规划模块。判断主人这次的请求是不是【复杂/多步/有先后依赖/会动到知识点·记录·题库·大批量生成】的任务。
...
只能用下列真实能力来规划,别编不存在的:
${toolList}
主人的请求:${String(ask).slice(0, 2000)}` + langInstruction(user.lang);
...
export async function agentLoop(runId, contents, exam, user, toolNotes = []) {
  ...
  try {
    ...
    // 规划:复杂/动系统的请求先出一份可预览的执行计划(简单请求跳过)
    try {
      const lu = [...contents].reverse().find((c) => c.role === "user" && Array.isArray(c.parts));
      ...
      if (maybeComplex(ask)) {
        ...
        if (plan && plan.complex && Array.isArray(plan.steps) && plan.steps.length) {
          const ps = plan.steps.slice(0, 8);
          addStep(runId, "plan", JSON.stringify({ summary: plan.summary || "", steps: ps }));
          system += "\n\n【本次任务的执行计划(已展示给主人)】请严格按此顺序推进,做完一步再下一步;涉及删除/清空/重排/重建等改动务必遵守计划里写明的保留策略,该弹确认的工具照常弹:\n" + ps.map((s, i) => `${i + 1}. ${s.title}${s.detail ? " — " + s.detail : ""}`).join("\n");
        }
      }
    } catch {}
    for (let i = 0; i < 12; i++) {
      ...
      if (calls.some((c) => WRITE_TOOLS.has(c.name))) {
        const token = crypto.randomBytes(16).toString("hex");
        const actions = calls.map((c, idx) => WRITE_TOOLS.has(c.name) ? { idx, tool: c.name, desc: describe(c.name, c.args || {}) } : null).filter(Boolean);
        db.prepare("UPDATE chat_runs SET status='pending', token=?, actions_json=?, pending_contents_json=?, pending_calls_json=?, updated_at=datetime('now') WHERE id=?")
        ...
        try { pushUser(user.id, { title: "🔐 需要你确认", body: "杀手想做一个改动,等你在 App 里确认", url: "/chat" }).catch(() => {}); } catch {}
        return;
      }
      const parts = [];
      for (const fc of calls) {
...
export function startRun(exam, user, contents) {
  const ins = db.prepare("INSERT INTO chat_runs(exam_id,user_id,status,steps_json) VALUES(?,?,?,?)").run(exam.id, user.id, "running", "[]");
