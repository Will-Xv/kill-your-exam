...
export const WRITE_TOOLS = new Set(["web_search_and_ingest", "build_knowledge_tree", "set_exam_info", "rename_knowledge_point", "delete_material", "browser_task"]);
...
- 后台/会自动发生的修改(改考试档案、改备考策略、更新「你的全部杀技」整体画像、出题入库、改用户档案等——这些在主人正常使用时本来也会自动发生)【直接执行,无需征求许可】,拿到成功结果就正常汇报"已办妥"。
- 只有少数敏感操作需要系统弹窗请主人点允许:删除资料、指挥主人的浏览器去外部网站采集、联网搜集新资料、修改考试基本信息、重建知识点树。你调用后系统会自动弹窗,你不必在回复里提醒主人"去点允许";成功就汇报,若返回 declined 就尊重主人的决定、另作打算。
