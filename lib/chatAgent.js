...
- 【听懂意图就动手,别抠字眼等口令】主人说的常是【目标/范围】而非工具名(如"规划到期末""按考纲全学完""把没学的章节也排进来""复习整门课")。这类【意图清晰】的话【不算模糊】,别拿上一条当挡箭牌反复追问。你要自己判断当前知识树/题库是否已覆盖这个范围:若没覆盖(如现在只有某几章、而期末要考全书),就【主动发起】扩建/重建知识树把范围补齐、再排计划——【绝不】非等主人说出"改/重建知识树"这几个字才动。判断范围用 get_exam_info / list_knowledge_points / 看考纲资料。重建这种结构大改仍会自动弹确认(框里写明保留旧记录),但【发起】它是你分内该主动做的,不是等主人喊口令。
- 【"规划到期末"= 补齐整门课的真实内容,不是加一个叫"期末"的单元】期末/Final 不是一个知识点、也不是一个章节名。要把期末【考纲里还没建的真实内容单元】(如"多元函数最优化""二重积分""向量场与线积分"…)作为一个个【正常章节】补进知识树,章节名用真实主题,【绝对不要】建一个叫"期末/Final/考试名"的章节把东西全塞进去。不清楚期末考哪些章时,先查:看补充资料/考纲、联网搜这门课的 syllabus、或直接问主人"期末覆盖哪几章",【别凭空编】。重建/扩建时【不要】传 timeBudgetMin(那是给"想在X分钟内复习完的聚焦小测"用的,会把整门课压成一个以考试名命名的单章——正是错误示范);整门课/期末走正常多章模式。知识点标题要【简短】,别把整句解释或公式塞进标题。
...
async function agentTurn(args) {
  const ok = (r) => (r && ((r.functionCalls && r.functionCalls.length) || (r.text && r.text.trim())));
  let res = await generate(null, args);
  if (ok(res)) return res;
  await new Promise((r) => setTimeout(r, 700));
  const res2 = await generate(null, args);
...
export async function runAgent(contents, exam, user, toolNotes) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    if (!calls || !calls.length) {
      const reply = (response.text && response.text.trim()) || emptyReplyMsg(response);
