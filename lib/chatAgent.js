...
function declsFor(user) { return [...functionDeclarations.filter((d) => !d.devOnly || user?.is_developer), ...brickToolDecls(user)]; }
// 是否写操作(需主人点允许):内置写工具,或 write=true 的砖头
export function isWrite(name) { return WRITE_TOOLS.has(name) || !!getBrick(name)?.write; }
// 无考试时也能用的、与具体考试无关的工具
const NOEXAM_TOOLS = new Set(["web_search", "read_overall_profile", "update_overall_profile", "get_profile", "set_profile", "list_memory", "forget_fact"]);
