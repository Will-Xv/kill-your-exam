...
export const functionDeclarations = [
  ...
  { name: "ui_read", description: "读取当前界面定制状态:全部功能注册表(含曾用/已退役的名称与图标,用于查重)+ 本考试当前功能放置表(每个功能在导航栏/更多/更多功能/大模块/隐藏的哪处)。要改 UI 前先读它,别凭空臆想界面长什么样。" },
  ...
  { name: "ui_create_feature", devOnly: true, description: "新建一个功能(可放置的栏目)。id 用英文短横线(如 exam-sprint);name 显示名;icon 一个 emoji;href 点击后去的页面路径(如 /practice);desc 一句话说明。【名称和图标不得与任何现有或曾用(含已删)功能重复】,撞了会被拒绝并告诉你占用者。默认放到 hidden,再用 ui_move_item 摆位。", parameters: { type: "object", properties: { id: { type: "string" }, name: { type: "string" }, icon: { type: "string" }, href: { type: "string" }, desc: { type: "string" }, where: { type: "string", enum: ["nav", "more", "morefeatures", "zone", "hidden"] } }, required: ["id", "name", "icon", "href"] } },
  { name: "ui_remove_feature", devOnly: true, description: "移除(退役)一个功能:从界面拿掉,名称/图标仍保留占用(可恢复)。传 featureId。", parameters: { type: "object", properties: { featureId: { type: "string" } }, required: ["featureId"] } },
  { name: "ui_rename_feature", devOnly: true, description: "给【自定义】功能改名/换图标(内置功能暂不支持改名)。新名称/图标若与任何现有或曾用功能重复会被拒绝并告知占用者。传 featureId + name?/icon?。", parameters: { type: "object", properties: { featureId: { type: "string" }, name: { type: "string" }, icon: { type: "string" } }, required: ["featureId"] } },
