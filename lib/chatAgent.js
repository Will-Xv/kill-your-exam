...
export const WRITE_TOOLS = new Set(["web_search_and_ingest", "delete_material", "delete_knowledge_point", "clear_questions", "browser_task"]);
...
export const functionDeclarations = [
  ...
  { name: "clear_questions", description: "一键清空题库里【还没做过】的题(用于换语言/换题型后重出题;已做过的题会保留)。可选 kpTitle 只清某知识点的。这是最快的清库方式,不要逐题删。", parameters: { type: "object", properties: { kpTitle: { type: "string" } } } }
];
...
export function describe(name, args) {
  switch (name) {
    ...
    case "clear_questions": return `清空${args.kpTitle ? "「" + args.kpTitle + "」" : "该考试"}还没做过的题(换题/换语言用,已做过的保留)`;
...
export async function execTool(name, args, exam, user) {
  switch (name) {
    ...
    case "clear_questions": {
      let sql = "DELETE FROM questions WHERE exam_id=? AND id NOT IN (SELECT question_id FROM attempts)";
      const params = [exam.id];
      if (args.kpTitle) { const kp = db.prepare("SELECT id FROM knowledge_points WHERE exam_id=? AND parent_id IS NOT NULL AND title LIKE ? LIMIT 1").get(exam.id, `%${args.kpTitle}%`); if (kp) { sql += " AND kp_id=?"; params.push(kp.id); } }
      const info = db.prepare(sql).run(...params);
      return { ok: true, note: `已清空 ${info.changes} 道还没做过的题;下次练习会按最新设置(语言/题型)重新出题。` };
    }
