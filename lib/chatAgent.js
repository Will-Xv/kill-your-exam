...
export async function runAgent(contents, exam, user, toolNotes) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    if (calls.some((c) => isWrite(c.name))) {
      ...
      const actions = calls.map((c, idx) => isWrite(c.name) ? { idx, tool: c.name, desc: confirmDesc(c.name, c.args || {}, user.lang) } : null).filter(Boolean);
...
export function actsBlock(runId) {
  ...
  return "\n\n【本轮真实改动记录·系统生成·不可更改】以下是你这一轮实际做成/没做成的每一件事(按发生顺序):\n" + lines +
    "\n汇报时【只能以这份记录为准】:标成功的才可以说做了;标没成功的必须如实说没做成;这份记录里【没有】的事情,一个字都不许说。用大白话说,别出现工具名/内部 ID。";
}
