...
export const functionDeclarations = [
  ...
  { name: "ui_read", description: "读取当前界面定制状态:全部功能注册表(含曾用/已退役的名称与图标,用于查重)+ 已发布的功能放置表(每个功能在导航栏/更多/更多功能/大模块/隐藏的哪处)。要改 UI 前先读它,别凭空臆想界面长什么样。", devOnly: true },
...
export async function execTool(name, args, exam, user) {
  ...
  switch (name) {
    ...
    case "ui_read": { const placement = getSetting("ui_item_placement", ""); const feats = listFeatures(); return { placement: placement || "(尚未发布功能放置表,当前用默认)", features: feats.map((x) => `${x.icon} ${x.name} [${x.feature_id}]${x.active ? "" : " (已退役,名称/图标仍占用)"}`).join("\n") }; }
