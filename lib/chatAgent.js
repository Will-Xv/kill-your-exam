...
function brickVisible(name, user) { return !!getBrick(name) && (!!user?.is_developer || brickPublished(name)); }
function visibleBricks(user) { return listBricks().filter((b) => user?.is_developer || brickPublished(b.name)); }
...
function isWrite(name) { return WRITE_TOOLS.has(name) || !!getBrick(name)?.write; }
...
export function describe(name, args) {
  switch (name) {
    ...
    default: { const b = getBrick(name); if (b) return `${b.title}` + (args && Object.keys(args).length ? `(${JSON.stringify(args).slice(0, 140)})` : ""); return name; }
  }
}
