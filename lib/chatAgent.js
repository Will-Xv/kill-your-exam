...
export const functionDeclarations = [
  ...
  { name: "send_file", description: "生成一个文件发给主人下载(比如错题整理、复习提纲、笔记汇总、导出题目、学习计划表等)。你把文件内容写在 content 里,选好格式;工具会返回下载链接——你【必须】在回复里用 Markdown 链接 [文件名](链接) 把它呈现给主人。format 支持:md(Markdown,默认)、txt(纯文本)、csv(表格,用逗号分隔、首行表头)、html。", parameters: { type: "object", properties: { filename: { type: "string", description: "文件名(可不带扩展名,会按 format 自动补)" }, content: { type: "string", description: "文件的完整文本内容" }, format: { type: "string", enum: ["md", "txt", "csv", "html"] } }, required: ["filename", "content"] } },
...
export async function execTool(name, args, exam, user) {
  switch (name) {
    ...
    case "send_file": {
      const fmt = ["md", "txt", "csv", "html"].includes(args.format) ? args.format : "md";
      const mime = { md: "text/markdown", txt: "text/plain", csv: "text/csv", html: "text/html" }[fmt];
      ...
      try { saveChatFile(id, Buffer.from(String(args.content || ""), "utf8")); } catch {}
