...
export const functionDeclarations = [
  ...
  { name: "build_knowledge_tree", description: "重新生成整个知识点树(会删掉现有知识点重建,属于危险操作,会弹确认)。默认 retain=keep(把旧记录按语义迁移到新知识点、最稳);可选 retain:keep=迁移旧记录(默认);summarize=把旧表现浓缩成观察、清掉原始做题记录;none=清掉记录与观察干净重来(题库保留)。★【怎么定 retain】:①主人【已经明确说了】怎么处理旧记录(如「记录别丢」「干净重来」)→直接按他说的传,不用再问;②主人【没说、而你判断这个选择对他有影响、需要他拍板】时→【用 ask_user_form 弹一个【单选】让他在三档里挑,不要用纯文字把三个选项列出来问】(选项:①保留并迁移旧做题记录和掌握度〔最稳·推荐〕②把旧表现浓缩成一句观察、清掉原始记录③清空记录干净重来),这正是「行动中需主人在几个明确选项里拍板」的典型场景;③拿不准但倾向保险→默认 keep 直接发起,确认框会写明保留策略、主人可当场否决。", parameters: { type: "object", properties: { retain: { type: "string", enum: ["keep", "summarize", "none"] }, timeBudgetMin: { type: "number", description: "【只有】当主人【明确说出一个很短的时间预算、要复习某一小块】时才传(如\"想一小时内复习完这几节\")。传了会把内容压成一个以考试名命名的单章并精简篇幅。【绝不能从范围词或考试名(整门课/期中/期末/某考试)去反推时间预算】——那属于正常多章重建,别传本参数。" }, emphasis: { type: "string", description: "本次侧重(只围绕它展开,可选)" } } } },
