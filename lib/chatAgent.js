import db, { getDocument, upsertDocument, getSetting } from "@/lib/db";
...
// 无考试时也能用的、与具体考试无关的工具
const NOEXAM_TOOLS = new Set(["web_search", "read_overall_profile", "update_overall_profile", "get_profile", "set_profile", "list_memory", "forget_fact", "ui_read", "list_learning_modes", "save_learning_mode", "activate_learning_mode", "delete_learning_mode"]);
...
export const functionDeclarations = [
  ...
  { name: "locate_material", description: "教材定位:主人给的复习指导若是「教材第X页第Y题 / p.42 ex.3 / 第五章习题5」这类【指向教材某处】的引用,用它在主人上传的资料里把对应内容找出来。返回命中的原文片段+出处(文件名/标题)、语义相近段落、以及图片类资料清单(它们的文字不在索引里,你要用多模态亲自看)。status: found/partial/not_found。【找不到或只是 partial 就如实告诉主人,绝不编造某页的题目内容】。", parameters: { type: "object", properties: { reference: { type: "string", description: "原样传入主人给的引用,如「教材第42页第3题」或「Chapter5 ex.2」" } }, required: ["reference"] }, devOnly: true },
  ...
  { name: "list_memory", description: "读取你对主人的长期记忆条目(本考试 + 全局,含 id/权重/时间/立场)。主人问『你到底记住了我哪些』时用。", devOnly: true },
  { name: "ui_read", description: "读取当前界面定制状态:全部功能注册表(含曾用/已退役的名称与图标,用于查重)+ 本考试当前功能放置表(每个功能在导航栏/更多/更多功能/大模块/隐藏的哪处)。要改 UI 前先读它,别凭空臆想界面长什么样。", devOnly: true },
  { name: "ui_move_item", description: "把一个功能移到某处(nav 导航栏 / more 更多菜单 / morefeatures 更多功能 / zone 首页大模块 / hidden 隐藏)。默认【电脑和手机一致】;只想改一端就传 breakpoint。改前先 ui_read;非必要不改、拿不准先问主人。按考试保存、留还原点、可 ui_undo 撤销。", parameters: { type: "object", properties: { featureId: { type: "string" }, where: { type: "string", enum: ["nav", "more", "morefeatures", "zone", "hidden"] }, breakpoint: { type: "string", enum: ["desktop", "mobile"], description: "省略=电脑和手机都改" }, index: { type: "integer", description: "在目标容器里的位置(省略=末尾)" } }, required: ["featureId", "where"] }, devOnly: true },
  { name: "ui_undo", description: "撤销本考试【最近一次】界面改动(还原到改动前)。", devOnly: true },
  { name: "ui_create_feature", description: "新建一个功能(可放置的栏目)。id 用英文短横线(如 exam-sprint);name 显示名;icon 一个 emoji;href 点击后去的页面路径(如 /practice);desc 一句话说明。【名称和图标不得与任何现有或曾用(含已删)功能重复】,撞了会被拒绝并告诉你占用者。默认放到 hidden,再用 ui_move_item 摆位。", parameters: { type: "object", properties: { id: { type: "string" }, name: { type: "string" }, icon: { type: "string" }, href: { type: "string" }, desc: { type: "string" }, where: { type: "string", enum: ["nav", "more", "morefeatures", "zone", "hidden"] } }, required: ["id", "name", "icon", "href"] }, devOnly: true },
  { name: "ui_remove_feature", description: "移除(退役)一个功能:从界面拿掉,名称/图标仍保留占用(可恢复)。传 featureId。", parameters: { type: "object", properties: { featureId: { type: "string" } }, required: ["featureId"] }, devOnly: true },
  { name: "ui_rename_feature", description: "给【自定义】功能改名/换图标(内置功能暂不支持改名)。新名称/图标若与任何现有或曾用功能重复会被拒绝并告知占用者。传 featureId + name?/icon?。", parameters: { type: "object", properties: { featureId: { type: "string" }, name: { type: "string" }, icon: { type: "string" } }, required: ["featureId"] }, devOnly: true },
  { name: "ui_migrate_ui", description: "把另一门考试(fromExamId)的整套功能放置表迁移(复制)到当前考试。", parameters: { type: "object", properties: { fromExamId: { type: "integer" } }, required: ["fromExamId"] }, devOnly: true },
  { name: "ui_set_nav_dock", description: "把导航栏停靠到某条边(top 顶部 / bottom 底部 / left 左侧竖排 / right 右侧竖排)。默认电脑和手机一致;只想改一端就传 breakpoint。这是【布局改动】,拿不准针对哪个平台就先问主人(见第6条规则)。可 ui_undo 撤销。", parameters: { type: "object", properties: { edge: { type: "string", enum: ["top", "bottom", "left", "right"] }, breakpoint: { type: "string", enum: ["desktop", "mobile"], description: "省略=电脑和手机都改" } }, required: ["edge"] }, devOnly: true },
  { name: "ui_home_layout_read", description: "读当前首页布局:用了哪个模板、杀手占哪一格(位置),以及所有可选模板和它们的格子位置。要改首页布局前先读它。", devOnly: true },
  { name: "ui_home_layout_set", description: "设置本考试的首页布局:选一个模板并把【杀手】放到某一格,从而把杀手在首页移到 左/右/上/下/某个角。其余格子自动填充首页内容。模板:single 整页 / lr 左右 / tb 上下 / quad 四格 / lsplit_r 左分上下·右整条 / l_rsplit 左整条·右分上下 / t_bsplit 上分左右·下整条 / tfull_bsplit 上整条·下分左右。killerZone 取 a/b/c/d(先用 ui_home_layout_read 看每个模板有哪些格、分别是什么位置)。【杀手必须占一格,绝不能隐藏】。可 ui_undo 撤销。", parameters: { type: "object", properties: { template: { type: "string", enum: ["single", "lr", "tb", "quad", "lsplit_r", "l_rsplit", "t_bsplit", "tfull_bsplit"] }, killerZone: { type: "string", enum: ["a", "b", "c", "d"] } }, required: ["template", "killerZone"] }, devOnly: true },
  { name: "ui_home_layout_off", description: "取消本考试的自定义首页布局,回到默认(杀手回到右侧常驻/手机浮动,依然可见)。可 ui_undo 撤销。", devOnly: true },
  { name: "ui_set_killer_home", description: "设置你自己(杀手)的显示形态:dock=占大格/常驻大面板(电脑右侧常驻或首页布局里的某一格),float=像手机一样浮动(一个 💬 圆按钮,点开是全屏抽屉)。你只有这两态。注意:电脑端如果首页是【整列(single)】布局,会自动变成浮动(没有侧栏空间)。【你绝不能把自己隐藏或移除;float 只是浮动,不是隐藏】。默认电脑手机一致,只改一端传 breakpoint。可 ui_undo 撤销。", parameters: { type: "object", properties: { mode: { type: "string", enum: ["dock", "float"] }, breakpoint: { type: "string", enum: ["desktop", "mobile"] } }, required: ["mode"] }, devOnly: true },
  { name: "forget_fact", description: "忘掉某一条长期记忆(软删除,可随时恢复)。主人明确要你忘掉某件事时用;先 list_memory 拿到 factId 再调。", parameters: { type: "object", properties: { factId: { type: "integer" } }, required: ["factId"] }, devOnly: true },
  { name: "save_learning_mode", description: "保存/更新一个【命名学习模式或配方】——把主人用大白话定的学习规则,存成一条可复用、可激活的规则集。name=模式名(如「冲刺模式」「苏格拉底模式」「先做题后讲」);rules=这套模式下你要遵守的规则,写清楚、可执行(例如:先5分钟讲概念→10分钟做题→5分钟复盘;数学类先给题、错了再反推概念;多用类比、少让我背)。scope=exam(只本考试)或 global(全局·你的全部杀技,所有考试通用)。activate=是否立即生效。生效后这套规则进入你的系统认知,你要一直遵守直到被停用。主人要你『记住以后怎么教/怎么排计划』时,就用它存成模式,而不是只答应一次。若规则里含【自动触发】,【除了 rules 里写清楚,还要在 triggers 里给出结构化触发器】,系统才会在真实做题/打开应用时确定性地自动执行。所有阈值都是参数、按主人说的数字填(引擎通用不写死)。\n【event=answer(做题后)】when 可选:consecutive_wrong(连错,配 n)、consecutive_correct(连对,配 n)、kp_consecutive_wrong(在【同一知识点】上连错,配 n)、accuracy_below(近期正确率过低,配 window=看最近几题、pct=低于百分之几)、mastery_below(当前知识点掌握度低于某档,配 level=weak|ok|mastered)、every(每 n 题一次,配 n)。\n【event=session(打开应用时,每日/每周级)】when 可选:daily_first(每天第一次)、weekly(每周某天,配 day=0~6,0=周日)、due_reviews_at_least(到期该复习的题≥n,配 n)、idle_days(已 n 天没做题,配 n)。\n【action 通用】:difficulty_down/difficulty_up(升降难度,可配 step)、difficulty_min/difficulty_max(锁最易/最难)、note(记一条观察,可配 text)、flag_review(把当前知识点标记为需重点复习)、insert_review(把当前题排进复习队列、尽快再考)、notify(给主人发一条站内信,配 title/text)。", parameters: { type: "object", properties: { name: { type: "string" }, rules: { type: "string" }, scope: { type: "string", enum: ["exam", "global"] }, activate: { type: "boolean" }, triggers: { type: "array", description: "结构化自动触发器(可选)。例:{event:\"answer\", when:\"kp_consecutive_wrong\", n:3, action:\"flag_review\"};{event:\"answer\", when:\"accuracy_below\", window:10, pct:60, action:\"difficulty_down\"};{event:\"session\", when:\"weekly\", day:0, action:\"notify\", title:\"每周回顾\", text:\"...\"};{event:\"session\", when:\"due_reviews_at_least\", n:5, action:\"notify\", text:\"你有该复习的题了\"}。", items: { type: "object", properties: { event: { type: "string", enum: ["answer", "session"] }, when: { type: "string", enum: ["consecutive_wrong", "consecutive_correct", "kp_consecutive_wrong", "accuracy_below", "mastery_below", "every", "daily_first", "weekly", "due_reviews_at_least", "idle_days"] }, n: { type: "integer" }, window: { type: "integer" }, pct: { type: "number" }, step: { type: "integer" }, level: { type: "string", enum: ["weak", "ok", "mastered"] }, day: { type: "integer" }, action: { type: "string", enum: ["difficulty_down", "difficulty_up", "difficulty_min", "difficulty_max", "note", "flag_review", "insert_review", "notify"] }, title: { type: "string" }, text: { type: "string" } } } } }, required: ["name", "rules"] }, devOnly: true },
  { name: "list_learning_modes", description: "列出主人所有学习模式/配方(全局+本考试)及是否已激活。改或引用模式前先读它。", devOnly: true },
  { name: "activate_learning_mode", description: "激活或停用一个已保存的学习模式(按名字)。active=true 激活、false 停用。主人说『进入冲刺模式』就激活对应模式;说『退出/切回长期模式』就停用旧的、激活新的。", parameters: { type: "object", properties: { name: { type: "string" }, active: { type: "boolean" } }, required: ["name", "active"] }, devOnly: true },
  { name: "delete_learning_mode", description: "删除一个学习模式(按名字)。", parameters: { type: "object", properties: { name: { type: "string" } }, required: ["name"] }, devOnly: true },
...
export async function execTool(name, args, exam, user) {
  ...
  switch (name) {
    ...
    case "list_learning_modes": { const ms = listModes(user.id, exam ? exam.id : null); return { modes: ms.map((m) => `「${m.name}」[${m.scope === "global" ? "全局" : "本考试"}]${m.active ? " ✅已激活" : " ⭘未激活"}:${(m.rules || "").slice(0, 140)}`).join("\n") || "(还没有任何学习模式)" }; }
    ...
    case "ui_read": {
      ...
      let placeStr = "(无考试上下文)";
      if (exam) { const pl = basePlacement(exam.id); placeStr = Object.entries(labels).map(([w, l]) => `${l}: ${(pl.desktop || []).filter((e) => e.where === w).map((e) => e.item).join("、") || "(空)"}`).join("\n"); }
      return { placement: placeStr, features: feats.map((x) => `${x.icon} ${x.name} [${x.feature_id}]${x.active ? "" : " (已退役,名称/图标仍占用)"}`).join("\n") };
    }
    case "ui_move_item": {
...
export function systemPrompt(exam, user) {
  ...
  const modes = user.is_developer ? activeModesDigest(user.id, exam ? exam.id : null) : ""; // 已激活的学习模式规则(仅开发者账号)
