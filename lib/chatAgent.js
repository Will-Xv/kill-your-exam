...
function assessBlock(exam) {
  try {
    ...
    // 【主人在资料页填的文字要真的送到你眼前】以前这里只取 c.item(条目名)、还只挑 must 且未完成的,
    // 于是主人在「其他文件或说明」自由文本框里写的整段笔记/大纲(存成 {kind:"qa", priority:"opt", done:true, answer:正文})
    // 被过滤掉、正文字段也从没被读 → 主人以为喂了资料,你却只能说"资料是空的"。
    // 而且它以前挂在"没有自评就直接 return"的后面,没自评时更是彻底送不出来。现在独立成块、与自评无关地always带上。
    const filled = cl.filter((c) => c && c.answer && String(c.answer).trim())
      .map((c) => `${c.item}:${String(c.answer).trim().slice(0, 400)}`).slice(0, 8);
    const notesBlock = filled.length
      ? `\n【主人在「资料」页里【亲手填写】的说明/笔记(这是他真的写下的内容,别再说"资料是空的"):\n${filled.join("\n")}\n据此排计划/出题时要真的用上这些内容,别给通用起点。】\n`
      : "";
    const sa = JSON.parse(exam.self_assessment || "null"); if (!sa) return notesBlock;
    ...
    if (!parts.length) return notesBlock;
