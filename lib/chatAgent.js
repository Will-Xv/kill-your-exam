...
import { listCheckpoints, lastCheckpoint, restore, addLesson, getLessons } from "@/lib/checkpoint";
...
export const WRITE_TOOLS = new Set(["web_search_and_ingest", "delete_material", "delete_knowledge_point", "clear_questions", "browser_task", "build_knowledge_tree", "rollback"]);
...
export const functionDeclarations = [
  ...
  { name: "rollback", description: "回档:把某次结构性操作还原到它执行前的状态。省略 checkpointId=撤销【最近一次】未撤销的操作(“撤销刚才那次”)。这是破坏性还原(会覆盖当前状态),会弹确认。若这次撤销是因为发现了问题/bug,请在 lesson 里用一句话写下教训(避免以后重犯),会记入你的长期教训库。", parameters: { type: "object", properties: { checkpointId: { type: "integer", description: "要回档到的检查点 id;省略=最近一次" }, lesson: { type: "string", description: "本次为何要撤销、以后要注意什么(可选但强烈建议)" } } } },
...
export async function execTool(name, args, exam, user) {
  switch (name) {
    ...
    case "rollback": {
      ...
      if (args.lesson) { try { addLesson(user.id, args.lesson); } catch {} }
      return { ok: true, note: `已回档:撤销了「${r.label || r.op}」(检查点 #${r.id})${args.lesson ? ",并记下教训" : ""}。相关考试已还原到该操作前的状态。` };
    }
    default: {
...
export async function runAgent(contents, exam, user, toolNotes) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    if (!calls || !calls.length) {
      ...
      db.prepare("INSERT INTO chat_messages(exam_id,role,content) VALUES(?,?,?)").run(rootExamId(exam.id), "model", reply);
...
export function startRun(exam, user, contents) {
  const ins = db.prepare("INSERT INTO chat_runs(exam_id,user_id,status,steps_json) VALUES(?,?,?,?)").run(rootExamId(exam.id), user.id, "running", "[]");
