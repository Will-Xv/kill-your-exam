...
export const functionDeclarations = [
  ...
  { name: "save_learning_mode", description: "保存/更新一个【命名学习模式或配方】——把主人用大白话定的学习规则,存成一条可复用、可激活的规则集。name=模式名(如「冲刺模式」「苏格拉底模式」「先做题后讲」);rules=这套模式下你要遵守的规则,写清楚、可执行(例如:先5分钟讲概念→10分钟做题→5分钟复盘;数学类先给题、错了再反推概念;多用类比、少让我背)。scope=exam(只本考试)或 global(全局·你的全部杀技,所有考试通用)。activate=是否立即生效。生效后这套规则进入你的系统认知,你要一直遵守直到被停用。主人要你『记住以后怎么教/怎么排计划』时,就用它存成模式,而不是只答应一次。若规则里含【自动触发】(如『连错n题自动降难度』『近期正确率低于60%就降难度』『每10题记一次薄弱点』),【除了 rules 里写清楚,还要在 triggers 里给出结构化触发器】,系统才会在真实做题时确定性地自动执行。所有阈值都是参数、按主人说的数字填(n=3就填3、n=5就填5,引擎通用不写死)。目前 event 只有 answer(做题后)。when:consecutive_wrong(连错,配 n)、consecutive_correct(连对,配 n)、accuracy_below(近期正确率过低,配 window=看最近几题、pct=低于百分之几)、every(每 n 题一次,配 n)。action:difficulty_down/difficulty_up(升降难度,可配 step 步长)、difficulty_min/difficulty_max(直接锁最易/最难)、note(记一条观察,可配 text)、flag_review(把当前知识点标记为需重点复习)。", parameters: { type: "object", properties: { name: { type: "string" }, rules: { type: "string" }, scope: { type: "string", enum: ["exam", "global"] }, activate: { type: "boolean" }, triggers: { type: "array", description: "结构化自动触发器(可选)。每项形如 {event:\"answer\", when:\"consecutive_wrong\", n:3, action:\"difficulty_down\"} 或 {event:\"answer\", when:\"accuracy_below\", window:10, pct:60, action:\"difficulty_down\"} 或 {event:\"answer\", when:\"every\", n:10, action:\"flag_review\"}。", items: { type: "object", properties: { event: { type: "string", enum: ["answer"] }, when: { type: "string", enum: ["consecutive_wrong", "consecutive_correct", "accuracy_below", "every"] }, n: { type: "integer" }, window: { type: "integer" }, pct: { type: "number" }, step: { type: "integer" }, action: { type: "string", enum: ["difficulty_down", "difficulty_up", "difficulty_min", "difficulty_max", "note", "flag_review"] }, text: { type: "string" } } } } }, required: ["name", "rules"] }, devOnly: true },
