...
- ★每步还要填 tools:这一步会用到的【工具名】数组(必须是下面清单里的真名,别编)。这决定了主人在同一张确认表里能一次性授权哪些改动——【漏填会导致执行到一半又去打扰他确认】,所以凡是会动数据的步骤务必把工具名写全。
只能用下列真实能力来规划,别编不存在的:
${toolList}
  ...
  try {
    ...
    if (maybeComplex(ask)) {
      ...
      if (plan && plan.complex && Array.isArray(plan.steps) && plan.steps.length) {
        ...
        const token = crypto.randomBytes(16).toString("hex");
        // 【D1·合成一张表】把计划里会动数据的工具抽出来当场变成勾选项,和计划一起呈现:
        // 主人一次提交 = 同意计划 + 授权这些改动;执行时已授权的不再弹确认(以前一句话要连点 4~5 次)。
        const _planTools = [];
        for (const st of planObj.steps) for (const tn of (st.tools || [])) {
          if (isWrite(tn) && !_planTools.includes(tn)) _planTools.push(tn);
        }
        const planActions = _planTools.map((tn, idx) => ({ idx, tool: tn, desc: confirmDesc(tn, {}, user.lang) }));
        addStep(runId, "plan", JSON.stringify(planObj));
        db.prepare("UPDATE chat_runs SET status='pending', pending_kind='plan', token=?, plan_json=?, actions_json=?, ask_text=?, pending_contents_json=?, updated_at=datetime('now') WHERE id=?")
          .run(token, JSON.stringify(planObj), JSON.stringify(planActions), ask, JSON.stringify(contents), runId);
...
export function resumePlanApprove(run, exam, user, approvals = null) {
  ...
  // 【D1】把主人在同一张表里的勾选落成本轮授权:勾了的执行时不再弹确认,没勾的直接拒绝、不执行。
  try {
    const acts = JSON.parse(run.actions_json || "[]");
    if (acts.length) {
      const approved = [], denied = [];
      for (const a of acts) { (approvals ? !!approvals[a.idx] : true) ? approved.push(a.tool) : denied.push(a.tool); }
      db.prepare("UPDATE chat_runs SET plan_auth_json=? WHERE id=?").run(JSON.stringify({ approved, denied }), run.id);
    }
  } catch {}
  // 原子认领:重复提交(双击/横幅与页面各发一个)只放行一个,避免并发跑出两个 runLoop
