const MAX_TOOL_ROUNDS = 24;   // 工具循环上限。Will:上限不许收紧,反而要放开——将来工具改成【模型自己语义搜索再调用】会多花几轮
...
import { retrieve, ragBlock, indexMaterial, materialParts, materialFilePart } from "@/lib/rag";
import { isHugePdf, readBigPdf } from "@/lib/pdfIndex";
...
import { FEATURE_ITEMS as _FEATS, getItem as _getItem } from "@/lib/uilab/items";
...
// 对外可用的"按用户语言取译文"(查字典,繁体走专用词典+简转繁兜底)。给别的模块拼用户可见文案用。
export function tUser(lang, k) { return _navName(lang || "zh", k); }
...
function navNameMap(lang) {
  ...
  const keys = ["追杀计划", "首页", "补充资料", "问问杀手", "我的", "学习", "本周计划表", "竞技场", "模拟考", "实践作业", "错题本", "设置"];
  // 【H7】以前这里写成 追杀计划=「Murder Plan」——用【中文书名号】括住英文名,杀手照抄进英文回复,
  // 就出现了「MAT235」「Murder Plan」这种中式标点包英文词(v13 复现)。这里不再给它任何中文书名号可抄。
  const pairs = keys.map((k) => `${k} = ${_navName(lang, k)}`).join("; ");
  return `\n【★这些栏目/页面在主人当前界面上的真实名字(跟主人说话时【只用等号右边这个名字】,别用左边的中文原名、也别中英并列):${pairs}。下面功能地图里写的是中文名,仅供你理解;对主人一律说右边的界面名。★另外:主人的界面【不是中文】时,【绝不要用「」《》这类中文标点】去括名字——用该语言自己的引号,或干脆不加引号。】\n`;
}

import { saveChatFile, readChatFile } from "@/lib/files";
import { classifyError } from "@/lib/errors";
import { ingestMaterialBuffer } from "@/lib/materialIngest";
import { pushUser, pushStatus } from "@/lib/notify";
import { checkInSyllabus, SCOPE_GUARD_ARG } from "@/lib/scopeGuard";
...
function assessBlock(exam) {
  try {
    // 注:主人在资料页自由文本框里写的说明【不在这里注入】——那会让它每一轮都进系统提示、白烧 token。
    // 改成【按需读】:list_materials 会把它一并列出(杀手要用时自己调),建/重建知识树时由 buildDossier 强制读。
...
export const WRITE_TOOLS = new Set(["delete_material", "delete_knowledge_point", "clear_questions", "build_knowledge_tree", "rollback", "redo", "clear_checkpoints", "ui_move_item", "ui_undo", "ui_create_feature", "ui_remove_feature", "ui_rename_feature", "ui_migrate_ui", "ui_set_nav_dock", "ui_home_layout_set", "ui_home_layout_off", "ui_set_killer_home", "save_learning_mode", "activate_learning_mode", "delete_learning_mode"]); // web_search_and_ingest 移出:直接执行、不弹允许
...
// 这些工具会【往某门考试里新建内容】→ 确认框要显式写明加到哪门,避免加错科目还没人发现
const CONTENT_CREATE_TOOLS = new Set(["assign_practical_task", "generate_question_set", "add_knowledge_point", "add_assignment", "create_custom_mode", "build_knowledge_tree", "exam_provision"]);
...
const PLAN_SAFE_WRITES = new Set(["exam_provision", "add_assignment", "update_assignment", "plan_from_syllabus", "add_plan_items", "plan_by_day", "set_task_due", "set_reminder", "set_auto_rule", "assign_practical_task"]);
// 无考试时也能用的、与具体考试无关的工具
...
export const functionDeclarations = [
  ...
  { name: "read_material", description: "【直接读某份资料的完整内容】给定资料 id(先用 list_materials 拿到 id),我会用多模态【原样打开这份文件】(PDF/图片/扫描件都行),把里面的文字/表格/图表内容读出来给你。★这是你【读资料的主力工具】:query_knowledge_base 只是【语义检索片段】,碰到纯扫描 PDF、或索引还没建好时会返回空——那【绝不代表文件读不了】,这时就用 read_material 按 id 把整份原件读出来。question 可选:只想找其中某部分(如「Quiz 和 Term Test 的日期与范围」)就填,系统会聚焦到它;留空=通读全文要点。【主人明确指了某份资料(如「用我的 syllabus」)时,就该用这个把它读出来,而不是去联网搜或糊弄】。", parameters: { type: "object", properties: { materialId: { type: "number" }, question: { type: "string", description: "想从这份资料里获取的具体内容(可选)" } }, required: ["materialId"] } },
  { name: "confirm_material_scope", description: "【确认某份资料确实属于本考试,清掉它的『疑似不符/不确定/超范围』标记】。当资料列表里某份被标了 ❓不确定 / ⚠️疑似不符 / 📚超范围,而主人【明确告诉你这份就是本考试的资料】(比如「这是我这门课的 syllabus」「这份就是本考试的」)时,用它把标记清掉——因为系统的自动判定是按【抽出的文件内容】判的,扫描件抽不出字就会误标不确定,和主人怎么说无关,所以要靠这个按主人的话来纠正。materialId=资料 id(先用 list_materials 拿到)。★只在主人【明确认领】后用,别自作主张清。", parameters: { type: "object", properties: { materialId: { type: "number" } }, required: ["materialId"] } },
  ...
  { name: "ask_user_form", description: "【在对话里弹出一个参数表单让主人填】——需要主人提供【几项具体参数】才能继续做某件事时用它(任何场景通用:排计划、建考试的偏好、设置某个规则的阈值、收集背景信息等)。它会像确认框一样【内嵌在聊天里】显示一个表单,主人填完提交,你就在【下一步】拿到这些值继续。★计划/参数类信息【优先用它一次问全,别在对话里一条条来回问】。只填【真正需要、且你还不知道】的字段,别把已知的也塞进来。title=表单标题(一句话说要收集什么);fields=字段数组,每个字段:key(英文标识)、label(给主人看的问题)、type(text文本/number数字/date日期/select下拉/radio单选/checkbox多选)、options(select/radio/checkbox 的选项数组,每项 {value,label})、allowOther(仅 radio:true 时在选项末尾自动加一个「其他(自己填)」,主人选它就能自由输入——用于「给几个选项、但也允许主人给出你没列到的答案」的场景)、default(默认值)、placeholder(提示)、required(是否必填)。\n★【什么时候用这个弹窗·重要边界】只在【你正在执行主人交办的某个行动、执行途中遇到一个必须主人拍板才能继续的岔路】时用——比如「这两门考试是合并成一门还是做成父子结构」「删除前保留哪一份」「按哪个日期」。这类问题【和主人刚要求的那个行动直接相关、且不问就没法往下做】。★这类抉择若是【几个明确选项里挑一个】(合并还是父子、删哪份、按哪个日期、重建时保留/浓缩/重来 等),就用 radio 单选弹给他【别用纯文字把选项一二三列出来让他打字回复】(需要选项外自填就开 allowOther)。【不要】用它来:①问学习/教学类问题(苏格拉底追问、「你觉得这题选哪个」、讲解中的互动——这些正常聊天里问);②征求「要不要我帮你做某件事」(那是行动【开始前】的意愿确认,直接在对话里问一句即可,别动不动弹表单);③把本可以一句话问清的小事做成表单。一句话:【行动中、为推进这个行动、非问不可】才弹;学习类和「要不要做」一律不弹。", parameters: { type: "object", properties: { title: { type: "string" }, fields: { type: "array", items: { type: "object", properties: { key: { type: "string" }, label: { type: "string" }, type: { type: "string", enum: ["text", "number", "date", "select", "radio", "checkbox"] }, options: { type: "array", items: { type: "object", properties: { value: { type: "string" }, label: { type: "string" } } } }, allowOther: { type: "boolean" }, default: { type: "string" }, placeholder: { type: "string" }, required: { type: "boolean" } }, required: ["key", "label", "type"] } } }, required: ["title", "fields"] } },
  ...
  { name: "changes_this_turn", description: "取本轮到目前为止的【真实改动记录】(和系统指令末尾自动附上的那份是同一份,随时可以再查一次)。返回按发生顺序列出你这一轮实际执行过的每个动作及其真实结果:做了什么、成功还是没成功、说明或失败原因。用途:跟主人汇报改动前核对事实,避免凭记忆说错。返回为空=这轮还没有任何改动。", parameters: { type: "object", properties: {} } },
  ...
  { name: "set_exam_info", description: "修改当前考试的信息。★durationMin=【这门考试真实的考试时长(分钟)】——主人说了(或大纲/官方说明里写了)就记下来,模拟考会按它计时;【不知道就别填、更别自己估一个】。", parameters: { type: "object", properties: { durationMin: { type: "integer" }, name: { type: "string" }, examDate: { type: "string" }, notes: { type: "string" } } } },
...
export function describe(name, args) {
  switch (name) {
    ...
    case "set_exam_info": return `修改考试信息:${[args.name && "名称→" + args.name, args.examDate && "日期→" + args.examDate, args.notes && "说明已更新", args.durationMin && "考试时长→" + args.durationMin + " 分钟"].filter(Boolean).join(",")}`;
...
export async function execTool(name, args, exam, user, runId = null) {
  ...
  // 【内容归属守门】往当前考试里新建内容前,拿这门课的课纲当尺子判一下在不在范围内(判据是课纲,不是学科)
  if (exam && SCOPE_GUARD_ARG[name]) {
    try {
      const _txt = SCOPE_GUARD_ARG[name](args || {});
      if (_txt) {
        const _chk = await checkInSyllabus(exam, _txt, user && user.lang);
        if (_chk && _chk.verdict === "out") {
          return { error: `【不在本门课的范围内·已拦下】「${_txt}」不在《${exam.name}》的课纲里${_chk.reason ? "(" + _chk.reason + ")" : ""}。别把它硬加进这门考试。请先告诉主人:这看起来不是《${exam.name}》的内容,然后问他——是要加到【别的哪一门考试】?还是要为它【新建一门考试】?还是他确认就是要放在《${exam.name}》里(他明确坚持你再加)。` };
        }
        // 【拿不准就别偷偷放行】以前 unsure(模棱两可)和判定出错都直接放过去,于是"用 If-Else 做数学决策"这类
        // 越界内容照样被塞进纯数学考试。现在改成:判不准也拦下来、让主人拍板,而不是替他默认"应该没事"。
        if (_chk && (_chk.verdict === "unsure" || _chk.verdict === "error")) {
          return { error: `【拿不准是不是本门课的内容·先别建】「${_txt}」我没法确定它在不在《${exam.name}》的课纲范围内${_chk.reason ? "(" + _chk.reason + ")" : ""}。【不要】自作主张建下去,先问主人一句:这是要放进《${exam.name}》,还是属于别的考试/要单开一门?他确认了你再建。` };
        }
      }
    } catch {}
  }
  switch (name) {
    ...
    case "read_material": {
      // 【按 id 直接多模态读原件】填补"有文件却读不出→编借口/私自联网"的缺口(Will 2026-07)。
      const mid = Number(args.materialId);
      const fam = familyScope(exam.id);
      const m = db.prepare(`SELECT id, filename, kind, status, exam_id FROM materials WHERE id=? AND exam_id IN ${scopeSql(fam)}`).get(mid);
      if (!m) return { error: `没找到 id=${args.materialId} 的资料(它不在当前考试家族里,或已删除)。先用 list_materials 核对 id。` };
      if (m.status !== "ready") return { error: `资料「${m.filename}」还没就绪(状态:${m.status}),稍后再读。` };
      // 【超大 PDF 走拆页读】整份读不了(超 Gemini ~50MB/1000页 上限),就按问题只读相关页(入库时已建了页码索引)。
      if (isHugePdf(m)) {
        const r = await readBigPdf(m, exam.id, args.question ? String(args.question).slice(0, 300) : "", user && user.lang);
        if (r.ok) return { ok: true, filename: m.filename, pages: r.pages, content: r.content, note: `《${m.filename}》太大、整份读不了,已【只读与问题最相关的第 ${(r.pages||[]).join("、")} 页】。若还要别的部分,把要找的东西说得更具体些我再定位。汇报以这些页的真实内容为准。` };
        return { error: `《${m.filename}》是超大 PDF,这次没定位到相关页(${r.error || ""})。可以把要找的东西说得更具体(如"Quiz 的日期在评分那一节"),我再按页定位;或让主人把关键几页单独截出来传。别编内容。` };
      }
      let part; try { part = await materialFilePart(m); } catch { part = null; }
      if (!part) return { error: `资料「${m.filename}」这次打不开(可能上传/转码出了问题)。如实告诉主人这份读不了、请他重传一次,别编造内容、也别拿联网搜的东西冒充。` };
      const q = args.question ? String(args.question).slice(0, 300) : "";
      const instr = (q
        ? `这是主人资料库里的文件「${m.filename}」。请【只依据这份文件本身】,找出并原样列清其中关于【${q}】的内容(有表格就把表格里的每一行/每个日期都抄出来,别概括、别遗漏、别脑补文件里没有的东西)。`
        : `这是主人资料库里的文件「${m.filename}」。请【只依据这份文件本身】通读并把关键内容原样提取出来(重要的日期、表格、要求逐条列清,别概括成一句、别脑补文件里没有的东西)。`)
        + `\n若文件是空白/无法辨认,就【如实说读不出来】,不要编造。`;
      try {
        const res = await generate(null, { contents: [{ role: "user", parts: [part, { text: instr }] }] });
        const txt = (res && res.text || "").trim();
        if (!txt) return { error: `文件「${m.filename}」这次没读出内容(可能是空白页或扫描质量差)。如实告诉主人读不出、请他把关键部分贴进聊天或重传,别编。` };
        return { ok: true, filename: m.filename, content: txt.slice(0, 8000), note: `已直接读取资料「${m.filename}」的内容。汇报/排计划一律以这份真实内容为准。` };
      } catch (e) {
        return { error: `读取「${m.filename}」时出错(${String(e && e.message || e).slice(0,80)})。如实告诉主人这份这次读不了,别用联网搜的内容顶替。` };
      }
    }
    case "confirm_material_scope": {
      const mid = Number(args.materialId);
      const m = db.prepare(`SELECT id, filename, offtopic FROM materials WHERE id=? AND exam_id IN ${scopeSql(familyScope(exam.id))}`).get(mid);
      if (!m) return { error: `没找到 id=${args.materialId} 的资料。先用 list_materials 核对 id。` };
      db.prepare("UPDATE materials SET offtopic=0, offtopic_reason=NULL WHERE id=?").run(mid);
      return { ok: true, note: `已把资料「${m.filename}」确认为本考试的资料,清掉了之前的疑似不符/不确定标记。` };
    }
    case "list_materials": { const _notes = (() => { try { const cl = JSON.parse(exam.checklist || "[]"); const f = cl.filter((c) => c && c.answer && String(c.answer).trim()).map((c) => `${c.item}:${String(c.answer).trim().slice(0, 800)}`); return f.length ? "\n\n【主人在资料页【手写填的说明/笔记】(不是上传的文件,但同样是他给你的资料,排计划/出题/建树都要真的用上)】\n" + f.join("\n") : ""; } catch { return ""; } })(); // 【资料按家族共享】db.js 里写明的设计原则:材料/RAG 一律用 familyScope,激活哪门考试都不该影响视图。
      // 资料页(/api/materials)、RAG 检索、喂给模型的附件(materialParts)本来都是家族范围,
      // 唯独这里一直是 exam_id=?(只看当前这一门)→ 页面上明明共享着,杀手却说"资料库为空",看着就像资料不共享了。
      // 现在改成家族范围,并像资料页那样标出【是从哪门考试共享过来的】,免得杀手把别门的资料当成本门的。
      const _fam = familyScope(exam.id);
      const m = db.prepare(`SELECT id, filename, kind, status, auto, offtopic, offtopic_reason, exam_id FROM materials WHERE exam_id IN ${scopeSql(_fam)} ORDER BY (exam_id=${Number(exam.id)}) DESC, id DESC`).all();
      const _nm = {}; for (const _id of _fam) { try { _nm[_id] = db.prepare("SELECT name FROM exams WHERE id=?").get(_id)?.name || ""; } catch {} }
      return { materials: m.map((x) => `[${x.id}] ${x.filename} (${x.kind},${x.status})${x.exam_id !== exam.id ? ` 〔共享自《${_nm[x.exam_id] || "同家族考试"}》〕` : ""}${x.auto ? " [系统自动配乐,非用户上传]" : ""}${x.offtopic === 1 ? " ⚠️[疑似与本考试主题不符:" + (x.offtopic_reason || "") + "]" : x.offtopic === 2 ? " ❓[不确定是否属于本考试,需向主人确认:" + (x.offtopic_reason || "") + "]" : x.offtopic === 3 ? " 📚[范围超出本考试,需向主人确认哪部分相关:" + (x.offtopic_reason || "") + "]" : ""}`).join("\n") + _notes || ("(资料库为空)" + _notes) }; }
    ...
    case "plan_overview": {
      const p = crossExamPlan(user.id, { totalMinutes: args.minutes, mode: args.mode });
      const curId = exam ? Number(exam.id) : null;
      const line = (e) => `${e.name}${Number(e.id) === curId ? "(主人现在正开着的这门)" : ""}:${e.daysLeft == null ? "考期未定" : e.daysLeft + "天后考"}、掌握${e.kpTotal ? Math.round((e.mastered / e.kpTotal) * 100) : 0}%、薄弱${e.weak}个、待复习${e.due}题 → 建议今天${e.allocMinutes}分钟`;
      // P4-8:规划器本来就给【每一门】都分了时间,是叙述时容易漏。把"必须全覆盖"的清单显式回给杀手,漏掉哪门一眼可见。
      const mustCover = p.exams.filter((e) => (e.allocMinutes || 0) > 0).map((e) => e.name);
      return {
        plan: p.exams.map(line).join("\n") || "(还没有考试)",
        topTask: p.topTask ? p.topTask.text : null, totalMinutes: p.totalMinutes, examCount: p.examCount,
        mustCover,
        note: mustCover.length ? `【必须全覆盖】上面分到了时间的这 ${mustCover.length} 门——${mustCover.join("、")}——【每一门都要出现在你给主人的安排里】,一门都不许漏(尤其别漏掉主人现在正开着的那门、和考期近的那门)。要是你判断某门今天确实不该占时间,也要【明说为什么先不排它】,而不是默不作声地跳过。` : "",
      };
    }
    ...
    case "ui_remove_feature": {
      if (["home", "mine"].includes(String(args.featureId))) return { ok: false, note: "这是固定栏目(首页 / 我的),不能移除或隐藏——它只能在导航栏里换个位置。" };
    ...
    case "changes_this_turn": {
      const acts = readActs(runId);
      if (!acts.length) return { changes: [], note: "本轮没有做出任何改动——回复里不要提任何改动。" };
      return {
        changes: acts.map((a) => ({ 做的事: a.t, 是否改动了数据: a.w ? "是" : "否", 结果: a.ok ? "成功" : "失败", 说明: a.note || a.err || "" })),
        note: "以上是本轮的真实记录。只照实汇报:成功的才说做了,失败的如实说没成功,这里没有的一律别说。用大白话,别出现工具名/ID。",
      };
    }
    ...
    case "set_exam_info": { const f = []; const v = []; if (args.name) { f.push("name=?"); v.push(args.name); } if (args.examDate) { f.push("exam_date=?"); v.push(args.examDate); } if (args.notes != null) { f.push("notes=?"); v.push(args.notes); } if (args.durationMin != null && Number(args.durationMin) > 0) { f.push("duration_min=?"); v.push(Math.round(Number(args.durationMin))); } if (f.length) { db.prepare(`UPDATE exams SET ${f.join(",")} WHERE id=?`).run(...v, exam.id); } return { ok: true, note: "已更新考试信息" }; }
...
职责:回答备考问题、按主人想法调整策略/档案、解读练习数据、帮找资料;主人给大目标时自己拆成多步依次调用工具。主人要你去某个【需要登录】的学习网站抓内容时,你【进不了需要登录的外部网站】,就如实说清楚——请主人自己把内容导出/截图/复制、或直接传进「补充资料」,你再据此帮他,别假装能去采集。
【诚实铁律 · 最高优先级】
...
${featureCatalog()}${navNameMap(user.lang)}${(() => {
  try {
    const ps = pushStatus(user.id);
    return ps.enabled
      ? "\n【主人的应用外推送:已开启(有 " + (ps.devices || 1) + " 台设备)。到点提醒能真正弹到系统通知里。】\n"
      : "\n【★主人的应用外推送:【没开】(" + (ps.reason === "pref_off" ? "他把推送开关关了" : "还没有任何设备注册过推送") + ")。所以【到点提醒现在发不出去】——你要给他设提醒【之前】就要如实说清楚:提醒是靠系统通知弹出来的,他得先在设置里打开通知/允许浏览器推送,否则到点了不会有任何动静(提醒【不会】留在收件箱里)。别在他没开的情况下拍胸脯说「到时候会提醒你」。】\n";
  } catch { return ""; }
})()}${APP_GUIDE}\n\n---\n【以下是本轮实时上下文(每轮会变,故放最后——让上面固定的规则/功能地图/工具描述命中隐式缓存、省 token)】${exam ? `\n【知识状态 · 记忆曲线(实时,以做题数据为准;与主人自述冲突时如实指出)】\n${knowledgeStateDigest(exam.id)}\n` : ""}${exam ? assessBlock(exam) : ""}`;
}
...
// P3-1 兜底:万一模型把内部推理/系统提示词片段/逐工具日志/元评论混进了最终回复,尽量清掉,只留给主人的话。
// 只在检测到【强泄露信号】时才动手,否则原样返回,避免误伤正常内容。
function cleanReply(t) {
  if (!t) return t;
  let s = String(t);
  const leak = /Keep the tone|不要用技术术语|Let's write the response|Let me write the (?:final )?response|现在(?:来)?写(?:给主人的)?回复|Everything is perfect|->\s*Succeeded|\bok:\s*(?:true|false)\b/i;
  if (!leak.test(s)) return s.trim();
  // 在"过渡语"处把前面的草稿切掉,保留真正的回复
  const transitions = [/Let's write the response[^\n]*/gi, /Let me write the (?:final )?response[^\n]*/gi, /现在(?:来)?写(?:给主人的)?回复[^\n]*/gi, /Everything is perfect[^\n]*/gi];
  for (const re of transitions) {
    const ms = [...s.matchAll(re)]; const m = ms.length ? ms[ms.length - 1] : null;
    if (m) { const after = s.slice(m.index + m[0].length).trim(); if (after.length > 30) { s = after; break; } }
  }
  // 逐行删掉仍带强泄露信号的行(工具日志 / ok:xx / 提示词片段 / 元评论)
  s = s.split("\n").filter((l) => !/->\s*Succeeded|\bok:\s*(?:true|false)\b|Keep the tone|不要用技术术语|Wait! This is|Let's write the response|Let me write the (?:final )?response/i.test(l)).join("\n");
  return s.trim();
}

// P3-1 根治:只从【非思考(thought)】的片段取回复文本。模型开思考时,思考片段(会复述系统提示词/工具日志/自言自语)绝不能进正文——
// SDK 的 res.text 会把所有片段拼一起,所以这里手动只挑 part.thought 不为真的 text 片段。
function replyText(res) {
  try {
    ...
    if (Array.isArray(parts)) {
      const t = parts.filter((p) => p && typeof p.text === "string" && !p.thought).map((p) => p.text).join("");
      if (meaningfulText(t)) return t;
    }
  } catch {}
  return (res && res.text) || "";
}
...
// 【P5-2·错误层文案也要跟用户语言走】这些串会【直接变成聊天消息落库】,发出去那一刻就定死了,
// 前端再怎么 i18n 也救不回来。v12 那次扫描只覆盖了砖头 title 和确认框模板,漏掉了整个错误处理层
// (v13 实测:英文账号连续 4 次收到整条中文的"AI 这次没接上…")。_navName 就是现成的"查字典+繁体兜底"实现,直接复用。
function emptyReplyMsg(res, lang = "zh") {
  const fr = res?.candidates?.[0]?.finishReason;
  if (fr === "MAX_TOKENS") return _navName(lang, "(这次要说的太长、被截断了 —— 你说一声「继续」我接着说,或把问题拆小一点)");
  if (fr === "SAFETY" || fr === "PROHIBITED_CONTENT" || fr === "RECITATION") return _navName(lang, "(这次回复被内容策略拦下了,换个说法我再试)");
  return _navName(lang, "(没有生成回复,请重试)");
}
export async function runAgent(contents, exam, user, toolNotes) {
  ...
  try {
    // 【不再每轮硬塞原件】杀手手上有 read_material(按 id 读全文)和 query_knowledge_base(检索)两个工具,
    // 需要看资料时它会自己去取 —— 这正是"模型主动看"成立的地方。以前每轮无条件附 3 份整份原件
    // (PDF 按页计费),多轮工具循环下来同一本书要被计费很多遍。
    const mp = [];
    if (mp.length) {
  ...
  for (let i = 0; i < MAX_TOOL_ROUNDS; i++) {
    ...
    if (!calls || !calls.length) {
      const rt = replyText(response); const reply = meaningfulText(rt) ? cleanReply(rt.trim()) : emptyReplyMsg(response, user && user.lang);
    ...
    if (calls.some((c) => isWrite(c.name))) {
      ...
      const actions = calls.map((c, idx) => isWrite(c.name) ? { idx, tool: c.name, desc: confirmDesc(c.name, c.args || {}, user.lang), intoExam: (CONTENT_CREATE_TOOLS.has(c.name) && exam) ? exam.name : null } : null).filter(Boolean);
...
async function makePlan(ask, user, extra = {}) {
  ...
  const schema = { type: "object", properties: {
    ...
    steps: { type: "array", items: { type: "object", properties: { title: { type: "string" }, detail: { type: "string" }, tools: { type: "array", items: { type: "string" } } }, required: ["title"] } }
  }, required: ["complex"] };
...
- ★title/detail 写给主人看,一律大白话,【不许出现工具名/英文函数名/内部 ID】。(appGuide 的 P3-5/P3-6 红线对杀手本人已有同样要求,但【规划模块用的是这份独立提示词、读不到 appGuide】,所以必须在这里再声明一次,否则那条红线到不了真正写计划的模型。)
- ★tools 字段填这一步会用到的工具真名(必须是下面清单里的,别编)。【主人看不到这个字段】,它只用来在主人同意计划后生成一张"这次会做哪些改动"的授权表,让他一次勾完、免得执行到一半反复被打扰。凡是会动数据的步骤务必填全。
- ★【只规划主人这次【明确要做】的事·绝不自作主张加没要求的操作】计划里的每一步都必须【直接服务于主人这次的请求】。【严禁】because你在工具清单里看到某个能力、或注意到某种"看起来可以顺手做"的关联,就把主人【没要求】的操作塞进计划——尤其是【合并考试(exam_merge)、改父子/挂靠结构、删除、清空、重建】这类结构性/破坏性动作:主人【提到了、涉及、或同时有多门考试】,【绝不等于】他要把它们合并或改动彼此关系。除非主人这次【话里明确要求】了(如"把这两门合并""把X挂到Y下面"),否则计划里【不许出现】这类步骤。拿不准某步是不是主人要的,就【别加】——宁可计划短一点、只做他明确要的,也不要多做他没让做的。
只能用下列真实能力来规划,别编不存在的:
${toolList}
...
// 把功能 id 换成【主人界面语言】下的栏目名(确认框里显示可读名字,而不是内部 id)。
function _featLabel(featureId, lang) {
  try { const it = _getItem(featureId); const lbl = (it && it.label) || featureId; return _navName(lang || "zh", lbl); } catch { return featureId; }
}
function confirmDesc(name, a = {}, lang = "zh") {
  switch (name) {
    ...
    case "ui_move_item": { const nm = _featLabel(a.featureId, lang); const tpl = a.where === "hidden" ? "隐藏〈{name}〉" : a.where === "nav" ? "把〈{name}〉移到导航栏" : a.where === "more" ? "把〈{name}〉移到更多菜单" : a.where === "morefeatures" ? "把〈{name}〉移到更多功能" : "把〈{name}〉放到首页"; return { t: tpl, p: { name: nm } }; }
    case "ui_undo": return { t: "撤销最近一次界面改动" };
    case "ui_create_feature": return { t: "新建功能「{name}」", p: { name: `${a.icon || ""} ${a.name || a.id || ""}`.trim() } };
    case "ui_remove_feature": return { t: "移除功能〈{name}〉", p: { name: _featLabel(a.featureId, lang) } };
    case "ui_rename_feature": return { t: "给功能〈{name}〉改名或换图标", p: { name: _featLabel(a.featureId, lang) } };
    case "ui_migrate_ui": return { t: "把另一门考试的界面搬到本考试" };
    case "ui_set_nav_dock": return { t: "调整导航栏停靠位置" };
    case "ui_home_layout_set": return { t: "调整首页布局" };
    case "ui_home_layout_off": return { t: "取消自定义首页布局" };
    case "ui_set_killer_home": return { t: "调整杀手在界面上的位置" };
    default: { const d = describe(name, a); const b = getBrick(name); if (b && b.title) return { t: b.title }; return d ? d : name; }
  }
}

export function toolLabelPublic(name, args) { return toolLabel(name, args); }
function toolLabel(name, args) {
  const a = args || {};
  switch (name) {
    case "ask_user_form": return "问你几个问题";
    ...
    case "confirm_material_scope": return `确认资料${args.materialId ? `[${args.materialId}]` : ""}属于本考试`;
    case "list_materials": return "查看资料库";
    case "locate_material": return `在资料里定位「${args.reference || ""}」`;
    case "read_material": return `读取资料${args.materialId ? `[${args.materialId}]` : ""}${args.question ? `找「${args.question}」` : "的内容"}`;
    ...
    default: { const dd = describe(name, a); return (dd && dd !== name) ? dd : name; } // 先用 describe(砖头标题/已描述的原生工具),都没有才退回原名(理论上不该发生)
  }
}
// 【本轮真实改动日志】每次工具执行后记一条【工具的真实返回】(成功/失败/说明)。
// 存在 chat_runs.act_log_json;确认恢复(resume)会清 steps_json 但【不清】它,所以整轮(含超时、多次确认)都在累积,
// 直到最终回复发出。只存【可读标签】不存内部工具名,杀手照抄也不会泄露函数名。
export function recordAct(runId, { label, write, ok, note, error }) {
  if (!runId) return;
  try {
    const row = db.prepare("SELECT act_log_json FROM chat_runs WHERE id=?").get(runId);
    const list = row?.act_log_json ? JSON.parse(row.act_log_json) : [];
    list.push({ t: String(label || "").slice(0, 160), w: !!write, ok: !!ok, note: String(note || "").slice(0, 300), err: String(error || "").slice(0, 200), ts: Date.now() });
    db.prepare("UPDATE chat_runs SET act_log_json=? WHERE id=?").run(JSON.stringify(list), runId); // 【不截断】这份日志是按【这一轮(run)】存的,不是全局;一轮里做了多少件就留多少件,一条都不能丢
  } catch {}
}
// 【程序写死·不靠提示词】每次调模型前把这段拼进系统指令:本轮到此为止的真实改动记录。
// 空也要给——明确告诉它"这轮什么都没改",它才没法凭空编出"已切换/已挪动/已提醒"。
export function actsBlock(runId) {
  const acts = readActs(runId);
  if (!acts.length) {
    return "\n\n【本轮真实改动记录·系统生成·不可更改】到目前为止你这一轮【没有做出任何改动】。所以最终回复里【完全不要提任何改动】(不许说已切换/已建立/已挪动/已提醒/已生成之类的话),也不要暗示做过什么。";
  }
  const lines = acts.map((a, i) => `${i + 1}. ${a.ok ? "✅ 成功" : "❌ 没成功"}:${a.t}${a.note ? " —— " + a.note : ""}${a.err ? " —— 失败原因:" + a.err : ""}`).join("\n");
  return "\n\n【本轮真实改动记录·系统生成·不可更改】以下是你这一轮实际做成/没做成的每一件事,【编号就是真实的执行先后顺序】:\n" + lines +
    "\n汇报时【只能以这份记录为准】,并且【必须按这里的编号顺序讲】(先做的先说,别打乱、别挑着说、别合并成一句含糊带过):标成功的才可以说做了;标没成功的必须如实说没做成、并说清卡在哪一步。这份记录里【没有】的事情,一个字都不许说。用大白话说,别出现工具名/内部 ID。";
}

export function readActs(runId) {
  try {
    const row = db.prepare("SELECT act_log_json FROM chat_runs WHERE id=?").get(runId);
    return row?.act_log_json ? JSON.parse(row.act_log_json) : [];
  } catch { return []; }
}
...
export async function runLoop(runId, contents, exam, user, toolNotes = [], planText = "") {
  ...
  try {
    // 【不再每轮盲喂资料】以前这里把"最新 3 份"pdf/图/音频塞进上下文(materialParts max:3):
    //   ①白烧 token;②只喂最新 3 份 → 早传的(如 syllabus id 小)永远进不来,杀手看不到就编借口/私自联网(Will 实测)。
    // 现在改成【按需读】:杀手要看哪份,自己用 read_material 按 id 读。听力题的音频走的是【出题路径】(generators.js),
    // 那边有独立的 materialParts,不受这里影响。
    for (let i = 0; i < MAX_TOOL_ROUNDS; i++) {
      addStep(runId, "think", "");
      const response = await agentTurn({ contents, system: system + actsBlock(runId), tools: [{ functionDeclarations: declsFor(user) }] });
      const calls = response.functionCalls;
      if (!calls || !calls.length) {
        const rt = replyText(response); const reply = meaningfulText(rt) ? cleanReply(rt.trim()) : emptyReplyMsg(response, user && user.lang);
        db.prepare("INSERT INTO chat_messages(exam_id,role,content) VALUES(?,?,?)").run(chatKey(exam, user), "model", reply);
      ...
      // 【D1】读本轮在计划确认表里拿到的授权:approved 的不再弹确认,denied 的直接拒绝、根本不执行。
      let _auth = { approved: [], denied: [] };
      try { const _row = db.prepare("SELECT plan_auth_json FROM chat_runs WHERE id=?").get(runId); if (_row && _row.plan_auth_json) _auth = JSON.parse(_row.plan_auth_json); } catch {}
      const _denied = calls.filter((c) => (_auth.denied || []).includes(c.name));
      if (_denied.length) {
        const parts0 = calls.map((fc) => ({ functionResponse: { name: fc.name, response: { result: (_auth.denied || []).includes(fc.name)
          ? { declined: true, note: `主人在计划确认表里【没有勾选】这一项(${toolLabel(fc.name, fc.args || {})}),所以没有执行。别再重复尝试;如实告诉他这项按他的意思跳过了,若他改主意可以再说一次。` }
          : { skipped: true, note: "同一批里有未获授权的操作,这一批整体跳过" } } } }));
        contents.push({ role: "user", parts: parts0 });
        for (const fc of _denied) { try { recordAct(runId, { label: toolLabel(fc.name, fc.args || {}), write: true, ok: false, note: "主人未勾选,已跳过" }); } catch {} }
        continue;
      }
      const _needGate = calls.some((c) => isWrite(c.name) && !(planText && PLAN_SAFE_WRITES.has(c.name)) && !(_auth.approved || []).includes(c.name));
      if (_needGate) {
        const token = crypto.randomBytes(16).toString("hex");
        const actions = calls.map((c, idx) => isWrite(c.name) ? { idx, tool: c.name, desc: confirmDesc(c.name, c.args || {}, user.lang), intoExam: (CONTENT_CREATE_TOOLS.has(c.name) && exam) ? exam.name : null } : null).filter(Boolean);
      ...
      for (const fc of calls) {
        ...
        const result = await execTool(fc.name, fc.args || {}, exam, user, runId);
        try { recordAct(runId, { label: toolLabel(fc.name, fc.args || {}), write: isWrite(fc.name), ok: !(result && (result.error || result.ok === false)), note: (result && result.note) || "", error: (result && result.error) || "" }); } catch {}
    ...
    const reply = "我这次连着做了好几步,先在这儿停一下喘口气。你刚让我做的这件事可能还没做完——跟我说一声「继续」,我就接着往下做。";
    db.prepare("INSERT INTO chat_messages(exam_id,role,content) VALUES(?,?,?)").run(chatKey(exam, user), "model", reply);
    db.prepare("UPDATE chat_runs SET status='done', reply=?, updated_at=datetime('now') WHERE id=?").run(reply, runId);
    addStep(runId, "done", "");
  } catch (e) {
    ...
    // 【按错误【种类】给不同建议,别一律说"再发一次"】项目里本来就有分类器(lib/errors.js:classifyError),
    // 但这里以前完全没用它:额度用完/触发限流也被说成"连接不太稳,直接再发一次" → 主人照做,于是一次次重发、
    // 一次次失败(v13 实测同一句话连续失败 4 次,收到的是一模一样的"连接不稳"提示)。这属于把【不可重试】
    // 的错误当成【可重试】来指导用户,比文案本身更误导。
    const _lang = (user && user.lang) || "zh";
    const _kind = (() => { try { return classifyError(e).type; } catch { return "unknown"; } })();
    let msg;
    if (_kind === "quota") msg = _navName(_lang, "(AI 的用量额度已经用完了,不是你的问题,再重发也还是发不出去——过一段时间再来,或让管理员看看额度。你的进度都在。)");
    else if (_kind === "rate_limit") msg = _navName(_lang, "(AI 这会儿被请求得太频繁,先歇一小会儿再发——马上重发大概率还是不行。你的进度都在。)");
    else if (_kind === "bad_key" || _kind === "no_key") msg = _navName(_lang, "(AI 服务的密钥没配好或已失效,这个得管理员处理,重发没用。你的进度都在。)");
    else if (_kind === "server" || _kind === "network") msg = _navName(_lang, "(AI 这次没接上/连接不太稳,你的进度没丢——直接再发一次刚才的话、或说「继续」就行)");
    else msg = _navName(_lang, "(出错了,请重试)");
    // 【存进度供重发接着用】只有本轮真的跑过工具(contents 里有 functionCall/Response)才值得存——
    // 否则没什么可续的。把累积的 contents 快照进 resume_contents_json,近期重发时 /api/chat 会接着它跑。
    let _hadTools = false;
    try { _hadTools = contents.some((c) => Array.isArray(c.parts) && c.parts.some((p) => p && (p.functionCall || p.functionResponse))); } catch {}
    try {
      if (_hadTools) db.prepare("UPDATE chat_runs SET status='error', reply=?, resume_contents_json=?, updated_at=datetime('now') WHERE id=?").run(msg, JSON.stringify(contents).slice(0, 2000000), runId);
      else db.prepare("UPDATE chat_runs SET status='error', reply=?, updated_at=datetime('now') WHERE id=?").run(msg, runId);
    } catch { try { db.prepare("UPDATE chat_runs SET status='error', reply=?, updated_at=datetime('now') WHERE id=?").run(msg, runId); } catch {} }
  ...
  try {
    ...
    if (maybeComplex(ask)) {
      ...
      if (plan && plan.complex && Array.isArray(plan.steps) && plan.steps.length) {
        ...
        const token = crypto.randomBytes(16).toString("hex");
        // 计划门【只谈计划】:同意/改计划。授权勾选放到同意之后的第二张表——
        // 否则主人点"按意见改计划"时,他刚勾的那些会随计划一起失效,逻辑上说不通。
...
// 主人同意计划 -> 按计划开工
export function resumePlanApprove(run, exam, user, approvals = null) {
  const contents = JSON.parse(run.pending_contents_json || "[]");
  let planText = "";
  try { const p = JSON.parse(run.plan_json || "{}"); planText = (p.steps || []).map((s, i) => `${i + 1}. ${s.title}${s.detail ? " — " + s.detail : ""}`).join("\n"); } catch {}
  // 【D1·第二张表】同意计划后,若计划里有会动数据的步骤,先弹一张【一次性授权表】:
  // 每项用计划步骤自己的【大白话标题】(不出现任何工具名,也不用空参数去套确认模板——那会露出 {id}/{name} 占位符),
  // 主人一次勾选提交,执行时就不再一层层弹确认了。
  try {
    const p = JSON.parse(run.plan_json || "{}");
    const items = [];
    for (const st of (p.steps || [])) {
      const ws = (st.tools || []).filter((tn) => isWrite(tn));
      if (ws.length) items.push({ idx: items.length, label: st.title || "", tools: ws });
    }
    if (items.length) {
      const token2 = crypto.randomBytes(16).toString("hex");
      // 保留 plan_json:授权表提交后再从它重建 planText 交给 runLoop(不额外加字段)
      db.prepare("UPDATE chat_runs SET status='pending', pending_kind='auth', token=?, actions_json=?, steps_json='[]', updated_at=datetime('now') WHERE id=? AND status='pending'")
        .run(token2, JSON.stringify(items.map((it) => ({ idx: it.idx, desc: { t: it.label }, tools: it.tools }))), run.id);
      addStep(run.id, "pending", "");
      return;   // 等主人提交授权表,再由 resumePlanAuth 开工
    }
  } catch {}
  // 计划里没有会动数据的步骤 → 不用再问,直接开工
  const _claim = db.prepare("UPDATE chat_runs SET status='running', token=NULL, plan_json=NULL, actions_json=NULL, ask_text=NULL, pending_kind=NULL, pending_contents_json=NULL, steps_json='[]', updated_at=datetime('now') WHERE id=? AND status='pending'").run(run.id);
  if (!_claim.changes) return;
...
// 【D1·第二张表】主人提交了「这次会做的改动」授权表 -> 落成本轮授权,然后按计划开工。
export function resumePlanAuth(run, exam, user, approvals = null) {
  const contents = JSON.parse(run.pending_contents_json || "[]");
  let planText = "";
  try { const p = JSON.parse(run.plan_json || "{}"); planText = (p.steps || []).map((s, i) => `${i + 1}. ${s.title}${s.detail ? " — " + s.detail : ""}`).join("\n"); } catch {}
  try {
    const items = JSON.parse(run.actions_json || "[]");
    const approved = [], denied = [];
    for (const it of items) {
      const ok = approvals ? !!approvals[it.idx] : true;
      for (const tn of (it.tools || [])) (ok ? approved : denied).push(tn);
    }
    // 同一个工具若既被勾又被取消(出现在多步里),以【勾选】为准,避免误伤
    const finalDenied = denied.filter((t) => !approved.includes(t));
    db.prepare("UPDATE chat_runs SET plan_auth_json=? WHERE id=?").run(JSON.stringify({ approved, denied: finalDenied }), run.id);
  } catch {}
  const claim = db.prepare("UPDATE chat_runs SET status='running', token=NULL, plan_json=NULL, actions_json=NULL, pending_kind=NULL, pending_contents_json=NULL, steps_json='[]', updated_at=datetime('now') WHERE id=? AND status='pending'").run(run.id);
  if (!claim.changes) return;   // 重复提交只放行一个
  Promise.resolve().then(() => runLoop(run.id, contents, exam, user, [], planText)).catch(() => { try { db.prepare("UPDATE chat_runs SET status='error' WHERE id=?").run(run.id); } catch {} });
}
