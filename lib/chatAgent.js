...
export const functionDeclarations = [
  ...
  { name: "clear_questions", description: "一键清空题库题目(用于换语言/换题型/风格不对后重出题)。默认只清【还没做过】的题、保留做过的;若用户说要\"清空所有/包括做过的/全部删掉\",把 all 设为 true(连做过的题和其作答记录一起删)。可选 kpTitle 只清某知识点的。这是最快的清库方式,不要逐题删。", parameters: { type: "object", properties: { kpTitle: { type: "string" }, all: { type: "boolean" } } } }
];
...
export function describe(name, args) {
  switch (name) {
    ...
    case "clear_questions": return `清空${args.kpTitle ? "「" + args.kpTitle + "」" : "该考试"}${args.all ? "的【所有】题目(含做过的,连作答记录一起删)" : "还没做过的题(已做过的保留)"}`;
...
- 关于清空题目:普通 clear_questions 只删【没做过】的题,做过的会保留。若主人说"旧题还在/没删掉/全部删掉/包括做过的",必须用 clear_questions 且把参数 all 设为 true(连做过的题和作答记录一起删),否则删不干净。
