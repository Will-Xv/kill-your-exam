...
export const functionDeclarations = [
  ...
  { name: "save_learning_mode", description: "保存/更新一个【命名学习模式或配方】——把主人用大白话定的学习规则,存成一条可复用、可激活的规则集。name=模式名(如「冲刺模式」「苏格拉底模式」「先做题后讲」);rules=这套模式下你要遵守的规则,写清楚、可执行(例如:先5分钟讲概念→10分钟做题→5分钟复盘;数学类先给题、错了再反推概念;多用类比、少让我背)。scope=exam(只本考试)或 global(全局·你的全部杀技,所有考试通用)。activate=是否立即生效。生效后这套规则进入你的系统认知,你要一直遵守直到被停用。主人要你『记住以后怎么教/怎么排计划』时,就用它存成模式,而不是只答应一次。若规则里含【自动触发】(如『连错2题自动降难度』『连对3题自动升难度』),【除了 rules 里写清楚,还要在 triggers 里给出结构化触发器】,系统才会在真实做题时确定性地自动执行;现支持:event=answer,when=consecutive_wrong|consecutive_correct,n=次数,action=difficulty_down|difficulty_up。", parameters: { type: "object", properties: { name: { type: "string" }, rules: { type: "string" }, scope: { type: "string", enum: ["exam", "global"] }, activate: { type: "boolean" }, triggers: { type: "array", description: "结构化自动触发器(可选)。每项:{event:\"answer\", when:\"consecutive_wrong\"|\"consecutive_correct\", n:整数, action:\"difficulty_down\"|\"difficulty_up\"}", items: { type: "object", properties: { event: { type: "string" }, when: { type: "string" }, n: { type: "integer" }, action: { type: "string" } } } } }, required: ["name", "rules"] }, devOnly: true },
