...
// 无考试时也能用的、与具体考试无关的工具
const NOEXAM_TOOLS = new Set(["web_search", "read_overall_profile", "update_overall_profile", "get_profile", "set_profile", "list_memory", "forget_fact", "ui_read"]);
...
export const functionDeclarations = [
  ...
  { name: "ui_set_killer_home", description: "把你自己(杀手)搬到哪显示:dock=默认(电脑右侧常驻大面板/手机浮动气泡)、nav=导航栏里的入口按钮、more=更多菜单里的入口、morefeatures=更多功能里的入口卡片。选 nav/more/morefeatures = 把自己【缩小成一个入口按钮】(点开是全屏抽屉),这【不是隐藏】,你依然随时可达。【你绝不能把自己彻底隐藏或移除,只能换位置或缩小】。默认电脑手机一致,只改一端传 breakpoint。可 ui_undo 撤销。", parameters: { type: "object", properties: { mode: { type: "string", enum: ["dock", "nav", "more", "morefeatures"] }, breakpoint: { type: "string", enum: ["desktop", "mobile"] } }, required: ["mode"] }, devOnly: true },
...
export async function execTool(name, args, exam, user) {
  ...
  switch (name) {
    ...
    case "ui_set_killer_home": { const kh = setKillerHome(exam.id, user.id, args.mode, args.breakpoint); return kh ? { ok: true, note: `杀手已搬到 ${({dock:"默认常驻",nav:"导航栏入口",more:"更多菜单入口",morefeatures:"更多功能入口"})[args.mode]||args.mode}${args.breakpoint ? "(" + args.breakpoint + ")" : "(电脑+手机)"};可 ui_undo 撤销` } : { ok: false, note: "mode 只能是 dock/nav/more/morefeatures" }; }
