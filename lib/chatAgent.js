...
- ★title/detail 是【写给主人看的】,一律用大白话,【绝对不要出现工具名/英文函数名/内部代号】(主人不关心也看不懂这些)。
- ★另外填一个 tools 字段:这一步会用到的工具名数组(必须是下面清单里的真名,别编)。【这个字段主人看不到】,只用来在他同意计划后生成一张"这次会做哪些改动"的授权表,让他一次性勾选、免得执行到一半又被反复打扰确认。所以凡是会动数据的步骤,务必把工具名填全——但再说一次,工具名只许出现在 tools 里,不许出现在 title/detail 里。
只能用下列真实能力来规划,别编不存在的:
${toolList}
