...
import { getDifficultyPref } from "@/lib/difficultyPref";
...
export function extractMemoryBg(user, examId, convoText) {
  ...
  const isDev = true; // 所有账号都做 exam/global 记忆分层
  (async () => {
    try {
      const props = {
...
export function difficultyHint(userId, examId) {
  // 若有【自动升降难度规则】算出的硬性档位,它优先级最高、直接覆盖软性判断(否则两套指令会打架、被平均掉)。
  try {
    if (examId) {
      const p = getDifficultyPref(examId);
      if (p) return `\n【难度档位 · 硬性(自动升/降难度规则得出,优先级最高)】接下来生成的【每一道题】difficulty 字段都必须 = ${p}(1易~3难),题目【实际难度】也要匹配这个档位:${p <= 1 ? "只出基础、直接、单步的题,不要出综合应用或高难题" : p >= 3 ? "多出综合应用、多步推理、高难度的题" : "以中档难度为主"}。【忽略下面任何与此冲突的难度建议】。`;
    }
  } catch {}
...
// ——— 记忆的读取/软删/恢复(给「你的全部杀技」页 + 杀手工具用;删除=软删 active=0,可恢复,便于排查)———
export function listMemory(userId, { scope = null, includeInactive = false } = {}) {
  let sql = "SELECT id,subject,kind,claim,valence,quote,weight,created_at,scope,active,exam_id FROM memory_facts WHERE user_id=?";
  const args = [userId];
  if (scope === "global") sql += " AND (scope='global' OR scope IS NULL)";
  else if (scope === "exam") sql += " AND scope='exam'";
  if (!includeInactive) sql += " AND active=1";
  sql += " ORDER BY created_at DESC LIMIT 500";
  return db.prepare(sql).all(...args).map((r) => ({ ...r, recency: +recencyWeight(r).toFixed(3) }));
}
export function forgetFact(userId, id) { return db.prepare("UPDATE memory_facts SET active=0 WHERE id=? AND user_id=?").run(id, userId).changes > 0; }
export function restoreFact(userId, id) { return db.prepare("UPDATE memory_facts SET active=1 WHERE id=? AND user_id=?").run(id, userId).changes > 0; }

// ——— 错题驱动的【细颗粒不熟条目】(仅开发者账号,后台非阻塞)———
// 知识点分得不够细时,从一道错题里识别一个"比知识点更细"的具体薄弱(某类题型的坑/某个小概念),
// 即使用户没记笔记,也自动存成 exam 作用域的 observation 记忆,纳入杀手记忆与后续训练。
export function analyzeMistakeBg(user, examId, ctx = {}) {
  if (!user) return; // 类11:所有账号,判分时顺手提炼细颗粒根因信号
  (async () => {
    try {
      const schema = { type: "object", properties: {
        hasFineGrained: { type: "boolean", description: "这道错题是否暴露了一个【比知识点更细】、值得单独长期记住的具体薄弱(某类题型的固定坑、某个小概念理解有瑕疵、某个反复用错的方法)" },
        subject: { type: "string", description: "简短的细颗粒薄弱标签,便于当记忆主题词,如「三角换元识别」「分部积分符号」「排除法过度自信」" },
        claim: { type: "string", description: "一句话:这个人在这个细点上具体怎么错/怎么弱" }
      }, required: ["hasFineGrained"] };
      const prompt = `考生刚作答了一道题(判分结果:${ctx.correct ? "答对" : "答错"})。请判断这次作答是否暴露了一个【比知识点更细】、值得【长期单独记住】的薄弱点(某类题型的固定坑、某个小概念理解有瑕疵、某个反复用错的方法),而不是泛泛的"这个知识点没掌握"。
- 只有确实能提炼出【具体、可复现、值得单独训练】的细点时,才 hasFineGrained=true 并给 subject/claim;否则 hasFineGrained=false。
- subject 要短;claim 一句话说清坑在哪。别为凑数硬编。
题目:${String(ctx.stem || "").slice(0, 300)}
${ctx.kpTitle ? "所属知识点:" + ctx.kpTitle + "\n" : ""}考生答案:${String(ctx.userAnswer || "").slice(0, 200)}
参考答案:${String(ctx.correctAnswer || "").slice(0, 200)}` + langInstruction(user.lang);
      const out = await generateJson(prompt, schema, {});
      if (out && out.hasFineGrained && out.subject) {
        addFact(user.id, examId, { subject: String(out.subject).slice(0, 40), kind: "observation", claim: out.claim || out.subject, valence: "weak", scope: "exam", quote: String(ctx.stem || "").slice(0, 120) });
      }
    } catch {}
  })();
}
