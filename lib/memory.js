...
export function difficultyHint(userId, examId) {
  let hard = "";
  try { if (examId) { const p = getDifficultyPref(examId); if (p) hard = `\n【硬性难度档位 · 自动规则得出】接下来的题目难度请【以 ${p} 为主】(1易~3难),这是根据主人设定的自动升/降难度规则算出的当前档位,优先级高于下面的软性判断。`; } } catch {}
  const digest = memoryDigest(userId);
  if (!digest) return hard;
