...
// 取该考试已存原始文件的多模态 parts(图片/音频等),供 Gemini 直接读取
export function materialParts(examId, { kinds = ["image", "audio"], max = 4, maxBytes = 18 * 1024 * 1024 } = {}) {
  let rows = [];
  try { rows = db.prepare("SELECT id, mime, kind FROM materials WHERE exam_id=? AND status='ready' AND stored=1 AND COALESCE(auto,0)=0 ORDER BY id DESC").all(examId); } catch { return []; }
  const out = []; let total = 0;
  for (const r of rows) {
    if (!kinds.includes(r.kind)) continue;
    const buf = readMat(r.id);
    if (!buf || buf.length > maxBytes || total + buf.length > maxBytes) continue;
    out.push({ inlineData: { mimeType: r.mime || "application/octet-stream", data: buf.toString("base64") } });
    total += buf.length;
...
// 便捷:把该考试的原始图片/音频作为多模态附件拼进一次生成调用
export function mmOpts(examId, prompt, extraParts = []) {
  const parts = [...materialParts(examId), ...(extraParts || [])];
