...
import { invalidateKnowledgeState } from "@/lib/mastery";
...
export async function retrieve(examId, query, k = 6) {
  const rows = db
    .prepare(`SELECT c.id, c.material_id, c.content, c.heading_path, c.embedding, m.filename FROM chunks c
  ...
  const scored = rows
    .map((r) => {
      ...
      return { id: r.id, material_id: r.material_id, content: r.content, heading_path: r.heading_path, filename: r.filename, score: cosine(qv, v) };
    })
...
// ★max 默认 0:以前默认 20,任何调用方一不留神就把【整个资料库】塞进一次请求(PDF 按页计费)。
// 现在必须【显式】写 max 才会附原件,且优先用 retrieve 的片段,别再无差别投喂。
export async function materialParts(examId, { kinds = ["image", "audio", "pdf"], max = 0 } = {}) {
  if (!(max > 0)) return [];
  let rows = [];
...
// ★不再默认附资料:只把调用方【明确给的】extraParts 挂上。想附原件请自己调 materialParts({max:n})。
...
export async function afterMaterialsChanged(examId) {
  ...
  // 资料一变就清掉【缓存的知识点讲解】——否则讲解还是旧的、读不到新文件。下次看知识点会用新资料重新生成(懒重建)。
  try { db.prepare(`DELETE FROM explanations WHERE kp_id IN (SELECT id FROM knowledge_points WHERE exam_id IN ${scopeSql(familyScope(examId))})`).run(); } catch {}
  try { invalidateKnowledgeState(examId); } catch {}
}

// 【只挂检索命中的媒体原件】retrieve 的结果带 material_id;若命中的资料是图片/音频(听力题、看图讲解
// 这类确实需要看/听原件的场景),就把【命中的那些】作为附件挂上,而不是把整个资料库倒进去。
// ★不设条数上限(Will):真命中几份就挂几份 —— 万一这道题确实要对着好几张图/几段音频呢。
//   天然上限就是 retrieve 的 k(通常 4~6 条),且要 kind 为 image/audio 才挂,不会失控。
// PDF 不在此列:PDF 的要点已经在 chunks 里(入库时 Gemini 读过并按页分节),要看具体页由 read_material 去取。
export async function hitMediaParts(examId, hits) {
  const ids = [...new Set((hits || []).map((h) => h.material_id).filter(Boolean))];
  if (!ids.length) return [];
  const out = [];
  for (const id of ids) {
    let r; try { r = db.prepare("SELECT id, mime, kind FROM materials WHERE id=? AND status='ready' AND stored=1").get(id); } catch {}
    if (!r || !["image", "audio"].includes(r.kind)) continue;
    const p = await materialFilePart(r);
    if (p) out.push(p);
  }
  return out;
