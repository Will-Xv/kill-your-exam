...
async function identifyStyle(buffer) {
  try {
    ...
    let audioPart;
    try { const up = await uploadMedia(buffer, "audio/mpeg", "mp3"); audioPart = { fileData: { fileUri: up.fileUri, mimeType: up.mimeType } }; }
    catch { audioPart = { inlineData: { mimeType: "audio/mpeg", data: buffer.toString("base64") } }; }
...
export async function alignStemToMusic(stem, instructions, style) {
  ...
  try {
    const out = await generateJson(
      `所给配乐经AI试听,实际风格是:「${style}」。下面是一道"跟随所给音乐即兴"的表演/形体题。如果题干或说明里声称的乐种/乐器/曲风与该实际风格不符(比如写了"戏曲打击乐/二胡古典"但实际并非),请据实改写:要么改成与实际风格一致,要么不指定乐种、只说"所给音乐"。保持它仍是"跟随所给音乐即兴"的形体/舞蹈类题,考核维度不变,不要新增具体曲名。★【语言】改写后的 stem/instructions 必须与【原题干本身的语言完全一致】(原题是英文就用英文改、是中文就用中文改),【绝不要】把题干翻译或改写成另一种语言——这道题会原样显示给考生,语言一换他就懵了。\n题干:${stem}\n说明:${instructions || "(无)"}\n返回改写后的 stem 与 instructions。`,
