...
export function createMode(user, exam, { kind, name, emoji, spec, meterLabel, winDesc, meterStart, meterDir, format }) {
  ...
  if (k === "exam_form") {
    ...
    try { moveFeature(exam.id, user.id, { featureId: fid, where: "morefeatures" }); } catch {}
  }
...
要有意思、有区分度、扣题。` + langInstruction(user.lang),
  ...
  for (const m of (out.modes || []).slice(0, 6)) {
    try { const r = createMode(user, exam, { kind: "exam_form", name: m.name, emoji: m.emoji, spec: m.spec, meterLabel: m.meterLabel, winDesc: m.winDesc, meterDir: m.meterDir, format: m.format }); created.push({ id: r.id, name: m.name, format: m.format === "video" ? "video" : "interactive" }); } catch {}
  }
  return { created };
}