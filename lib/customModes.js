...
export function createMode(user, exam, { kind, name, emoji, spec, meterLabel, winDesc, meterStart, meterDir, format, where }) {
  ...
  const info = db.prepare(`INSERT INTO custom_modes(exam_id,user_id,kind,name,emoji,spec,meter_label,win_desc,meter_start,meter_dir,scope,format)
    VALUES(?,?,?,?,?,?,?,?,?,?,'exam',?)`).run(
    exam.id, user.id, k, String(name || "自定义模式").slice(0, 24), String(emoji || (k === "exam_form" ? "🎯" : "🎲")).slice(0, 4),
  ...
  if (k === "exam_form") {
    const fid = "xform" + id;
    const label = String(name || "自定义考核").slice(0, 20);   // 硬上限:任何位置(尤其导航栏)都要短
    const icon = String(emoji || (format === "video" ? "🎬" : "🎯")).slice(0, 4);
    try { saveCustomItem({ id: fid, label, icon, desc: String(winDesc || "自定义考核形式").slice(0, 48), href: "/arena?launch=" + id }); } catch {}
    try { registerFeature({ feature_id: fid, name: label, icon }); } catch {}
    const w = WHERES.includes(where) ? where : "hidden";   // 默认只存竞技场(hidden=不摆到导航栏/首页/更多);要摆到界面得明确指定 where
...
每个考核给:
- name(考核名,【务必极简短】:≤12字符/约6个汉字,别用长句、别加花哨修饰词——各处栏目/导航栏都要放得下)、emoji(一个)
...
要有意思、有区分度、扣题。` + langInstruction(user.lang),
  ...
  for (const m of (out.modes || []).slice(0, 6)) {
    try { const r = createMode(user, exam, { kind: "exam_form", name: m.name, emoji: m.emoji, spec: m.spec, meterLabel: m.meterLabel, winDesc: m.winDesc, meterDir: m.meterDir, format: m.format, where: "hidden" }); created.push({ id: r.id, name: m.name, format: m.format === "video" ? "video" : "interactive" }); } catch {}
  }
  return { created };
}