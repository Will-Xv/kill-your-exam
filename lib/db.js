...
function openDb() {
  ...
  db.pragma("busy_timeout = 5000");
    ...
    try { db.exec("ALTER TABLE chat_runs ADD COLUMN act_log_json TEXT"); } catch {}
    try { db.exec("ALTER TABLE chat_runs ADD COLUMN plan_auth_json TEXT"); } catch {}
    try { db.exec("ALTER TABLE chat_runs ADD COLUMN resume_contents_json TEXT"); } catch {}  // 出错时把已跑完的工具进度(含结果)存这,近期重发接着用、不从零重做(Will:超时重发别重来)  // D1:计划门里主人一次性勾选的授权 {approved:[工具名],denied:[工具名]},执行时据此免再弹/直接拒  // 本轮真实改动日志(resume 不清空,累积到最终回复发出为止)
    // 【2026-07-19 迁移】「更多(☰ 菜单)」已取消:把存量放置表里 where="more" 的项直接改写成 "morefeatures",
    // 老用户原来放在更多里的栏目就落到首页「更多功能」网格里,不会失联。
    try {
      const rows = db.prepare("SELECT key, value FROM settings WHERE key='ui_item_placement' OR key LIKE 'ui_placement:%'").all();
      for (const r of rows) {
        let pl; try { pl = JSON.parse(r.value || "null"); } catch { continue; }
        if (!pl || typeof pl !== "object") continue;
        let touched = false;
        for (const bp of ["desktop", "mobile"]) {
          if (!Array.isArray(pl[bp])) continue;
          for (const e of pl[bp]) if (e && e.where === "more") { e.where = "morefeatures"; touched = true; }
        }
        if (touched) db.prepare("UPDATE settings SET value=? WHERE key=?").run(JSON.stringify(pl), r.key);
      }
    } catch {}
    ...
    try { db.exec(`ALTER TABLE practical_tasks ADD COLUMN completed_at TEXT`); } catch {}
    try { db.exec(`CREATE TABLE IF NOT EXISTS auto_rules (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, exam_id INTEGER, kind TEXT, freq TEXT, weekday INTEGER, hour INTEGER, minute INTEGER, text TEXT, next_run TEXT, last_run TEXT, active INTEGER DEFAULT 1, created_at TEXT DEFAULT (datetime('now')))`); } catch {}  // 循环自动规则:定时提醒/本周计划摘要                              // assignment 用它标完成(practical 靠里程碑)
    ...
    // 【考试分组·已退役 2026-07】原设计:把跨学科考试放进一个"组"里、纯界面组织不合并数据。后来改为
    //【所有考试默认就算一个组、靠首页 chips 切换】,不再有手动"建分组"这回事 → 砖头 exam_group_*、
    // lib/examGroups.js、components/ExamGroupPrompt.js、/api/exam-group 均已删除。这两张表也不再建。
    // (老账号里可能残留空表,无害;若要彻底清可手动 DROP。)
    ...
    // 建索引时顺带判断的:这份资料里有没有【成套的现成题目】(卷子/习题册=1,讲义/笔记=0,-1=还没判断过)。
    // 有题的才值得再花一次钱读原件把题抄进题库 —— 讲义类零浪费。
    try { db.exec("ALTER TABLE materials ADD COLUMN has_questions INTEGER DEFAULT -1"); } catch {}
    try { db.exec("ALTER TABLE materials ADD COLUMN questions_extracted INTEGER DEFAULT 0"); } catch {}
    // 上传做题:记住这次传的是哪几个文件,题库面板才能标出"来自上传做题的哪份文件"
    try { db.exec("ALTER TABLE quiz_sessions ADD COLUMN filenames TEXT"); } catch {}
    // 上传做题的附件会另存一份隐藏资料(供题目插图裁剪用),这里记住它们的 id
    try { db.exec("ALTER TABLE quiz_sessions ADD COLUMN fig_mids TEXT"); } catch {}
    // 资料的"角色":空=主人自己传的正经资料;'figure'=题目插图(系统从卷子里裁出来的,
    // 不该出现在资料列表/选文件出卷里 —— 它是题目的一部分,不是一份资料)。
    // 单靠 auto=1 区分不了:auto 早就被「[配乐]」占了,而配乐是要让主人看见、能删的。
    try { db.exec("ALTER TABLE materials ADD COLUMN role TEXT"); } catch {}
    ...
    try { db.exec("ALTER TABLE exams ADD COLUMN aggregate_children INTEGER DEFAULT 0"); } catch {}
    try { db.exec("ALTER TABLE exams ADD COLUMN duration_min INTEGER"); } catch {}  // 【真实考试时长(分钟)】只记主人/大纲说过的真实值;不知道就留空,绝不由 AI 编一个 // 汇总复习:母考试是否把子树的知识点/题库/进度合并复习
    ...
    try { db.exec("ALTER TABLE chat_runs ADD COLUMN form_json TEXT"); } catch {}  // 通用对话内参数表单(ask_user_form)的字段规格
    ...
    // 【Token 用量·按用户+日期+模型聚合】每次 Gemini 调用累加一行,供管理面板看每个用户烧了多少。
    // thoughts = 思考(reasoning)token —— 它【不含在 candidatesTokenCount 里】但计费,所以必须单独记,
    // 否则只看输入/输出会严重低估。user_id=0 表示后台任务(cron/入库/判题)等拿不到请求用户的调用。
    ...
    db.exec(`CREATE UNIQUE INDEX IF NOT EXISTS ix_token_usage ON token_usage(user_id, day, model);`);
    try { db.exec(`ALTER TABLE token_usage ADD COLUMN tool_use INTEGER NOT NULL DEFAULT 0`); } catch {}
...
// 把激活考试切到指定考试(校验归属)。用于打开别的考试的任务时自动切考试,让整个 UI(学习/模拟/掌握度/杀手)跟着对齐,而不是 403。
export function setActiveExam(userId, examId) {
  const e = db.prepare("SELECT id, user_id FROM exams WHERE id=? AND deleted_at IS NULL").get(examId);
  if (!e || e.user_id !== userId) return false;
  db.prepare("UPDATE exams SET status='archived' WHERE user_id=? AND status='active'").run(userId);
...
// 【考试自身范围·给"模拟考/快速复习"这类只该考本门的场景用】:开了汇总复习的母考试=自己+全部子孙;否则只有它自己(绝不含兄弟/父,避免家族其他考试内容串进来)。
export function ownScope(examId) {
  const id = Number(examId);
  const ex = db.prepare("SELECT aggregate_children FROM exams WHERE id=?").get(id);
  if (!(ex && ex.aggregate_children)) return [id];
  const ids = [id], seen = new Set(ids); let frontier = [id], guard = 0;
  while (frontier.length && guard++ < 200) {
    const next = [];
    for (const pid of frontier) for (const k of db.prepare("SELECT id FROM exams WHERE parent_exam_id=? AND deleted_at IS NULL").all(pid)) if (!seen.has(k.id)) { seen.add(k.id); ids.push(k.id); next.push(k.id); }
    frontier = next;
  }
  return ids;
}
// 【禁止建同名考试】某用户名下(未删除)是否已有完全同名的考试(去空格、忽略大小写);excludeId 可排除自己。
export function examNameExists(userId, name, { parentExamId = null, excludeId = null } = {}) {
  const n = String(name || "").trim().toLowerCase();
  if (!n) return false;
  const p = parentExamId != null ? Number(parentExamId) : null; // 同一层级去重:顶层 vs 顶层、同一母考试下的子考试之间(允许不同课都有 Quiz 1)
  try {
    const rows = db.prepare("SELECT id, name, parent_exam_id FROM exams WHERE user_id=? AND deleted_at IS NULL").all(userId);
    return rows.some((r) => Number(r.id) !== Number(excludeId) && ((r.parent_exam_id != null ? Number(r.parent_exam_id) : null) === p) && String(r.name || "").trim().toLowerCase() === n);
  } catch { return false; }
}
...
export function resetUserData(userId) {
  ...
  const acct = db.prepare("SELECT username, password_hash, salt, is_admin, is_developer, lang, email, google_sub, name, avatar_url FROM users WHERE id=?").get(uid) || {};
  const examIds = db.prepare("SELECT id FROM exams WHERE user_id=?").all(uid).map((r) => r.id);
  const examList = "(" + (examIds.length ? examIds.join(",") : "0") + ")";
  const del = (sql, ...a) => { try { db.prepare(sql).run(...a); } catch {} };
  const tx = db.transaction(() => {
    // 1) 先删【间接挂靠】的子表(它们不带 user_id/exam_id,靠 kp_id/question_id/recipe_id 关联),避免删了父行后找不到
    del(`DELETE FROM explanations WHERE kp_id IN (SELECT id FROM knowledge_points WHERE exam_id IN ${examList})`);
    del(`DELETE FROM review_queue WHERE question_id IN (SELECT id FROM questions WHERE exam_id IN ${examList})`);
    del(`DELETE FROM recipe_phase_state WHERE recipe_id IN (SELECT id FROM recipes WHERE user_id=?)`, uid);
    del(`DELETE FROM recipe_versions WHERE recipe_id IN (SELECT id FROM recipes WHERE user_id=?)`, uid);
    // 2) 通用清扫:遍历所有表,凡带 user_id 的按本用户删、凡带 exam_id 的按本用户的考试(含无考试对话的哨兵 -uid)删。
    //    这样新增的表也不会漏(彻底把账号清成"同 ID 的新用户")。
    let tables = [];
    try { tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'").all().map((r) => r.name); } catch {}
    for (const t of tables) {
      if (t === "users" || t === "settings" || t === "sessions") continue; // sessions=登录态,绝不能清(否则账号被登出→401);users/settings另行处理
      let cols = [];
      try { cols = db.prepare(`PRAGMA table_info(${t})`).all().map((c) => c.name); } catch {}
      if (cols.includes("user_id")) del(`DELETE FROM ${t} WHERE user_id=?`, uid);
      if (cols.includes("exam_id")) { del(`DELETE FROM ${t} WHERE exam_id IN ${examList}`); del(`DELETE FROM ${t} WHERE exam_id=?`, -uid); }
    }
    // 3) 考试本体
    del(`DELETE FROM exams WHERE user_id=?`, uid);
    // 4) settings 里按用户/考试存的键:按天排期、日期穿越、时区、每门考试的实践模式与界面放置
    del(`DELETE FROM settings WHERE key=?`, `day_plan:${uid}`);
    del(`DELETE FROM settings WHERE key=?`, `dev_day_offset:${uid}`);
    del(`DELETE FROM settings WHERE key=?`, `tz:${uid}`);
    for (const eid of examIds) { del(`DELETE FROM settings WHERE key=?`, `practical_mode:${eid}`); del(`DELETE FROM settings WHERE key=?`, `ui_placement:${eid}`); }
    // 5) 把用户行本身【删掉再用同一个 id + 同登录重建】——等于新建一个账号,只是 id 和登录不变。
    //    其余所有列(通知偏好/时区/整体画像/onboarded/guide_version 等)都回到建表默认值,不残留任何旧状态。
    del(`DELETE FROM users WHERE id=?`, uid);
    try {
      db.prepare(`INSERT INTO users (id, username, password_hash, salt, is_admin, is_developer, lang, email, google_sub, name, avatar_url) VALUES (?,?,?,?,?,?,?,?,?,?,?)`)
        .run(uid, acct.username, acct.password_hash, acct.salt, acct.is_admin || 0, acct.is_developer || 0, acct.lang || "en", acct.email ?? null, acct.google_sub ?? null, acct.name ?? null, acct.avatar_url ?? null);
    } catch {}
  });
  tx();
