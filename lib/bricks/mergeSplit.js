...
registerBrick({
  ...
  inputs: [
    ...
    { key: "examDate", type: "string", required: false, desc: "两门考试【日期不一样】时,主人决定合并后用哪个日期(YYYY-MM-DD)。日期相同或只有一方有日期时不用传。" },
    { key: "mode", type: "string", required: false, desc: "主人选定的结构:'merge'=真合并成一门(移动数据+软删来源,不可轻易拆回);'family'=不要合并、改成父子结构(把来源挂到目标下当子考试,数据打通但各自保留)。【没拿到主人明确选择前不要传】——留空会触发反问,让你去问主人要哪种。" },
  ],
  run: async (args, ctx) => {
    ...
    // 【结构决策·不替主人做主·最先判】合并 vs 父子家族,是两个后果完全不同的选择(合并会移动数据+软删来源、
    // 家族只是挂父子且各自保留),【无论日期是否冲突,都必须先拿到主人的明确选择】。以前这道门被错误地【绑在日期冲突上】
    // ——只有"两门都有日期且不同"才会问,于是没设日期/只有一方有日期时,杀手就直接闷头合并了(Will 实测遇到)。
    if (args.mode !== "merge") {
      if (args.mode === "family") {
        return { ok: false, needsDecision: "use_set_parent",
          note: `主人要的是【父子结构】而不是合并。请【不要】调用 exam_merge,改用 exam_set_parent 把「${src.name}」挂到「${dst.name}」下面(数据打通、各自保留日期与身份,以后也能拆开)。` };
      }
      return { ok: false, needsDecision: "merge_or_family",
        note: `先别急着合并——这两门考试放一起有【两种完全不同】的方式,我不替主人做主,请先问清楚他要哪种:\n①【真合并成一门】(exam_merge):把「${src.name}」的知识点/题目/作答/资料全部【移动】进「${dst.name}」,然后把「${src.name}」删掉。合并后就是一门课了,想拆回来只能靠回档。\n②【父子结构】(exam_set_parent):把「${src.name}」挂到「${dst.name}」下面当子考试,两门【各自保留】自己的身份、日期、进度,但资料/复习在家族内打通。以后随时能拆开。\n拿到主人的选择后:要①就带 mode=\"merge\" 再调一次(若两门日期不同还要问清用哪个日期、用 examDate 传);要②就改调 exam_set_parent。` };
    }
    // 【日期决策·不替主人做主】确认要合并后,若两边都有日期且【不一样】,还要问用哪个日期。
    // 日期相同、或只有一方有日期 → 无歧义,直接合并并继承那个日期。
    const _sd = src.exam_date ? String(src.exam_date).slice(0, 10) : null;
    const _dd = dst.exam_date ? String(dst.exam_date).slice(0, 10) : null;
    const _picked = args && args.examDate && /^\d{4}-\d{2}-\d{2}$/.test(String(args.examDate)) ? String(args.examDate) : null;
    if (!_picked && _sd && _dd && _sd !== _dd) {
      return {
        ok: false, needsDecision: "exam_date",
        note: `没有合并——这两门考试的日期不一样(「${src.name}」是 ${_sd},「${dst.name}」是 ${_dd}),我不替主人决定。请先问主人两件事:①他是想把它们【合并成一门】,还是其实想要【父子结构】(把一门挂到另一门下面当小任务、数据打通但各自保留自己的日期,用 exam_set_parent)?②如果确认要合并成一门,合并后【用哪个日期】(${_sd} / ${_dd},或者他另给一个)?拿到答复后:要父子结构就改用 exam_set_parent;要合并就把选定的日期通过 examDate 传进来再调一次。`,
      };
    }
    ...
    const tx = db.transaction(() => {
      ...
      // 软删来源。
      // 【日期继承】主人明确指定了就用他指定的;否则两边日期相同、或只有一方有日期(上面的决策门已保证不会是"不同且都有"),
      // 直接用那个唯一确定的日期。不继承的话合并出来的考试会是「No date」——首页没倒计时、跨考试排期也会漏排它(P4-9)。
      try {
        const keep = _picked || (!_dd && _sd ? _sd : null);
        if (keep) { db.prepare("UPDATE exams SET exam_date=? WHERE id=?").run(keep, dst.id); stats.dateInherited = keep; }
      } catch {}
