...
registerBrick({
  ...
  run: async (args, ctx) => {
    ...
    const tx = db.transaction(() => {
      ...
      // 软删来源。
      // 【日期继承】合并后目标考试的日期取两者中【更紧迫(更早)】的那个;目标原本没日期就直接用来源的。
      // 不继承的话合并出来的考试会是「No date」——首页没有倒计时、跨考试排期也会因为缺日期把它排错/漏掉(P4-9)。
      try {
        const sd = src.exam_date ? String(src.exam_date).slice(0, 10) : null;
        const dd = dst.exam_date ? String(dst.exam_date).slice(0, 10) : null;
        const keep = (!dd && sd) ? sd : (sd && dd && sd < dd) ? sd : null;
