...
registerBrick({
  ...
  run: async (args, ctx) => {
    ...
    const r = createMode(ctx.user, exam, { kind: args.kind, name: args.name, emoji: args.emoji, spec: args.spec, meterLabel: args.meterLabel, winDesc: args.winDesc, meterDir: args.meterDir, format: args.format });
...
registerBrick({
  ...
  description: "针对当前考试/学习内容,由 AI【创意生成】若干贴合内容的考核形式(互动答辩/情境模拟/角色扮演/视频应用等),直接入库供选用。用户说「给我出几个庄子的考核」「AI 帮我想几种考法」「自动生成考核形式」时用。count=生成几个(默认3,上限5)。生成后用户可到 /arena 直接玩,或再手改。",
