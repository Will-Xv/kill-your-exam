...
registerBrick({
  ...
  description: "把这门考试的知识点【按学习进程均匀铺进按天排期】。★仅当【没有任何带日期的考核(quiz/期中/期末/作业)】、纯粹想按节奏把一门学完时才用本工具;【一旦有考核日期,别用它均匀铺】——要先看每个考核考啥、在什么时候前必须学完,用 add_plan_items 按锚点排(见 appGuide「锚定考核」)。★优先让主人用页面上的『排学习计划』弹窗来定这些参数(计划类的问题走弹窗、别在对话里一条条追问);当参数已经明确时才用本工具直接生成。mode:'deadline'(有考试日期,用 examDate)/'until'(学到某天,用 targetDate)/'weeks'(用几周,用 weeks)/'open'(没时间要求→按每天能学多久的节奏铺、并估算大概几周)。dailyMinutes 每天能学多久(分钟);skipWeekends=true 跳过周末,或 skipDays=[要跳过的周几0..6]。生成后写进按天排期,主人可在「本周计划表」里改或同意。",
  inputs: [
