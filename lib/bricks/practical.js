...
import { assignTask, listTasks, deleteTask, setPracticalMode, maybeAutoAssign, createAssignment, setTaskDue } from "@/lib/practical";
...
registerBrick({
  ...
  run: async (args, ctx) => {
    ...
    if (Array.isArray(topics) && topics.length) {
      ...
      const results = await Promise.all(list.map((tp, i) => assignTask(ctx.user, exam, { topic: tp, dueDate: dues[i] }).catch(() => null)));
...
registerBrick({
  name: "add_assignment", category: "practical", title: "把上传的作业标成『作业助手型作业』", write: true,
