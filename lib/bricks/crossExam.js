...
// —— 【考试分组】纯界面/今日任务组织(不动作用域、不合并数据;和家族/exam_merge 分开)——
import { listGroups as _lg, createGroup as _cg, addToGroup as _ag, removeFromGroup as _rg, deleteGroup as _dg, findGroup as _fg } from "@/lib/examGroups";
function _resolveExamIds(userId, names) {
  const rows = db.prepare("SELECT id,name FROM exams WHERE user_id=? AND deleted_at IS NULL").all(userId);
  const ids = [], miss = [];
  for (const nm of (Array.isArray(names) ? names : [])) {
    const kw = String(nm || "").toLowerCase().trim(); if (!kw) continue;
    const hit = rows.find((r) => String(r.name || "").toLowerCase() === kw) || rows.find((r) => String(r.name || "").toLowerCase().includes(kw) || kw.includes(String(r.name || "").toLowerCase()));
    if (hit) ids.push(hit.id); else miss.push(nm);
  }
  return { ids, miss };
}
function _asArr(x) { if (Array.isArray(x)) return x; try { const p = JSON.parse(x); return Array.isArray(p) ? p : (x ? [x] : []); } catch { return x ? [x] : []; } }

registerBrick({
  name: "exam_group_create", category: "cross_exam", title: "建一个考试分组(界面/今日任务组织,不合并数据)", write: true,
  description: "把几门(可跨学科)考试放进一个【分组】方便统一看今日任务。【纯界面/管理组织,不合并知识树/掌握度/资料——和「把小测挂到母考试下(exam_set_parent)+汇总复习」以及「exam_merge 真合并」完全不同】。用户有一堆互不相关的考试想放一起管时用。name=分组名;examNames=要放进来的考试名(JSON 数组,按名字匹配)。",
  inputs: [{ key: "name", type: "string", required: true, desc: "分组名(如「本学期」「期末季」)" }, { key: "examNames", type: "json", required: false, desc: "要放进这个分组的考试名数组(JSON,如 [\"MAT235\",\"BIO120\"])" }],
  run: async (args, ctx) => {
    const { ids, miss } = _resolveExamIds(ctx.user.id, _asArr(args.examNames));
    const gid = _cg(ctx.user.id, args.name, ids);
    return { ok: true, groupId: gid, added: ids.length, note: `已建分组「${args.name}」,放入 ${ids.length} 门考试${miss.length ? ";没找到:" + miss.join("、") : ""}(这是界面/今日任务分组,各考试数据仍各自独立)` };
  },
});
registerBrick({
  name: "exam_group_add", category: "cross_exam", title: "往考试分组里加考试", write: true,
  description: "把考试加进一个已有的【分组】。name=分组名;examNames=考试名数组(JSON)。",
  inputs: [{ key: "name", type: "string", required: true, desc: "分组名" }, { key: "examNames", type: "json", required: true, desc: "要加入的考试名数组(JSON)" }],
  run: async (args, ctx) => {
    const g = _fg(ctx.user.id, args.name); if (!g) return { ok: false, note: `没找到分组「${args.name}」` };
    const { ids, miss } = _resolveExamIds(ctx.user.id, _asArr(args.examNames));
    const n = _ag(g.id, ids);
    return { ok: true, added: n, note: `已往分组「${g.name}」加入 ${n} 门${miss.length ? ";没找到:" + miss.join("、") : ""}` };
  },
});
registerBrick({
  name: "exam_group_remove", category: "cross_exam", title: "从考试分组里移出考试", write: true,
  description: "把考试从一个【分组】里移出(考试本身不受影响)。name=分组名;examNames=考试名数组(JSON)。",
  inputs: [{ key: "name", type: "string", required: true, desc: "分组名" }, { key: "examNames", type: "json", required: true, desc: "要移出的考试名数组(JSON)" }],
  run: async (args, ctx) => {
    const g = _fg(ctx.user.id, args.name); if (!g) return { ok: false, note: `没找到分组「${args.name}」` };
    const { ids } = _resolveExamIds(ctx.user.id, _asArr(args.examNames));
    const n = _rg(g.id, ids);
    return { ok: true, removed: n, note: `已从分组「${g.name}」移出 ${n} 门` };
  },
});
registerBrick({
  name: "exam_group_list", category: "cross_exam", title: "列出所有考试分组", write: false,
  description: "列出用户建的所有【考试分组】及各组里的考试。",
  inputs: [],
  run: async (args, ctx) => {
    const gs = _lg(ctx.user.id);
    const nameOf = (id) => db.prepare("SELECT name FROM exams WHERE id=?").get(id)?.name || ("#" + id);
    return { ok: true, groups: gs.map((g) => ({ name: g.name, exams: g.examIds.map(nameOf) })) };
  },
});
registerBrick({
  name: "exam_group_delete", category: "cross_exam", title: "删掉一个考试分组", write: true,
  description: "删掉一个【考试分组】(只删分组,里面的考试都不受影响)。name=分组名。",
  inputs: [{ key: "name", type: "string", required: true, desc: "分组名" }],
  run: async (args, ctx) => {
    const g = _fg(ctx.user.id, args.name); if (!g) return { ok: false, note: `没找到分组「${args.name}」` };
    _dg(ctx.user.id, g.id);
    return { ok: true, note: `已删除分组「${g.name}」(里面的考试都还在)` };
  },
});
