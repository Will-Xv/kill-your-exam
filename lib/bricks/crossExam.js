...
registerBrick({
  ...
  run: async (args, ctx) => {
    ...
    const info = db.prepare("INSERT INTO exams(name,user_id,status,exam_type,parent_exam_id,assess_status) VALUES(?,?,'active',?,?,'done')").run(name, ctx.user.id, args.examType || null, parent);
    return { examId: info.lastInsertRowid };
  },
});
...
// 4) 解除小任务关系
registerBrick({
  name: "exam_unset_parent", category: "cross_exam", title: "解除小任务关系", write: true,
  description: "把 examId 的父考试清空(不再是任何考试的小任务)。数据保留不动。",
  ...
  run: async (args, ctx) => {
    ...
    db.prepare("UPDATE exams SET parent_exam_id=NULL WHERE id=?").run(a.id);
    return { ok: true, examId: a.id };
  },
});
...
registerBrick({
  ...
  description: "把 fromExamId 的若干知识点复制进 toExamId(新建点,保留章节归属;目标里没有的章节会自动建)。kpIds 省略=复制全部点。underChapter 可把它们统一归到目标里某个章节名下。返回 idMap(来源点id→新点id)。",
...
registerBrick({
  ...
  description: "按 kpMap(来源知识点id→目标知识点id)把来源考试对应的题目复制进目标考试(重挂 exam_id/kp_id)。withAttempts=true 时把这些题的作答记录(练习历史/掌握度依据)也复制过去,从而「保留进度」。返回复制的题数与作答数。",
