...
import db, { examNameExists, getActiveExam } from "@/lib/db";
import { embed, cosine } from "@/lib/gemini";
import { registerBrick } from "@/lib/bricks/registry";
import { snapshot } from "@/lib/checkpoint";
import { recomputeReviewFromAttempts, mapSubExamMasteryToFamily } from "@/lib/mastery";
...
// 1b) 按名字/关键词把【所有】同名或疑似重复的考试都揪出来做候选(整理/合并前用,防漏)
registerBrick({
  name: "find_similar_exams", category: "cross_exam", title: "找出所有同名/疑似重复的考试(整理合并前用)", write: false,
  description: "主人要【整理/合并/清掉某科目的一堆记录】(如「把 MAT137 相关的都合并成一门」)时【先用它】,别靠 exam_list 肉眼挑——极易漏掉重复项(尤其是同名但日期不同、或已归档的)。传 query=科目名/关键词(如「MAT137」),返回名字里包含该关键词的【全部】考试(含已归档,不含已删),并按【归一化后的名字】给出【重复分组】(同名多条=真重复)。你必须把返回的【每一条】候选都列给主人、逐条确认要不要并/保留哪一门做主,别自己替主人省略任何一条;计数也以本工具返回的为准(别再说“共 4 条”却漏了第 5 条)。",
  inputs: [{ key: "query", type: "string", required: true, desc: "科目名或关键词,如「MAT137」「Calculus」" }],
  run: async (args, ctx) => {
    const q = String(args.query || "").trim();
    if (!q) throw new Error("query required");
    const norm = (x) => String(x || "").toLowerCase().replace(/[\s\p{P}]+/gu, " ").trim();
    const nq = norm(q);
    const rows = db.prepare("SELECT id,name,exam_type,exam_date,status,parent_exam_id FROM exams WHERE user_id=? AND deleted_at IS NULL ORDER BY id ASC").all(ctx.user.id);
    const matches = rows.filter((e) => norm(e.name).includes(nq)).map((e) => {
      const kp = db.prepare("SELECT COUNT(*) n FROM knowledge_points WHERE exam_id=? AND parent_id IS NOT NULL").get(e.id).n;
      const at = db.prepare("SELECT COUNT(*) n FROM attempts WHERE exam_id=?").get(e.id).n;
      return { id: e.id, name: e.name, examType: e.exam_type || null, examDate: e.exam_date || null, status: e.status, parentExamId: e.parent_exam_id || null, kpCount: kp, attempts: at, norm: norm(e.name) };
    });
    // 按归一化名字分组,count>1 的就是真·重复
    const byNorm = {};
    for (const m of matches) (byNorm[m.norm] = byNorm[m.norm] || []).push(m);
    const duplicateGroups = Object.entries(byNorm).filter(([, arr]) => arr.length > 1)
      .map(([nm, arr]) => ({ norm: nm, name: arr[0].name, count: arr.length, ids: arr.map((x) => x.id), items: arr.map((x) => ({ id: x.id, name: x.name, examDate: x.examDate, status: x.status })) }));
    return { query: q, total: matches.length, matches, duplicateGroups, note: `共找到 ${matches.length} 条包含「${q}」的考试${duplicateGroups.length ? `,其中 ${duplicateGroups.reduce((s, g) => s + g.count, 0)} 条属于 ${duplicateGroups.length} 组同名重复` : ""}。把每一条都报给主人确认,别漏。` };
  },
});
...
registerBrick({
  ...
  inputs: [{ key: "name", type: "string", required: true, desc: "考试/任务名称" }, { key: "examType", type: "string", required: false, desc: "类型,如 quiz/assignment/exam/study/language" }, { key: "examDate", type: "string", required: false, desc: "考试日期 YYYY-MM-DD(知道就填上,别留空——没日期首页不显示倒计时、跨考试排期也会漏排它)" }, { key: "parentExamId", type: "number", required: false, desc: "父考试 id(设为它的小任务)" }, { key: "langNative", type: "string", required: false, desc: "母语(语言类考试)" }, { key: "langKnown", type: "string", required: false, desc: "已会外语,逗号分隔,可多门" }, { key: "langTarget", type: "string", required: false, desc: "正在学的目标语" }],
  run: async (args, ctx) => {
    ...
    const _d = args && args.examDate && /^\d{4}-\d{2}-\d{2}$/.test(String(args.examDate)) ? String(args.examDate) : null;
    const info = db.prepare("INSERT INTO exams(name,user_id,status,exam_type,exam_date,parent_exam_id,assess_status) VALUES(?,?,?,?,?,?,'done')").run(name, ctx.user.id, status, args.examType || null, _d, parent);
    let langBg = null;
...
registerBrick({
  name: "switch_exam", category: "cross_exam", title: "把当前考试切换到另一门", write: true,
  description: "【真正切换主人的当前(active)考试】。主人说「切到 BIO120」「focus on X exam」「先做另一门」时用——之后所有默认作用于当前考试的操作(练习、今日任务、模式激活、计划等)才会作用到那门。★不切换就【别声称】切了:如果主人想在【另一门】考试上做事(如给 BIO120 排 cram 练习),你必须【先调 switch_exam 真正切过去】,再做后续;绝不能只在嘴上说「已切换/已锁定」却没真切。examName=目标考试名(模糊匹配)或 examId。",
  inputs: [{ key: "examName", type: "string", required: false, desc: "目标考试名(模糊匹配)" }, { key: "examId", type: "number", required: false, desc: "目标考试 id" }],
  run: async (args, ctx) => {
    let target = null;
    if (args && args.examId) { target = db.prepare("SELECT id, name FROM exams WHERE id=? AND user_id=? AND deleted_at IS NULL").get(Number(args.examId), ctx.user.id); }
    else if (args && args.examName) {
      const kw = String(args.examName).trim().toLowerCase();
      const rows = db.prepare("SELECT id, name FROM exams WHERE user_id=? AND deleted_at IS NULL AND parent_exam_id IS NULL ORDER BY id DESC").all(ctx.user.id);
      target = rows.find((r) => String(r.name || "").toLowerCase() === kw) || rows.find((r) => String(r.name || "").toLowerCase().includes(kw)) || rows.find((r) => kw.includes(String(r.name || "").toLowerCase()));
    }
    if (!target) return { ok: false, note: `没找到考试${args && args.examName ? "「" + args.examName + "」" : ""},切换失败。`, candidates: db.prepare("SELECT name FROM exams WHERE user_id=? AND deleted_at IS NULL AND parent_exam_id IS NULL").all(ctx.user.id).map((r) => r.name).slice(0, 8) };
    db.prepare("UPDATE exams SET status='archived' WHERE user_id=? AND status='active'").run(ctx.user.id);
    db.prepare("UPDATE exams SET status='active' WHERE id=?").run(target.id);
    return { ok: true, switchedExamId: target.id, name: target.name, note: `已切换到「${target.name}」,之后的操作都作用于它。` };
  },
});
...
// 【改考试的"已完成"状态】(2026-08,Will)
// 语义要点(和界面上那个按钮完全一致,别自作主张扩大):completed_at 只是【打个"考完了"的标记】——
//   ·【不】改 status、【不】切走当前考试、【不】删任何数据;考试照样能选中、能练、能查;
//   · 唯一区别是:不再显示倒计时、不再按"快到期"抢占今日任务的优先级。
// 所以它是可逆的:随时能取消。真子考试(有父考试)标记完成后,可以把它的掌握度【映射】进家族知识树,
// 但那是【会改数据】的一步,默认不做 —— 先问主人。
registerBrick({
  name: "exam_set_completed", category: "cross_exam", title: "标记考试为已完成 / 取消完成", write: true,
  description: "把一门考试标记为【已完成】(考完了),或【取消】完成标记。done=true 标记完成、false 取消。examId 不填就用【当前考试】。主人说「XX 考完了」「这门已经结束了」「把它标成完成」「我考完期中了」时用;说「我还没考完/标错了/撤销完成」时用 done=false。★这只是打个标记:不改状态、不切走、不删数据,考试照样能练能查,只是不再显示倒计时、也不再因为临近而抢占今日任务优先级 —— 随时可撤销,所以不必如临大敌,但仍要跟主人确认是哪一门。★若这是一门【子考试】(挂在某门母考试下),标记完成后【可以】把它的掌握度映射进家族知识树(母考试和同家族其它考试的对应知识点),那一步【会改数据】、默认不做:本工具会在返回里提示你,请【去问主人要不要映射】,他同意了再用 exam_map_mastery_to_family。",
  inputs: [
    { key: "done", type: "boolean", required: true, desc: "true=标记为已完成;false=取消完成标记" },
    { key: "examId", type: "number", required: false, desc: "考试 id;不填=当前考试" },
  ],
  run: async (args, ctx) => {
    const exam = args.examId ? own(ctx.user.id, args.examId) : (ctx.exam || getActiveExam(ctx.user.id));
    if (!exam) throw new Error("没有找到这门考试(或它不属于你)");
    const done = !!args.done;
    if (done && exam.completed_at) return { ok: true, examId: exam.id, name: exam.name, done: true, note: `「${exam.name}」本来就已经是已完成状态,没有改动。` };
    if (!done && !exam.completed_at) return { ok: true, examId: exam.id, name: exam.name, done: false, note: `「${exam.name}」本来就不是已完成状态,没有改动。` };
    try { snapshot(ctx.user.id, [exam.id], { op: "exam_completed", label: `${done ? "标记完成" : "取消完成"}:${exam.name}` }); } catch {}
    db.prepare(done ? "UPDATE exams SET completed_at=datetime('now') WHERE id=?" : "UPDATE exams SET completed_at=NULL WHERE id=?").run(exam.id);
    const isSub = !!exam.parent_exam_id;
    return {
      ok: true, examId: exam.id, name: exam.name, done, isSubExam: isSub,
      note: done
        ? `已把「${exam.name}」标记为已完成。它不再显示倒计时、也不再因为临近而抢占今日任务;内容都还在,随时能练能查,想撤销就再叫我一声。`
          + (isSub ? ` ★这是一门子考试 —— 要不要把它的掌握度【映射】进家族知识树(让母考试和同家族其它考试的对应知识点也知道你已经学到什么程度)?这一步会改数据,你说要我再做(exam_map_mastery_to_family)。` : "")
        : `已取消「${exam.name}」的完成标记,它重新按正常考试对待(倒计时和优先级都回来了)。`,
    };
  },
});

// 子考试完成后:把它的掌握度语义映射进家族知识树。★会改数据,必须主人明确同意后才调。
registerBrick({
  name: "exam_map_mastery_to_family", category: "cross_exam", title: "把子考试的掌握度映射进家族知识树", write: true,
  description: "把一门【子考试】的掌握度,按语义映射到它所在家族(母考试和同家族其它考试)的对应知识点上 —— 这样你在小测里学到的东西,母考试那边也知道。通常在子考试标记完成之后用。★这一步【会改数据】,必须【主人明确同意】才调用,别自己顺手做。examId 不填=当前考试。",
  inputs: [{ key: "examId", type: "number", required: false, desc: "子考试 id;不填=当前考试" }],
  run: async (args, ctx) => {
    const exam = args.examId ? own(ctx.user.id, args.examId) : (ctx.exam || getActiveExam(ctx.user.id));
    if (!exam) throw new Error("没有找到这门考试(或它不属于你)");
    if (!exam.parent_exam_id) return { ok: false, reason: "not_sub_exam", note: `「${exam.name}」不是子考试(没有母考试),没有可映射的家族。` };
    try { snapshot(ctx.user.id, [exam.id], { op: "map_mastery_family", label: `映射掌握度到家族:${exam.name}` }); } catch {}
    const r = await mapSubExamMasteryToFamily(exam.id);
    return { ok: true, examId: exam.id, name: exam.name, ...r };
  },
});
