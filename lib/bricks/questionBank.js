...
registerBrick({
  name: "resolve_reference_list", category: "question_bank", title: "把「教材第X页第Y题」型复习清单解析成真题入库", write: true,
  description: "主人上传的复习资料若是【指针型】——每条不是题目本身,而是指向教材某处(题目1=教材第57页第3题、p.42 ex.3、第五章习题5),用这个:逐条在他上传的【教材】里定位原题、提取出来、作为真题(固定题)入库,之后练习/模拟就能做到这些真题。给 materialId(那份指针清单资料的 id,可用 list_materials 查)或直接给 text。找不到/是扫描图/定位到但没完整题目的会如实列出,【绝不编题】。markMust=true 把入库的都标为必出。入库后可配合 bank_set_closed 只考这些。",
  inputs: [
    { key: "materialId", type: "number", required: false, desc: "指针清单资料的 id(与 text 二选一)" },
    { key: "text", type: "string", required: false, desc: "直接粘贴指针清单文本(与 materialId 二选一)" },
    { key: "markMust", type: "boolean", required: false, desc: "入库题是否标为必出(默认否)" },
  ],
  run: async (args, ctx) => {
    const exam = examOf(ctx); if (!exam) throw new Error("没有当前考试");
    const r = await resolveReferenceList(ctx.user, exam, { text: args.text, materialId: args.materialId, markMust: !!args.markMust });
    return { ok: true, total: r.total, added: r.added, reason: r.reason || null,
      hits: (r.hits || []).map((h) => ({ ref: h.reference, stem: h.stem, status: h.status })),
      misses: (r.misses || []).map((m) => m.reference),
      needImages: (r.needImages || []).map((m) => m.reference) };
  },
});