...
import { saveRecipe, listRecipes, activateRecipe, getActiveRecipe, currentPhase, recipeProgress, revertRecipe, recipeVersions, activeRulesSummary } from "@/lib/recipes";
...
规则:每个 phase 给 name、selector(覆盖哪些知识点:type=chapters时 value 填章节名/type=weak 表示薄弱点/type=all 全部)、method(用什么方式学)、exit(何时进下一阶段,默认 mastery_ge level=ok)。若用户说"AI按表现选"就用 method.type=ai_choose。goal 填一句话目标。忠于用户描述,别乱加阶段。★如果这段流程【模糊、缺关键信息、或前后矛盾】,以致你无法可靠地拆成阶段,就【不要硬拆】——把 phases 留空、在 clarify 里列出必须先问清主人的具体问题。` + langInstruction(ctx.user.lang),
      PHASE_SCHEMA
    );
