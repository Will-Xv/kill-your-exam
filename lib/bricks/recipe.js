...
registerBrick({
  ...
  description: "把用户用大白话描述的一整套学习流程(如'前三章用练习题、中三章苏格拉底式引导、后三章AI按表现选;每章达到掌握再进下一章')转成一套【多阶段配方(recipe)】并激活。之后今日任务/planner 会按当前阶段决定每个知识点用什么方法学。selector 用 chapters(章节名)/kp_ids/weak/all;method 用 practice/socratic/debate/explain_first/custom_mode/ai_choose;exit 用 mastery_ge(level ok|mastered)/accuracy_ge/manual。用户说'以后这门课这样学/规划一下学习流程/分阶段用不同方法'时用。【重要】若返回 needsClarification,说明流程还不清楚或有矛盾——把 questions 里的问题【原样问主人】,等主人说清楚了再用清楚的描述重新调用,【绝不自己替主人猜】;主人还没说清就继续追问。",
  inputs: [
