...
B. 若考生只是想【简单重排/换几个知识点/删掉自由练习】(没有顺序要求)→ steps 留空数组,改填:kpIds(1~4个,根因优先)、includeReview(默认true,说不复习就false)、freeTarget(默认10,说不练就0)。` + langInstruction(user.lang);
    ...
    try {
      out = await generateJson(prompt, { type: "object", properties: {
        steps: { type: "array", items: { type: "object", properties: {
          type: { type: "string", enum: ["practice", "debate", "socratic", "study", "review", "free"] },
          kpId: { type: "integer" },
          n: { type: "integer" },
        }, required: ["type"] } },
    ...
    if (steps.length) {
      ...
      for (const st of steps.slice(0, 12)) {
        ...
        const needsKp = ["practice", "debate", "socratic", "study"].includes(st.type);
        if (needsKp) {
          ...
          items.push(it);
          summaryTasks.push(`${st.type === "debate" ? "辩论" : st.type === "socratic" ? "苏格拉底" : st.type === "study" ? "学" : "练"}·${title}${st.n ? "×" + st.n : ""}`);
