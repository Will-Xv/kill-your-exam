...
registerBrick({
  ...
  description: "【纯数字微调】就地修改今天今日任务里的【题数/轮数】,不重挑知识点、不重排、不跑 AI——又快又稳。用于:『自由练习改成5题』『问答改成2题、辩论改成1轮』『把某步的题数改成3』这类【只改数量、别的都不动】的小请求。★这类小调参【优先用本工具】,不要用会重排整份计划的 customize_daily_plan(那个慢、且会重挑知识点)。freeCount=把自由练习设成几题(0=去掉自由练习);changes=改某些有序步骤的数量,每项 {match, n}:match 可填步骤类型(practice/debate/socratic/explore)或知识点标题里的关键词,n=新的题数/轮数。",
  inputs: [
  ...
  run: async (args, ctx) => {
    ...
    return { ok: true, note: `已微调今日任务(其它步骤不动):${notes.join("、")}`, tasks: out.map((i) => i.type === "kp" ? `学:${i.title}` : i.type === "review" ? "复习到期错题" : i.type === "free" ? `自由练习×${i.target}` : `${i.type}·${i.title || ""}${i.n ? "×" + i.n : ""}`) };
  },
});
