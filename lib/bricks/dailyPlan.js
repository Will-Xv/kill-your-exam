...
import { tweakRecipeCounts, saveRecipe } from "@/lib/recipes";
...
// 砖头:把【单个学习方法】一键固化——建单阶段配方并激活 + 立刻加进今天的今日任务。
// 场景:杀手在聊天里给主人做完一段苏格拉底引导/辩论/自由探索/带练后,主人同意「把这个方法列成每日任务」。
// 走这一个轻通道,避免为单个方法去跑重的多阶段 recipe_save,也避免"建了配方却忘了加到今天"这种只做一半。
const _PIN_METHOD_LABEL = { socratic: "苏格拉底引导", debate: "辩论", explore: "自由探索", practice: "练习题" };
registerBrick({
  name: "pin_method_as_daily", category: "planning", write: true,
  title: "把某个学习方法固化成每日任务(建配方+今天就加上)",
  description: "主人在被【苏格拉底引导/辩论/自由探索/练习】等互动学习中,同意『把这个方法列成每日任务』时用它——【一步到位·又快又轻】:①建一个【单阶段学习配方】并激活(以后这门课/所有考试按此方法学,日常规划自然带上);②同时把这一步【立刻加进今天的今日任务】(不用等明天)。method 取 socratic/debate/explore/practice;kpId=这次针对的具体知识点(在聊某个知识点就带上;不带=用当前最薄弱的点、配方按薄弱点走);scope=exam(默认·仅本考试)/global(所有考试长期通用),主人没说清就用 exam 并在回话里点一句「先只用在这门考试,想所有考试通用告诉我」。★这是给【单个方法】用的轻通道,【绝不要】为它去跑多阶段 recipe_save(那又慢又重),也别只建配方不加今天、或只加今天不建配方——本工具一次把两件事都做了。",
  inputs: [
    { key: "method", type: "string", required: true, desc: "socratic/debate/explore/practice" },
    { key: "kpId", type: "integer", required: false, desc: "针对的知识点 id(在讨论具体知识点就带上;不带=用当前最薄弱点)" },
    { key: "scope", type: "string", required: false, desc: "exam(默认,仅本考试)/global(所有考试长期通用)" },
    { key: "name", type: "string", required: false, desc: "配方名(可选;不填自动起名如『每日苏格拉底引导』)" },
  ],
  run: async (args, ctx) => {
    const user = ctx.user;
    const exam = ctx.exam || getActiveExam(user.id);
    if (!exam) return { ok: false, reason: "no_exam" };
    const method = ["socratic", "debate", "explore", "practice"].includes(String(args.method)) ? String(args.method) : null;
    if (!method) return { ok: false, reason: "bad_method", note: "method 需是 socratic/debate/explore/practice 之一。" };
    const scope = args.scope === "global" ? "global" : "exam";
    const label = _PIN_METHOD_LABEL[method];
    // 解析目标知识点:给了 id 用 id(且算"针对具体点");没给就取当前最薄弱/未学的一个(仅用于今天这一步+起名)。
    let mm = []; try { mm = masteryMatrix(exam.id); } catch {}
    let kp = args.kpId != null ? (mm.find((k) => Number(k.id) === Number(args.kpId)) || null) : null;
    const targeted = !!kp;
    if (!kp) kp = mm.find((k) => k.level === "weak") || mm.find((k) => k.level === "unlearned") || mm[0] || null;
    // ① 单阶段配方:针对具体知识点→selector=kp_ids;否则→薄弱点。
    const selector = targeted ? { type: "kp_ids", value: [String(kp.id)] } : { type: "weak" };
    const goal = `每天用【${label}】${targeted ? "攻克「" + kp.title + "」" : "攻克薄弱点"}`;
    const spec = { goal, phases: [{ id: "p1", name: label, selector, method: { type: method }, exit: { type: "manual" } }] };
    const name = (args.name && String(args.name).trim()) || `每日${label}`;
    let recipe = null;
    try { recipe = saveRecipe(user.id, exam.id, { name, description: goal, spec, scope, activate: true }); }
    catch (e) { return { ok: false, reason: "recipe_failed", note: "配方没建成:" + String((e && e.message) || e) }; }
    // ② 立刻加进今天的今日任务(socratic/debate/explore/practice 都需要一个具体 kp)。
    let addedToday = false;
    if (kp) {
      const { items } = currentDailyItems(user.id, exam);
      const out = (items || []).map((x) => ({ ...x }));
      const exists = out.some((x) => x.type === method && Number(x.kpId) === Number(kp.id));
      if (!exists) {
        _snapDaily(user, exam, `加每日${label}`);
        const step = { type: method, kpId: kp.id, title: kp.title };
        const revIdx = out.findIndex((x) => x.type === "review"); // 排在"复习"之后、其余之前,优先被看到
        if (revIdx >= 0) out.splice(revIdx + 1, 0, step); else out.unshift(step);
        const today = todayStr();
        db.prepare("DELETE FROM daily_plans WHERE exam_id=? AND date=?").run(exam.id, today);
        db.prepare("INSERT INTO daily_plans(exam_id,date,items_json,completed,custom) VALUES(?,?,?,0,1)").run(exam.id, today, JSON.stringify(out));
      }
    ...
    return {
      ok: true, recipeId: recipe && recipe.id, method, methodLabel: label,
      scope, scopeLabel: scope === "global" ? "所有考试通用" : "仅本考试",
      target: targeted ? kp.title : "薄弱点", addedToday,
      note: `已把【${label}】${targeted ? "对「" + kp.title + "」" : "(薄弱点)"}设成${scope === "global" ? "所有考试" : "这门考试"}的固定学法${addedToday ? ",并加进了今天的今日任务" : "(暂无可加的知识点,先进配方,明天起自然带上)"}。可用 recipe_revert 撤回、或去总规划页管理。` + (scope === "exam" ? "(先只用在这门考试;想所有考试都这样跟我说)" : ""),
    };
  },
});
