...
// 【收件箱文案要跟用户语言走】以前标题写死中文直接落库(发出去那一刻就是中文,前端再怎么 i18n 也救不回来),
// 英文账号的收件箱里于是全是"学习提醒"。这里在【发送时】按该用户的语言取译文。
function _t(userId, zh) {
  try { const u = db.prepare("SELECT lang FROM users WHERE id=?").get(userId); const d = u && u.lang ? DICTS[u.lang] : null; return (d && d[zh]) || zh; } catch { return zh; }
}
...
// 投递某用户所有到期未送的提醒:【收件箱留一份 + 应用外推送】。幂等。返回投递条数。
// ★推送【真的没发出去】(sent=0,比如主人还没允许通知/订阅失效)时【不标记已投递】,留着下次继续试——
//   等他开了通知就能补上。收件箱那份靠 key 幂等,重试不会刷屏。
// ★作废线:逾期超过 GIVE_UP_HOURS 小时的提醒就不再重试推送了(标已投递)——过了一天的提醒本来也没意义了,
//   而且能避免无限重试堆积;收件箱里那份一直都在,不会丢内容。
...
export async function deliverDue(userId) {
  ...
  for (const r of rows) {
    try { sendLetter(userId, { title: "⏰ " + _t(userId, "学习提醒"), body: r.text || _t(userId, "该学习啦"), key: "reminder-" + r.id }); } catch {}
    let sent = 0;
    try { const res = await pushUser(userId, { title: "⏰ Kill Your Exam", body: r.text || "", url: "/" }); sent = (res && res.sent) || 0; } catch {}
    const overdueHours = (() => { try { return (nowMs() - new Date(String(r.due_at).replace(" ", "T") + "Z").getTime()) / 3600000; } catch { return 0; } })();
    if (sent > 0 || overdueHours > GIVE_UP_HOURS) {
