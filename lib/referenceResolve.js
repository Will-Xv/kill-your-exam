...
async function readReferenceFromImages(exam, reference, user) {
  ...
  // 逐份资料准备「要发给模型看的图」:PDF 就【精确渲染引用里的那一页】(不发整本);图片直接发原图。
  const parts = [];
  for (const m of mats) {
    const buf = readMat(m.id); if (!buf) continue;
    const isPdf = m.kind === "pdf" || /pdf/i.test(m.mime || "");
    if (isPdf) {
      if (!parsed.pages.length) continue; // 没页码就别发整本 PDF(太大);交给文本路径
      for (const pg of parsed.pages.slice(0, 3)) {
        try { const png = await renderPdfPage(buf, Number(pg)); if (png) parts.push({ inlineData: { mimeType: "image/png", data: png.toString("base64") }, src: `${m.filename} p.${pg}` }); } catch {}
        if (parts.length >= 4) break;
      }
    } else {
      parts.push({ inlineData: { mimeType: m.mime || "image/jpeg", data: buf.toString("base64") }, src: m.filename });
    }
    if (parts.length >= 4) break;
  }
  ...
  let res; try {
    res = await generate(null, { contents: [{ role: "user", parts: [
      ...
      { text: `这是考生上传的教材页面(图片/扫描件)。请在其中找到「${reference}」(如 教材第X页第Y题 / p.42 ex.3)对应的那一道题,把【正好对应它的那一道题】提取出来(只取这一道,别带相邻题/页码标记)。stem 保持教材原文的语言与措辞、忠于原文,【不要翻译、不要改写】。
