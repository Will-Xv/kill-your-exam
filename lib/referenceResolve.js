  ...
  for (const m of mats) {
    ...
    if (isPdf) {
      ...
      if (parsed.pages.length) {
        // 前3页+本页+后3页(共7页窗口):题干/图可能跨页,或页码有偏差
        const win = new Set();
        for (const p of parsed.pages) { const pn = Number(p); if (pn) for (let d = -3; d <= 3; d++) { const q = pn + d; if (q >= 1) win.add(q); } }
        try { send = await extractPdfPages(buf, [...win].slice(0, 15)); } catch {}
      }
      if (!send && buf.length <= 14 * 1024 * 1024) send = buf;                                        // 没页码就发整份(限 14MB:base64 膨胀33%后≈18.6MB,<20MB 请求上限)
      if (!send) continue;                                                                            // 大 PDF 又没页码 → 跳过,交给文本路径
      parts.push({ inlineData: { mimeType: "application/pdf", data: Buffer.from(send).toString("base64") } });
    } else {
      if (buf.length > 14 * 1024 * 1024) continue; // 14MB:base64后≈18.6MB,留余量给 20MB 请求上限
      parts.push({ inlineData: { mimeType: m.mime || "image/jpeg", data: buf.toString("base64") } });
    }
