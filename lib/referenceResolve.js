...
async function readReferenceFromImages(exam, reference, user) {
  // 直接把原始图片 / PDF 发给 Gemini——它能原生读取 PDF(含扫描件/示意图,自己做 OCR 与看图),
  // 让它在其中找到引用对应的那道题。不需要本地渲染 PDF。
  ...
  for (const m of mats) {
    ...
    if (buf.length > 18 * 1024 * 1024) continue; // 太大就跳过,别撑爆一次调用
    parts.push({ inlineData: { mimeType: isPdf ? "application/pdf" : (m.mime || "image/jpeg"), data: buf.toString("base64") } });
