...
import { uploadMedia } from "@/lib/gemini";
...
- 【语言·重要】:stem 保持【教材原文的语言与措辞】,一字不改地忠于原文,【不要翻译、不要改写】(除非主人明确要求翻译)。
...
async function readReferenceFromImages(exam, reference, user) {
  // 把原始图片 / PDF 发给 Gemini(它能原生读 PDF、含扫描件/示意图)。
  // PDF 且引用带页码 → 只抽出那一页发过去(精确、再大的书也行);否则整份(≤18MB)。
  ...
  const mats = db.prepare(`SELECT m.id, m.mime, m.kind, m.filename FROM materials m
    ...
    AND (m.kind='image' OR m.kind='pdf' OR m.mime LIKE 'image/%' OR m.mime='application/pdf') ORDER BY m.id`).all().slice(0, 5);
  // 一律走 File API(见 CLAUDE.md):PDF 带页码就抽「窗口页」上传、否则整份(≤50MB);图片直接上传。
  const parts = [];
  for (const m of mats) {
    const buf = readMat(m.id); if (!buf) continue;
    const isPdf = m.kind === "pdf" || /pdf/i.test(m.mime || "");
    let send = buf, mime = m.mime || (isPdf ? "application/pdf" : "image/jpeg"), ext = isPdf ? "pdf" : "jpg";
    if (isPdf) {
      mime = "application/pdf";
      if (parsed.pages.length) {
        const win = new Set(); // 前3页+本页+后3页(共7页窗口):题干/图可能跨页,或页码有偏差
        for (const p of parsed.pages) { const pn = Number(p); if (pn) for (let d = -3; d <= 3; d++) { const q = pn + d; if (q >= 1) win.add(q); } }
        try { const ex = await extractPdfPages(buf, [...win].slice(0, 15)); if (ex) send = ex; } catch {}
      }
      if (send.length > 50 * 1024 * 1024) continue; // 太大又没页码可抽 → 交给文本路径
    } else if (buf.length > 50 * 1024 * 1024) { continue; }
    try { const up = await uploadMedia(Buffer.from(send), mime, ext); parts.push({ fileData: { fileUri: up.fileUri, mimeType: up.mimeType } }); } catch {}
  ...
  let res; try {
    res = await generate(null, { contents: [{ role: "user", parts: [
      ...parts,
      { text: `这是考生上传的教材页面(图片/扫描件,可能是引用页及其前后几页的上下文)。请在其中找到「${reference}」(如 教材第X页第Y题 / p.42 ex.3)对应的那一道题,把【正好对应它的那一道题】提取出来(只取这一道,别带相邻题/页码标记)。stem 保持教材原文的语言与措辞、忠于原文,【不要翻译、不要改写】。
...
找不到、或那处不是一道完整的题,ok=false。【绝不编造】。` + langInstruction(user.lang) }
  ...
  if (out && out.ok && out.stem && String(out.stem).trim().length >= 6) {
    return { ok: true, qtype: out.qtype || "short", stem: String(out.stem).trim(), answer: out.answer || "" };
  }
  return { ok: false };
}
...
export function getResolveBanner(userId, examId) {
  try {
    const raw = getSetting("resolve_banner:" + userId); if (!raw) return null;
    const b = JSON.parse(raw);
    if (examId != null && b && b.examId != null) {
      let scope = [Number(examId)]; try { scope = familyScope(examId); } catch {}
