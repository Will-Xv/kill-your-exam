...
export async function resolveReferenceList(user, exam, { text, materialId, markMust = false, ingest = true } = {}) {
  ...
  let _dbg = {};
  try { const c = db.prepare(`SELECT COUNT(*) n FROM chunks c JOIN materials m ON m.id=c.material_id WHERE c.exam_id IN ${scopeSql(familyScope(exam.id))} AND m.status='ready'`).get(); _dbg = { examId: exam.id, scopeChunks: c.n, refs: refs.map((r) => r.reference) }; } catch (e) { _dbg = { err: String(e) }; }
  return { total: refs.length, added: hits.filter((h) => h.questionId).length, hits, misses, needImages, _dbg };
}
