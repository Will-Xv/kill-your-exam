...
async function readReferenceFromImages(examId, reference, user) {
  const mats = db.prepare(`SELECT m.id, m.mime, m.kind, m.filename FROM materials m
    WHERE m.exam_id IN ${scopeSql(familyScope(examId))} AND m.status='ready' AND m.stored=1
  ...
  for (const m of mats) {
    ...
    let res; try {
      res = await generate(null, { contents: [{ role: "user", parts: [
        ...
        { text: `这是考生上传的教材(可能是扫描件/图片)。请在其中找到「${reference}」(如 教材第X页第Y题 / p.42 ex.3)对应的那一道题,把【题目原文】完整提取出来。
...
export async function resolveReferenceList(user, exam, { text, materialId, markMust = false, ingest = true } = {}) {
  ...
  for (const r of refs) {
    ...
    else if (loc.status === "found" && loc.matches && loc.matches[0] && loc.matches[0].snippet) {
      // 定位很有把握(页码+题号都命中)但 AI 没提炼出来:直接用定位到的教材原文片段当题干(简答,AI 判分),不丢这道真题。
      stem = String(loc.matches[0].snippet).replace(/^…|…$/g, "").trim();
    }
    if (!stem || stem.length < 6) {
      // 文本里没搞定 → 多模态兜底:直接读原始图片/扫描PDF(扫描教材也能读,不用用户自己拍)。
      const mm = await readReferenceFromImages(exam.id, reference, user);
