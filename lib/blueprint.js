...
export async function composeFromBlueprint(exam, user, bp, opts = {}) {
  ...
  if (closed) {
    ...
    if (remain > 0) {
      const pool = db.prepare(`SELECT * FROM questions WHERE exam_id=? AND origin='fixed' AND flagged=0 ${perf ? "AND qtype='perform'" : ""} AND id NOT IN (${[...seen, 0].join(",")}) ORDER BY RANDOM() LIMIT ?`).all(exam.id, remain);
      for (const q of pool) { picked.push(q); seen.add(q.id); }
    }
  } else {
    // 2b) 常规:按蓝图知识点抽题补足到目标题数(题库不够就即时生成)
