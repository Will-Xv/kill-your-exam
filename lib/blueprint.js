...
export async function generateBlueprint(exam, user, instructions = "") {
  ...
  const schema = { type: "object", properties: {
    totalMarks: { type: "integer" }, durationMin: { type: "integer" }, overview: { type: "string" },
...
- plan:一个数组,列出正式考试要覆盖的知识点以及各应出几道题 count——覆盖重要知识点、题量与重要性匹配,总题量控制在 15~30 之间。kpTitle 尽量用上面清单里的原文;若清单为空,用你判断的知识点名。
只做结构规划,不要写具体题目。`;
...
export async function composeFromBlueprint(exam, user, bp, opts = {}) {
  ...
  for (const item of (bp.plan || [])) {
    ...
    if (!kp) continue;
    const need = Math.max(1, Math.min(item.count || 1, 10));
