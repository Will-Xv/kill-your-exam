...
export async function composeFromBlueprint(exam, user, bp, opts = {}) {
  ...
  // 只出用户“没做过”的题(必考原题 must_include 除外,它们在上面已强制置顶)
  const notDone = "AND id NOT IN (SELECT question_id FROM attempts WHERE exam_id=" + Number(exam.id) + " AND question_id IS NOT NULL)";

  // 1) 必考原题(用户题库里标了 must_include 的):原样、置于卷首、每次必出
  const mustRows = db.prepare(`SELECT * FROM questions WHERE exam_id=? AND origin='fixed' AND must_include=1 AND flagged=0 ORDER BY id ASC`).all(exam.id);
  for (const q of mustRows) { if (!seen.has(q.id)) { picked.push(q); seen.add(q.id); } }

  const poolForKp = (kpId) => db.prepare(`SELECT * FROM questions WHERE exam_id=? AND kp_id=? AND flagged=0 ${perfClause} ${notDone} AND id NOT IN (${[...seen, 0].join(",")}) ORDER BY RANDOM()`).all(exam.id, kpId);
  const takeFromKp = async (kp, need) => {
    if (need <= 0) return;
    let pool = poolForKp(kp.id);
    if (pool.length < need && !opts.reuseOnly) {
      try { await generateQuestionsForKp(exam, kp, need - pool.length + 1, user.lang); }
  ...
  if (closed) {
    ...
    if ((target === 0 && picked.length === 0) || (target > 0 && picked.length < target)) {
      const leafKps = db.prepare(`SELECT * FROM knowledge_points WHERE exam_id=? AND parent_id IS NOT NULL ORDER BY RANDOM()`).all(exam.id);
  ...
  if (picked.length === 0) {
    const any = db.prepare(`SELECT * FROM questions WHERE exam_id=? AND flagged=0 ${perfClause} ${notDone} AND id NOT IN (${[...seen, 0].join(",")}) ORDER BY RANDOM() LIMIT ?`).all(exam.id, target > 0 ? target : 20);
    for (const q of any) { picked.push(q); seen.add(q.id); }
  }

  const kpCount = db.prepare("SELECT COUNT(*) n FROM knowledge_points WHERE exam_id=? AND parent_id IS NOT NULL").get(exam.id).n;
  const bankCount = db.prepare(`SELECT COUNT(*) n FROM questions WHERE exam_id=? AND flagged=0 ${perfClause}`).get(exam.id).n;
