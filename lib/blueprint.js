...
export const DEFAULT_MARKS = { single: 2, multi: 3, judge: 1, fill: 3, short: 10, perform: 20 };   // 供 mockFromMaterials 复用(别再各处重复定义)
...
export async function generateBlueprint(exam, user, instructions = "") {
  ...
  const realDur = Number(exam.duration_min) > 0 ? Math.round(Number(exam.duration_min)) : null; // 只认【记录在案的真实时长】,没有就让 AI 别编
...
- totalMarks 总分。
- durationMin 时长(分钟):${realDur ? `【这门考试真实的考试时长已经确认是 ${realDur} 分钟】,durationMin 就【必须原样填 ${realDur}】,一分钟都不许改,题量与总分都要按这个时长来配。` : `【真实考试时长目前【未知】】——【不要编一个时长】,durationMin 请填 0(表示未知)。★也【不要】拿一个假想时长去反推题量:题量要按【这门考试真实会考多少题】来定(有官方说明就用官方的,没有就按同类考试的真实题量),并在 sourceNote 里说明「真实考试时长未知,需要主人补充」。`}
...
只做结构规划,不要写具体题目。`;
  ...
  bp.durationMin = realDur || (Number(bp.durationMin) > 0 ? Number(bp.durationMin) : null); // 有真实时长就以它为准;AI 填 0/空 = 未知,不伪造
...
export async function composeFromBlueprint(exam, user, bp, opts = {}) {
  ...
  const mustRows = db.prepare(`SELECT * FROM questions WHERE exam_id IN ${scSql} AND origin IN ${USER_ORIGINS} AND must_include=1 AND flagged=0 ORDER BY id ASC`).all();
  for (const q of mustRows) { if (!seen.has(q.id)) { picked.push(q); seen.add(q.id); } }

  // 【模拟考只复用真题,别的现出】题库里攒的 AI 旧题是"以前随机出的",拿来凑一份模拟卷既不成体系也容易撞见做过的;
  // 只有真题(is_real)和主人指定的必考原题(must_include)值得反复出;其余一律现场按代表性重出。
  // ★从资料里抄进题库的原题【不进这个白名单】(Will 定):那些题是留给「做原题」直接做的,
  //   模拟考自己组卷时不该把它们混进来 —— 除非它本身就是历年真题(那时 is_real=1,上面第一个条件已经放行)。
  // freshFrom = 本次生成之前的最大题目 id,用来把"刚刚为这个知识点生成的新题"放行(否则新题因为不是真题会被自己过滤掉)。
  const poolForKp = (kpId, freshFrom = null) => db.prepare(`SELECT * FROM questions WHERE exam_id IN ${scSql} AND kp_id=? AND flagged=0 ${perfClause} ${notDone}
    AND id NOT IN (${[...seen, 0].join(",")})
    AND (is_real=1 OR must_include=1${freshFrom != null ? " OR id > " + Number(freshFrom) : ""})
    ORDER BY is_real DESC, RANDOM()`).all(kpId);
  const maxQid = () => db.prepare("SELECT COALESCE(MAX(id),0) m FROM questions").get().m;
  const takeFromKp = async (kp, need) => {
    ...
    if (pool.length < need && !opts.reuseOnly) {
      ...
      const before = maxQid();
      try { await generateQuestionsForKp(kpExam, kp, need - pool.length + 1, user.lang, { representative: true }); }
      catch (e) { genErrors.push(String((e && e.message) || e)); }
      pool = poolForKp(kp.id, before);
    }
  ...
  if (closed) {
    ...
    if (remain > 0) {
      const pool = db.prepare(`SELECT * FROM questions WHERE exam_id=? AND origin IN ${USER_ORIGINS} AND flagged=0 ${perfClause} ${notDone} AND id NOT IN (${[...seen, 0].join(",")}) ORDER BY RANDOM() LIMIT ?`).all(exam.id, remain);
  ...
  // 2d) 兜底②:仍为空(生成整个失败了)→ 先只找真题;真题也没有,才退而取任何已有的题。
  // 宁可给一份"抽自题库"的卷子,也不能给一张白卷 —— 但顺序上真题优先,AI 旧题是最后一招。
  if (picked.length === 0) {
    const lim = target > 0 ? target : 20;
    let any = db.prepare(`SELECT * FROM questions WHERE exam_id IN ${scSql} AND flagged=0 ${perfClause} ${notDone} AND is_real=1 AND id NOT IN (${[...seen, 0].join(",")}) ORDER BY RANDOM() LIMIT ?`).all(lim);
    if (!any.length) any = db.prepare(`SELECT * FROM questions WHERE exam_id IN ${scSql} AND flagged=0 ${perfClause} ${notDone} AND id NOT IN (${[...seen, 0].join(",")}) ORDER BY RANDOM() LIMIT ?`).all(lim);
