...
export async function composeFromBlueprint(exam, user, bp, opts = {}) {
  ...
  const poolForKp = (kpId) => db.prepare(`SELECT * FROM questions WHERE exam_id=? AND kp_id=? AND flagged=0 ${perfClause} AND id NOT IN (${[...seen, 0].join(",")}) ORDER BY RANDOM()`).all(exam.id, kpId);
  ...
  if (closed) {
    ...
    if (remain > 0) {
      const pool = db.prepare(`SELECT * FROM questions WHERE exam_id=? AND origin='fixed' AND flagged=0 ${perfClause} AND id NOT IN (${[...seen, 0].join(",")}) ORDER BY RANDOM() LIMIT ?`).all(exam.id, remain);
  ...
  if (picked.length === 0) {
    const any = db.prepare(`SELECT * FROM questions WHERE exam_id=? AND flagged=0 ${perfClause} AND id NOT IN (${[...seen, 0].join(",")}) ORDER BY RANDOM() LIMIT ?`).all(exam.id, target > 0 ? target : 20);
