...
export function extractFrames(inputPath, { fps = 5, height = 720, maxFrames = 200, q = 5 } = {}) {
  const dir = tmpDir();
  try {
    const r = spawnSync("ffmpeg", ["-nostdin", "-i", inputPath, "-vf", `fps=${fps},scale=-2:${height}`, "-q:v", String(q), "-y", path.join(dir, "f_%05d.jpg")], { timeout: 120000, maxBuffer: 1 << 28 });
...
export function probeDurationSec(inputBuffer) {
  const dir = tmpDir(); const inp = path.join(dir, "in");
  try {
    fs.writeFileSync(inp, inputBuffer);
    const r = spawnSync("ffprobe", ["-v", "error", "-show_entries", "format=duration", "-of", "default=noprint_wrappers=1:nokey=1", inp], { timeout: 30000 });
...
export function transcodeToMp4(inputBuffer) {
  const dir = tmpDir(); const inp = path.join(dir, "in"); const out = path.join(dir, "v.mp4");
  try {
    fs.writeFileSync(inp, inputBuffer);
    // 保留分辨率(最高 1280 宽,不放大、不下采样到低于 720p)、近无损画质(crf 20)、保留音轨
    const r = spawnSync("ffmpeg", ["-nostdin", "-i", inp, "-vf", "scale='min(1280,iw)':-2", "-c:v", "libx264", "-preset", "veryfast", "-crf", "26", "-pix_fmt", "yuv420p", "-c:a", "aac", "-b:a", "96k", "-movflags", "+faststart", "-y", out], { timeout: 240000, maxBuffer: 1 << 28 });
