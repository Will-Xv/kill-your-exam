...
【防泄题】同一组题内答案不得泄露、各题不要高度相似或反复考同一点。` + (lessons ? "\n\n【避免重蹈以下已知毛病】\n" + lessons : "") + langRule;
  ...
  for (const q of (out.questions || []).slice(0, count)) {
    ...
    if (q.qtype === "perform") {
      ...
      ins.run(exam.id, kp.id, "perform", JSON.stringify({ stem: q.stem, captureType: cap, mediaMaterialId, analyzeAudio: aa, countdownSec: p.countdownSec || 3, autoStopAfterMediaSec: p.autoStopAfterMediaSec || 7, maxDurationSec: 300, rubric: p.rubric || [], instructions: (p.instructions || "") + (autoMusic ? " 【配乐说明】所给音乐是系统自动附带的练习配乐,风格不一定完全贴合本考试,重点是跟上它的节奏与情绪表达;正式考试请用你自备的曲目,也可在「补充资料」上传替换。" : "") }), JSON.stringify({ rubric: p.rubric || [], notes: q.explanation || "" }), q.difficulty || 2, sourceType, refs);
      n++; continue;
    }
