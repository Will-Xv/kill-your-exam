...
export async function generateQuestionsForKp(exam, kp, count, lang) {
  ...
  const performBlock = `\n【表演题规格】qtype="perform"(考生录音/录像作答)需填:captureType(audio/video)、mediaMaterialId(要放的音频素材 id,可选:${audioList || "(暂无,填 0)"};没有填 0)、analyzeAudio(舞蹈/形体=music;声乐/台词/朗诵/演讲=recorded;两者=both)、countdownSec(一般3)、autoStopAfterMediaSec(音频结束后几秒自动停,一般7)、rubric(评分维度)、instructions(说明);stem 写命题(如"跟随所给音乐即兴舞蹈")。`;
  ...
  const directive = performOnly
    ? `【最高优先级 · 必须严格遵守】只出 qtype="perform" 的录音/录像作答题,一道笔试/选择/判断/填空/简答都不要出。${otherNote ? "\n考生原话:" + otherNote : ""}\n`
    : ((otherNote ? `【考生补充要求 · 优先遵守】${otherNote}\n` : "") + (perfExam ? `【本考试是艺术/表演/技能类:务必包含 qtype="perform" 录音/录像作答题;笔试题只用于确需的知识性内容。】\n` : ""));
  const genPrompt = performOnly
    ? directive + `为「${exam.name}」出 ${count} 道【表演任务题】(qtype 全部为 "perform"),围绕「${kp.title}」(章节:${chapter}),按真实考试规则设计。${performBlock}\n每题填全 perform 字段,stem 写命题,绝不输出笔试题。` + langRule
...
【防泄题】同一组题内答案不得泄露、各题不要高度相似或反复考同一点。` + (lessons ? "\n\n【避免重蹈以下已知毛病】\n" + lessons : "") + langRule;
  ...
  for (const q of (out.questions || []).slice(0, count)) {
    ...
    if (q.qtype === "perform") {
      ...
      ins.run(exam.id, kp.id, "perform", JSON.stringify({ stem: q.stem, captureType: cap, mediaMaterialId: p.mediaMaterialId || null, analyzeAudio: aa, countdownSec: p.countdownSec || 3, autoStopAfterMediaSec: p.autoStopAfterMediaSec || 7, maxDurationSec: 300, rubric: p.rubric || [], instructions: p.instructions || "" }), JSON.stringify({ rubric: p.rubric || [], notes: q.explanation || "" }), q.difficulty || 2, sourceType, refs);
      n++; continue;
    }
