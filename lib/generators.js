...
export async function buildKnowledgeTree(exam, lang) {
  ...
  const treePrompt = `根据考试档案${sampleText ? "和资料摘要" : ""}${materialParts(exam.id).length ? "以及附件里的图片/音频原件" : ""}生成「${exam.name}」的知识点树(章→知识点),每章3~10点,具体可学、可练。若有音频/图片资料,请把需要多模态练习的能力(如听力理解、看图分析)也纳入。【重要·只放能学能练的知识/技能】知识点树里【绝对不要】放考务/事务性内容——如报名/缴费/考试流程/机考纸笔切换/成绩单(如 eTRF)下载与申请、单科重考政策与申请流程、证书办理、考场规则/礼仪、备考时间线规划等;这些属于「考前准备」,不是知识点。只收录真正要学的知识与要练的技能(如听力题型技巧、阅读定位、写作结构、口语表达、词汇语法等)。\n档案:\n${dossier.slice(0, 8000)}\n${sampleText ? "资料摘要:\n" + sampleText : ""}` + langInstruction(lang);
...
export async function rebuildKnowledgeTree(exam, lang, mode = "keep") {
  ...
  const stats = await buildKnowledgeTree(exam, lang); // 删旧点、建新点、算覆盖
