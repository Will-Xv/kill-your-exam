...
export async function buildKnowledgeTree(exam, lang, opts = {}) {
  ...
  const targetN = budget ? Math.max(3, Math.min(40, Math.round(budget / 5))) : null; // 约每点 5 分钟,估算叶子点总数
  const sizeRule = budget
    ? `【篇幅要求·最重要】这是用户希望在约 ${budget} 分钟内复习完的【聚焦小测/任务】,不是一整门课。知识点树必须【精简】:章节只要 1~3 章;叶子知识点【总数】控制在约 ${targetN} 个(宁少勿多、不要超),只放这次真正会考/要复习的核心点,【绝不要】为了完整而堆砌或把大主题拆得太细。${opts.emphasis ? `本次侧重:${opts.emphasis};只围绕该侧重展开,其它一概不放。` : ""}${opts.notes ? `范围/说明:${opts.notes}。严格限定在这个范围内。` : ""}`
    : "每章3~10点";
