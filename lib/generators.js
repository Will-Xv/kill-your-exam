...
【防泄题】同一组题内答案不得泄露、各题不要高度相似或反复考同一点。` + (lessons ? "\n\n【避免重蹈以下已知毛病】\n" + lessons : "") + langRule;
  ...
  for (const q of (out.questions || []).slice(0, count)) {
    ...
    if (q.qtype === "perform") {
      ...
      ins.run(exam.id, kp.id, "perform", JSON.stringify({ stem: q.stem, captureType: cap, mediaMaterialId, analyzeAudio: aa, countdownSec: p.countdownSec || 3, autoStopAfterMediaSec: p.autoStopAfterMediaSec || 7, maxDurationSec: 300, rubric: p.rubric || [], instructions: p.instructions || "" }), JSON.stringify({ rubric: p.rubric || [], notes: q.explanation || "" }), q.difficulty || 2, sourceType, refs);
      n++; continue;
    }
