...
export async function buildKnowledgeTree(exam, lang, opts = {}) {
  ...
  const sizeRule = budget
    ? `【篇幅要求·最重要】这是用户希望在约 ${budget} 分钟内复习完的【聚焦小测/任务】,不是一整门课。知识点树必须【极精简】:最多 1~2 章;叶子知识点【总数】只要 ${targetN} 个左右(比如约一小时就只列 3~4 个最核心的点),【宁可少也绝不要多、绝不要超过 ${targetN + 1} 个】。只挑这次最该复习的核心点,把相关的小点合并成一个大点,【绝对不要】为了完整而罗列或把主题拆细。${opts.emphasis ? `本次侧重:${opts.emphasis};只围绕它,其它一概不放。` : ""}${opts.notes ? `范围/说明:${opts.notes}。严格限定在这个范围内。` : ""}`
    : "每章3~10点";
