...
import { retrieve, ragBlock } from "./rag";
...
export async function generateQuestionsForKp(exam, kp, count, lang) {
  ...
  const out = await generateJson(
    `为「${exam.name}」出 ${count} 道题,考察「${kp.title}」(章节:${chapter})。题型混合,客观题为主。
${hits.length ? "必须依据资料:\n" + ragBlock(hits) : "无资料支撑,只出保守的基本概念题,不编造具体数字。"}
single/multi给4选项,answer写字母(多选如AC);judge的answer写"对"或"错"(保持中文);fill写标准答案;short写评分要点。explanation解释原因。difficulty 1~3。资料语言与输出语言不同时术语可保留原文,其余不混语言。` + langInstruction(lang),
    qSchema);
