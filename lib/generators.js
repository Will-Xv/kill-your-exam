...
import { difficultyHint } from "./memory";
import { snapshot, integrityFix } from "./checkpoint";
import { generateJson, embed, cosine, langInstruction, examLangInstruction } from "./gemini";
import { retrieve, ragBlock, materialParts, mmOpts, hitMediaParts } from "./rag";
import { lazyLookup } from "./lazyLookup";
import { resolveExamLang } from "./examlang";
import { findAndStoreMusic, alignStemToMusic } from "./music";
import { AiError } from "./errors";
import { weakDimHint } from "./performDims";
...
export async function buildKnowledgeTree(exam, lang, opts = {}) {
  ...
  // 【重建也要保住小测的小规模·Will】建考试时传了 timeBudgetMin/durationMin 才会走"宁少勿多"(单章、几个点);
  // 但重建(build_knowledge_tree)只传 emphasis、不传时长 → 以前 budget=null → 按整门课"每章3~10点"铺 → 小测重建后暴涨到十几个点。
  // 兜底读考试自己存的 duration_min:限时小测才有值 → 重建仍是小规模;没记时长=不是小测,budget 仍为 null、正常铺开。
  const budget = opts.timeBudgetMin ? Number(opts.timeBudgetMin)
    : (opts.durationMin ? Number(opts.durationMin)
    : (exam && exam.duration_min ? Number(exam.duration_min) : null));
  ...
  const sizeRule = budget
    ...
    : "每章3~10点;【章节名必须是真实内容单元名(如\"多元函数最优化\"\"二重积分\"),绝不能用考试名或\"期中/期末/Final/Midterm/复习\"之类当章节名】";
  const treeParts = await materialParts(exam.id, { max: 60 }); // 建树把【全部】教材都喂给 Gemini(File API 多模态);不要因为封顶漏掉教材
  const treePrompt = `根据考试档案${sampleText ? "和资料摘要" : ""}${treeParts.length ? "以及附件里的图片/音频原件" : ""}生成「${exam.name}」的知识点树(章→知识点),${sizeRule},具体可学、可练。【知识点标题必须简短·最重要】每个 point 只写【简短主题名】(中文约4~14字 / 英文2~6词),不要写成整句、不要带冒号解释或长描述,只点出「考什么主题/什么能力」,细节留到讲解和出题环节。【标题里可以有公式】——但公式【必须用 $...$ 包裹】(如 $\\nabla f=0$、$f_{xx}f_{yy}-f_{xy}^2$),不要裸露反斜杠命令,否则前端显示成源码。若有音频/图片资料,请把需要多模态练习的能力(如听力理解、看图分析)也纳入。【重要·只放能学能练的知识/技能】知识点树里【绝对不要】放考务/事务性内容——如报名/缴费/考试流程/机考纸笔切换/成绩单(如 eTRF)下载与申请、单科重考政策与申请流程、证书办理、考场规则/礼仪、备考时间线规划等;这些属于「考前准备」,不是知识点。只收录真正要学的知识与要练的技能(如听力题型技巧、阅读定位、写作结构、口语表达、词汇语法等)。\n档案:\n${dossier.slice(0, 8000)}\n${sampleText ? "资料摘要:\n" + sampleText : ""}` + langInstruction(lang);
  const tree = await generateJson(treePrompt, schema, await mmOpts(exam.id, treePrompt, treeParts));   // 建树【确实要全部教材】:显式传 treeParts(mmOpts 已不再默认附)
...
// ★answer 必须 required:下面收题时 `if (!q.answer) continue` 硬性要求它,schema 不写的话
//   模型漏填就会把题【静默丢光】,表现成"没出成题"(与 questions/generate 同一个坑,一起修)。

// opts.representative:小测/模拟考用。这两个场景要的是"这个知识点最该考的那道题",
// 不是在知识点范围里随便找个角落出一道 —— 所以额外给一段命题要求(见下面 repBlock)。
export async function generateQuestionsForKp(exam, kp, count, lang, opts = {}) {
  ...
  // 【只挂检索命中的媒体】出题本来就先 retrieve 了相关片段(见上一行 hits);
  // 以前还无差别附最多 6 份整份原件 + mmOpts 再附 20 份 —— 检索白做、账单爆炸。
  // 现在:文字要点走 hits;只有当命中的资料本身是图片/音频(听力题、看图题真的需要原件)时,才挂【那一两份】。
  // 【懒加载补救】检索没命中时才多花一次便宜调用让模型自己挑资料;命中良好则零开销
  const rescue = await lazyLookup(exam.id, `${chapter} ${kp.title}`, hits, lang);
  ...
  let weakDims = ""; if (perfOn) { try { weakDims = weakDimHint(exam.id, { kpId: kp.id }); } catch {} }
  const weakDimBlock = weakDims ? `\n【自适应重点 · 按上次评分驱动】考生此前在这些维度偏弱:${weakDims}。本次【命题与评分维度(rubric)都要重点针对这些弱维度】设计,让考生这次就练到它们;别再泛泛评。\n` : "";
  const directive = perfOn
    ? `【最高优先级 · 必须严格遵守】只出 qtype="perform" 的录音/录像作答题,一道笔试/选择/判断/填空/简答/理论题都不要出。这是艺术/表演/技能类考试,考生要用录音或录像作答。${otherNote ? "\n考生原话:" + otherNote : ""}${weakDimBlock}\n`
    : (otherNote ? `【考生补充要求 · 优先遵守】${otherNote}\n` : "");
  const repBlock = opts.representative ? `
【必须出有代表性的题 · 这是小测/模拟考,不是随机练习】
- 出这个知识点【最该考的那道】:真实考试反复考的核心考法、最典型的题型与设问方式。
- 不要挑边角料:不要冷僻定义、不要资料里一笔带过的细节、不要纯记忆的年份/编号,也不要为了新鲜而故意刁钻。
- 有历年真题/样卷/考纲作参照时,照着它们的考法与难度出;没有就照这个学科最通行的考法出。
- 这道题要能真正区分"懂了"和"没懂"——做对说明掌握了这个知识点,做错能暴露具体在哪没懂。
` : "";
  const genPrompt = perfOn
    ? directive + `为「${exam.name}」出 ${count} 道【表演任务题】(qtype 全部为 "perform"),围绕「${kp.title}」(章节:${chapter}),按真实考试规则设计。\n【重要】如果「${kp.title}」其实是考务/规则/防作弊/录制要领之类的事务性知识点,请【忽略它】,改为围绕这门艺术考试真正要考的表演能力(命题表演/台词/声乐/形体/朗诵/即兴/演奏等)出题。${performBlock}\n每题填全 perform 字段,stem 写命题,绝不输出任何笔试/选择/判断/填空/简答题。` + repBlock + langRule
    : directive + `为「${exam.name}」出 ${count} 道题,考察「${kp.title}」(章节:${chapter})。题型按这门考试的性质来定。
${hits.length ? "必须依据资料:\n" + ragBlock(hits) : (rescue ? "" : "无资料支撑,只出保守的基本概念题,不编造具体数字。")}${rescue}
single/multi给4选项,answer写字母(多选如AC);judge写"对"或"错"(中文);fill写标准答案;short写评分要点;explanation必须写(每道题都要,选择题也要:说明正确项为什么对、迷惑项错在哪);difficulty 1~3。资料语言与输出语言不同时术语可保留原文,其余不混语言。${difficultyHint(exam.user_id, exam.id)}
...
【防泄题】同一组题内答案不得泄露、各题不要高度相似或反复考同一点。` + repBlock + (lessons ? "\n\n【避免重蹈以下已知毛病】\n" + lessons : "") + langRule;
  const out = await generateJson(genPrompt, qSchema, await mmOpts(exam.id, genPrompt, mparts));
...
export async function augmentKnowledgeTree(exam, lang) {
  ...
  const sampleText = sample.map((s) => `${s.heading_path}: ${s.c}`).join("\n").slice(0, 8000);
  const parts = await materialParts(exam.id, { max: 8 });
  ...
  let out; try { out = await generateJson(prompt, schema, await mmOpts(exam.id, prompt, parts)); } catch { return 0; }   // 补知识树:显式传 parts
...
// 后台重建:立即返回(把考试置为“重建中”,详情页/学习页会 gate 住半成品树),重建在后台跑完再清状态。
// 避免长重建把杀手这一轮请求拖到超时;失败时清状态并保留回档点供手动还原。
export function startRebuild(exam, user, mode = "keep", opts = {}) {
  try { db.prepare("UPDATE exams SET setup_state='generating', setup_progress=? WHERE id=?").run("重建知识点树…", exam.id); } catch {}
  rebuildKnowledgeTree(exam, user?.lang, mode, opts)
    .then((r) => { try { db.prepare("UPDATE exams SET setup_state=NULL, setup_progress=NULL WHERE id=?").run(exam.id); } catch {} return r; })
    .catch((e) => {
      try { db.prepare("UPDATE exams SET setup_state=NULL, setup_progress=NULL WHERE id=?").run(exam.id); } catch {}
      try { console.error("[rebuild] failed exam=" + exam.id, e?.message || e); } catch {}
    });
  return { status: "generating", examId: exam.id };
}

export async function rebuildKnowledgeTree(exam, lang, mode = "keep", opts = {}) {
  const m = ["keep", "summarize", "none"].includes(mode) ? mode : "keep";
  try { snapshot(exam.user_id, [exam.id], { op: "rebuild_tree", label: `重建「${exam.name}」知识点树(${m})`, runId: opts.runId || null }); } catch {}
  ...
  // 重建后知识点 id 全变了:清掉家族的今日任务缓存,否则首页今日任务还指向旧 kpId,点了在新树里找不到、只停在学习页。
  try { db.prepare(`DELETE FROM daily_plans WHERE exam_id IN ${scopeSql(familyScope(exam.id))}`).run(); } catch {}
