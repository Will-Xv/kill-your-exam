...
export async function buildKnowledgeTree(exam, lang, opts = {}) {
  ...
  const tree = await generateJson(treePrompt, schema, await mmOpts(exam.id, treePrompt));
...
export async function generateQuestionsForKp(exam, kp, count, lang) {
  ...
  const mparts = await materialParts(exam.id, { max: 6 });
...
【防泄题】同一组题内答案不得泄露、各题不要高度相似或反复考同一点。` + (lessons ? "\n\n【避免重蹈以下已知毛病】\n" + lessons : "") + langRule;
  const out = await generateJson(genPrompt, qSchema, await mmOpts(exam.id, genPrompt));
...
export async function augmentKnowledgeTree(exam, lang) {
  ...
  let out; try { out = await generateJson(prompt, schema, await mmOpts(exam.id, prompt)); } catch { return 0; }
