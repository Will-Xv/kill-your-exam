...
export async function buildKnowledgeTree(exam, lang, opts = {}) {
  ...
  // 小测(有时间预算):不分章——把所有章节的点合并成一个扁平清单(只合并、不删点)。
  if (budget && Array.isArray(tree.chapters) && tree.chapters.length) {
    const allPoints = tree.chapters.flatMap((ch) => Array.isArray(ch.points) ? ch.points : []);
    tree.chapters = [{ title: exam.name, points: allPoints }];
  }
