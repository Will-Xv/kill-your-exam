...
import { currentUserId } from "./reqctx";   // token 用量归属到当前请求的用户
...
export async function generate(prompt, opts = {}) {
  ...
  return withRetry(async () => {
    ...
    try { recordUsage(model, res && res.usageMetadata); } catch {}
    return res;
  }, opts.tries);
}

// 【Token 用量落库】按 用户×日期×模型 累加,管理面板据此看每个人烧了多少。
// ★thoughts(思考/reasoning token)必须单独记:它【不包含在 candidatesTokenCount 里】却照样计费——
//   只统计输入/输出会严重低估用量(开了 thinking 的模型尤其明显)。
// ★cached(命中缓存的输入)是 prompt 的【子集】,不是另外加的量——算钱时要拆成
//   (prompt - cached) 走全价 + cached 走折扣价,所以必须单独存、单独显示。
// ★tool_use 是工具调用产生的输入,也计费。total 直接用官方 totalTokenCount。
// 拿不到请求用户(后台入库/判题/cron)时记到 user_id=0 的「后台/系统」账上,不硬塞给某个人。
export function recordUsage(model, u) {
  if (!u) return;
  const prompt = u.promptTokenCount || 0;
  const cached = u.cachedContentTokenCount || 0;
  const thoughts = u.thoughtsTokenCount || 0;          // 思考 token
  const output = u.candidatesTokenCount || 0;
  const toolUse = u.toolUsePromptTokenCount || 0;   // 工具调用产生的输入 token(也计费)
  const total = u.totalTokenCount || (prompt + output + thoughts + toolUse);
  const hit = prompt ? Math.round((cached / prompt) * 100) : 0;
  console.log(`[TOKENS] model=${model} prompt=${prompt} cached=${cached}(${hit}%) thoughts=${thoughts} output=${output} tool=${toolUse} total=${total}`);
  try {
    const uid = Number(currentUserId() || 0) || 0;
    const day = new Date().toISOString().slice(0, 10);
    db.prepare(`INSERT INTO token_usage(user_id,day,model,calls,prompt,cached,thoughts,output,tool_use,total,updated_at)
      VALUES(?,?,?,1,?,?,?,?,?,?,datetime('now'))
      ON CONFLICT(user_id,day,model) DO UPDATE SET
        calls=calls+1, prompt=prompt+excluded.prompt, cached=cached+excluded.cached,
        thoughts=thoughts+excluded.thoughts, output=output+excluded.output, tool_use=tool_use+excluded.tool_use,
        total=total+excluded.total, updated_at=datetime('now')`).run(uid, day, String(model || ""), prompt, cached, thoughts, output, toolUse, total);
  } catch {}
}
...
export function handSliceNote(attachments) {
  ...
  // 四处用到它的场景【全都与成绩相关】(练习判卷、模考判卷、竞技场、以及追问——追问结束的 finalize
  // 会据讨论修订判分/掌握度),所以一律用计分口径,不再分场景。
  return "【关于手写/草稿图片附件·先看这条】第1张是整页手写的【完整图】,后面几张是【同一页从上到下切开的切片】。★两者【内容完全一样】,只是同一份内容的两种呈现(整页看得全、切片看得清),【不是两份,切片里也没有整页之外的新内容】。因此:①【默认只读一遍就够】——优先看切片(字迹更清晰),按从上到下的顺序接起来就是完整内容;整页不必再逐字重看一遍。②【不要把同一段内容识别两遍、也不要重复计分】,更不要因为同一句话出现两次就以为考生写了两遍。③只有当【切片里某处看不清】,或需要【确认某段写在整页的什么位置、和哪一步对应】时,才回头看整页核对那一处——不必整体交叉比对。④相邻切片之间可能有【重叠】,重叠处是同一段内容,同样只算一次。";
}

// 【追问里附了草稿纸】草稿是演算/思考过程,不是最终作答——不说清楚,模型会把草稿当正式答案来评对错。
export function draftAttachNote(attachments) {
  if (!(attachments || []).some((a) => /^draft(-p\d+of\d+)?\./.test((a && a.name) || ""))) return "";
  return "【附件里有他的草稿纸】那是考生在草稿纸上的【演算/思考过程】,【不是他提交的正式作答】。请用它去看懂他【是怎么想的】——哪一步开始跑偏、哪里算错、卡在哪个概念上,据此针对性地回应他的追问;【不要把草稿当成正式答案去判对错、也不要因为草稿写得潦草、算到一半或没写完就说他答错了】。★这一条尤其要紧:这段讨论结束后【可能据此修订这道题的判分和掌握度】,所以改分只能依据他【正式提交的作答】以及讨论中【事实层面确认】的对错,【绝不能因为草稿上的涂改、试错、写歪的中间步骤而扣他的分】——草稿本来就是用来试错的。";
}
