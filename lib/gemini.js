...
export function handSliceNote(attachments) {
  if (!(attachments || []).some((a) => /^handwriting-p/.test((a && a.name) || ""))) return "";
  return "【关于手写附件·先看这条】第1张是整页手写的【完整图】,后面几张是【同一页从上到下切开的切片】。★两者【内容完全一样】,只是同一份作答的两种呈现(整页看得全、切片看得清),【不是两份答案,切片里也没有整页之外的新内容】。因此:①【默认只读一遍就够】——优先看切片(字迹更清晰),按从上到下的顺序接起来就是完整作答;整页不必再逐字重看一遍。②【不要把同一段内容识别两遍、不要重复计分】,更不要因为同一句话出现两次就以为考生写了两遍。③只有当【切片里某处看不清】,或需要【确认某段写在整页的什么位置、跟哪道题/哪一步对应】时,才回头看整页核对那一处——不必整体交叉比对。④相邻切片之间可能有【重叠】,重叠处是同一段内容,同样只算一次。";
}
