...
export function handSliceNote(attachments, opts = {}) {
  if (!(attachments || []).some((a) => /-p\d+of\d+\./.test((a && a.name) || ""))) return "";   // handwriting-p1of3.png / draft-p1of3.png
  // scoring=false 用于【追问/讨论】这类不打分的场景——那里说"别重复计分"是驴唇不对马嘴。
  const scoring = opts.scoring !== false;
  return "【关于手写/草稿图片附件·先看这条】第1张是整页手写的【完整图】,后面几张是【同一页从上到下切开的切片】。★两者【内容完全一样】,只是同一份内容的两种呈现(整页看得全、切片看得清),【不是两份,切片里也没有整页之外的新内容】。因此:①【默认只读一遍就够】——优先看切片(字迹更清晰),按从上到下的顺序接起来就是完整内容;整页不必再逐字重看一遍。②【不要把同一段内容识别两遍】"
    + (scoring ? ",【也不要重复计分】,更不要因为同一句话出现两次就以为考生写了两遍" : ",更不要因为同一句话出现两次就以为他说了两遍")
    + "。③只有当【切片里某处看不清】,或需要【确认某段写在整页的什么位置、和哪一步对应】时,才回头看整页核对那一处——不必整体交叉比对。④相邻切片之间可能有【重叠】,重叠处是同一段内容"
    + (scoring ? ",同样只算一次。" : ",当成一处看就行。");
}
...
export function draftAttachNote(attachments) {
  ...
  return "【附件里有他的草稿纸】那是考生在草稿纸上的【演算/思考过程】,【不是他提交的正式作答】。请用它去看懂他【是怎么想的】——哪一步开始跑偏、哪里算错、卡在哪个概念上,据此针对性地回应他的追问;【不要把草稿当成正式答案去判对错、也不要因为草稿写得潦草或没写完就说他答错了】。";
}
