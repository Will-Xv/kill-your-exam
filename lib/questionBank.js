...
// 用户提供的题库:凡是【来自主人自己的题目文件/文本】的原题都算,可标记 must_include(每次必出)。
// 配合 exams.closed_bank(封闭题库):开启后练习和模拟都只从这些题出。
//
// 【2026-08 修】以前这里只认 origin='fixed'(也就是只认在这个面板里手动粘贴的那条路),
// 于是「上传做题」抄出来的题(origin='upload')明明是主人自己卷子上的原题,题库却一直显示 0 道 —— Will 反馈。
// 现在三条产出原题的路一视同仁:
//   fixed  = 在题库面板手动粘贴/添加,或从资料里「做原题」摘出来的
//   upload = 「上传做题」从上传的卷子里抄出来的
// (AI 生成的 generated/online 不算原题,永远不进这个面板。)
export const USER_ORIGINS = "('fixed','upload')";
...
export function bankList(examId) {
  const rows = db.prepare(`SELECT * FROM questions WHERE exam_id=? AND origin IN ${USER_ORIGINS} AND flagged=0 ORDER BY must_include DESC, id ASC`).all(examId);
  return rows.map((q) => {
    let body = {}, ans = {};
    try { body = JSON.parse(q.body); } catch {}
    try { ans = JSON.parse(q.answer); } catch {}
    // 来源:哪份文件来的。有 filename 就报文件名(做原题选文件时能对上号);手动录入的没有文件。
    let src = null;
    try { const r = JSON.parse(q.source_refs || "[]"); const f = (r[0] && r[0].filename) || ""; if (f) src = { via: q.origin === "upload" ? "upload" : "material", filename: f, materialId: (r[0] && r[0].material_id) || null }; } catch {}
    if (!src) src = { via: q.origin === "upload" ? "upload" : "manual", filename: "", materialId: null };
    return { id: q.id, qtype: q.qtype, stem: body.stem || "", options: body.options || [], answer: ans.answer || "", explanation: ans.explanation || "", must: !!q.must_include, origin: q.origin, src };
  });
}
...
export function bankSetMust(examId, id, on) {
  db.prepare(`UPDATE questions SET must_include=? WHERE id=? AND exam_id=? AND origin IN ${USER_ORIGINS}`).run(on ? 1 : 0, Number(id), examId);
}
export function bankDelete(examId, id) {
  db.prepare(`DELETE FROM questions WHERE id=? AND exam_id=? AND origin IN ${USER_ORIGINS}`).run(Number(id), examId);
}
