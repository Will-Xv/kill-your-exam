...
const MAX_READ = 2;           // 最多真去读几份资料(防止一次请求读一堆)
...
export async function lazyLookup(examId, topic, hits, lang) {
  try {
    ...
    const pick = await generateText(
      ...
      `请判断【哪几条最可能真正包含这个主题的内容】,只回复编号、用逗号隔开、最多 3 个;\n` +
      `如果看下来没有一条跟这个主题相关,就只回复 none。不要解释、不要输出别的字。\n\n${list}`,
      { timeoutMs: 60000 });
    const idx = String(pick || "").match(/\d+/g) || [];
    if (!idx.length || /none/i.test(String(pick || ""))) return "";
    const chosen = [...new Set(idx.map((x) => rows[Number(x) - 1]).filter(Boolean))].slice(0, MAX_READ + 1);
    if (!chosen.length) return "";
    const out = [];
    let reads = 0;
    for (const c of chosen) {
      // PDF 且索引里记了页段 → 真去抽那几页读(细节全、又只读几页);其它 → 直接用索引里的要点
      if (c.kind === "pdf" && /^p:\d+-\d+$/.test(String(c.heading_path || "")) && reads < MAX_READ) {
