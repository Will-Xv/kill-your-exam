...
function descendants(rootId) {
  ...
  const walk = (pid, depth) => {
    if (depth > 20) return;
    const kids = db.prepare("SELECT id,name,status,exam_date FROM exams WHERE parent_exam_id=? AND deleted_at IS NULL ORDER BY id").all(pid);
    for (const k of kids) { if (seen.has(k.id)) continue; seen.add(k.id); out.push({ id: k.id, name: k.name, status: k.status, exam_date: k.exam_date || null, depth }); walk(k.id, depth + 1); }
  };
  walk(rootId, 0);
  return out;
}
