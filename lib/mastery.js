...
export function masteryMatrix(examId) {
  ...
  try {
    const ins = db.prepare(`SELECT kp_id, kind, created_at FROM insights
      WHERE exam_id=? AND kp_id IS NOT NULL AND kind IN ('understanding','gap')`).all(examId);
  ...
  return kps.map((kp) => {
    ...
    for (const it of insArr) {
      ...
      wSum += w; wCorrect += w * (it.kind === "understanding" ? 1 : 0);
    }
    const acc = wSum ? wCorrect / wSum : 0;
    const evidence = arr.length + insArr.length;
    let level = "unlearned";
...
export function recordCrossKp(examId, questionId, crossKp, skipKpId) {
  ...
  for (const c of crossKp) {
    ...
    if (c.kind !== "understanding" && c.kind !== "gap") continue;
    seen.add(kid);
    const text = (String(c.insight || "").trim() || (c.kind === "understanding" ? "在其它题的作答/讨论中体现出对该知识点的理解" : "在其它题的作答/讨论中暴露出对该知识点的薄弱")).slice(0, 300);
    try {
