import db, { examScope, ownScope, scopeSql, familyScope } from "./db";
...
// 一次作答对掌握度的【有效正确度】0~1:基础=对/错;粗心错≈0.6、猜对≈0.5;再叠加自定义标签影响(down/up 各 ±0.4)。
export function attemptVal(a) {
  let val = a.correct ? 1 : 0;
  if (a.tag === "careless" && !a.correct) val = 0.6;
  else if (a.tag === "guessed" && a.correct) val = 0.5;
  try { let adj = 0; for (const l of (JSON.parse(a.labels || "[]") || [])) { if (l && l.effect === "down") adj -= 0.4; else if (l && l.effect === "up") adj += 0.4; } if (adj) val = Math.max(0, Math.min(1, val + adj)); } catch {}
  return val;
}
export function masteryMatrix(examId) {
  ...
  const rows = db.prepare(`SELECT kp_id, correct, created_at, tag, labels FROM attempts
  ...
  return kps.map((kp) => {
    ...
    for (const a of arr) {
      ...
      wSum += w; wCorrect += w * attemptVal(a);
    }
    ...
    return { ...kp, attempts: arr.length, insights: insArr.length, accuracy: Math.round(acc * 100), level, rootCause: !!kp.root_cause };
  });
}

// 单个知识点的当前掌握度等级(与 masteryMatrix 同一套规则,供“做题→记忆”即时判定)
export function kpMasteryLevel(kpId) {
  const arr = db.prepare("SELECT correct, created_at, tag, labels FROM attempts WHERE kp_id=? AND mode!='resolved'").all(Number(kpId));
  let wSum = 0, wCorrect = 0;
  for (const a of arr) {
    const days = (Date.now() - new Date((a.created_at || "").replace(" ", "T") + "Z").getTime()) / 86400000;
    const w = Math.exp(-Math.max(0, days) / 14);
    wSum += w; wCorrect += w * attemptVal(a);
  }
  let insCount = 0;
...
export function updateReviewQueue(questionId, correct) {
  ...
  if (!correct) {
    if (row) db.prepare("UPDATE review_queue SET interval_level=0, due_date=date(?, '+1 day') WHERE id=?").run(todayStr(), row.id);
    else db.prepare("INSERT INTO review_queue(question_id, due_date, interval_level) VALUES(?, date(?, '+1 day'), 0)").run(questionId, todayStr());
    return;
  }
  if (!row) return;
  const lvl = row.interval_level + 1;
  if (lvl >= INTERVALS.length) db.prepare("DELETE FROM review_queue WHERE id=?").run(row.id);
  else db.prepare(`UPDATE review_queue SET interval_level=?, due_date=date(?, '+${INTERVALS[lvl]} day') WHERE id=?`).run(lvl, todayStr(), row.id);
}
...
export function dueReviewCount(examId) {
  // 【与 /api/review 对齐:复习按本考试范围(ownScope),不串家族其他考试】
  // 以前这里用 examScope(=家族)→ 子考试今日任务显示的到期错题数是【整个家族】的,
  // 但点进去做复习(/api/review)只有本考试的 → 数字和实际对不上(Will 实测)。CLAUDE 已记'review/mock 改用 ownScope'。
  return db.prepare(`SELECT COUNT(*) n FROM review_queue rq JOIN questions q ON q.id=rq.question_id
    WHERE q.exam_id IN ${scopeSql(ownScope(examId))} AND q.flagged=0 AND rq.due_date <= ?`).get(todayStr()).n;
}
...
export function examSummary(examId) {
  ...
  return {
    ...
    kpTotal: m.length,
    weak, mastered,
    weakKps: m.filter((k) => k.level === "weak").slice(0, 6).map((k) => ({ id: k.id, title: k.title, rootCause: !!k.rootCause })),
    rootCauseKps: m.filter((k) => k.rootCause).slice(0, 6).map((k) => ({ id: k.id, title: k.title, level: k.level })),
    // 全部叶子知识点标题(用于跨考试重叠检测)
...
export function recordCrossKp(examId, questionId, crossKp, skipKpId) {
  ...
  // 家族级校验:知识点可能属于家族里的子/母考试(竞技场/交叉KP素材是家族范围取的),不能只认当前考试自己的点,否则信号被静默丢弃、掌握度不动。
  const rows = db.prepare(`SELECT id, title, exam_id FROM knowledge_points WHERE exam_id IN ${scopeSql(examScope(examId))}`).all();
  const valid = new Map(rows.map((r) => [r.id, r]));
  const applied = [];
  const seen = new Set();
  for (const c of crossKp) {
    const kid = Number(c.kpId);
    const kp = valid.get(kid);
    if (!kp || kid === Number(skipKpId) || seen.has(kid)) continue;
    ...
    const qid = kp.exam_id === Number(examId) ? (questionId || null) : null; // 洞察挂到该知识点【自己所属考试】名下,保引用一致
    try {
      db.prepare("INSERT INTO insights(exam_id,kp_id,question_id,kind,text) VALUES(?,?,?,?,?)").run(kp.exam_id, kid, qid, c.kind, text);
      applied.push({ kpId: kid, title: kp.title, kind: c.kind });
    } catch {}
  }
  return applied;
}

// 真子考试完成后:把它【自己知识点】的掌握度【语义映射】到家族里其它考试(母考试+同家族兄弟)的对应知识点。
// 只写 insights(与 recordCrossKp 一致):已掌握/一般→understanding(点绿),薄弱→gap;未学的不映射。阈值 0.6(跨考试从严,避免误映射)。
export async function mapSubExamMasteryToFamily(subExamId) {
  const sub = db.prepare("SELECT id, name, parent_exam_id FROM exams WHERE id=?").get(Number(subExamId));
  if (!sub || !sub.parent_exam_id) return { ok: false, reason: "not_subexam" };
  // masteryMatrix 是家族聚合——过滤出【子考试自己】的叶子知识点及其档位
  const mm = masteryMatrix(subExamId).filter((k) => Number(k.exam_id) === Number(subExamId) && k.parent_id);
  const src = mm.filter((k) => k.level === "mastered" || k.level === "ok" || k.level === "weak");
  if (!src.length) return { ok: true, mapped: 0, note: "这门子考试还没有可映射的掌握度" };
  const famSql = scopeSql(familyScope(subExamId));
  const targets = db.prepare(`SELECT kp.id, kp.title, kp.exam_id, ch.title chapter FROM knowledge_points kp LEFT JOIN knowledge_points ch ON ch.id=kp.parent_id WHERE kp.exam_id IN ${famSql} AND kp.parent_id IS NOT NULL AND kp.exam_id != ?`).all(Number(subExamId));
  if (!targets.length) return { ok: true, mapped: 0, note: "家族里没有别的考试的知识点可映射" };
  let srcVecs, tgtVecs;
  try {
    srcVecs = await embed(src.map((k) => `${k.chapter ? k.chapter + " / " : ""}${k.title}`));
    tgtVecs = await embed(targets.map((k) => `${k.chapter ? k.chapter + " / " : ""}${k.title}`));
  } catch { return { ok: false, reason: "embed_failed" }; }
  const cross = [];
  src.forEach((sk, i) => {
    let best = -1, bj = -1;
    tgtVecs.forEach((tv, j) => { const sc = cosine(srcVecs[i], tv); if (sc > best) { best = sc; bj = j; } });
    if (bj >= 0 && best >= 0.6) {
      const tk = targets[bj];
      cross.push({ kpId: tk.id, kind: sk.level === "weak" ? "gap" : "understanding", insight: `子考试「${sub.name}」完成:该点在那门里${sk.level === "weak" ? "仍薄弱" : "已掌握"}` });
    }
  });
  if (!cross.length) return { ok: true, mapped: 0, note: "没有语义足够接近的对应知识点" };
  const applied = recordCrossKp(subExamId, null, cross, null);
  return { ok: true, mapped: applied.length, kps: applied.map((a) => a.title) };
}
...
function _buildKnowledgeState(examId) {
  ...
  const weak = m.filter((k) => k.level === "weak").map((k) => k.title);
  const _sc = scopeSql(ownScope(examId));   // 复习范围=本考试,与 dueReviewCount / /api/review 一致
  let dueTitles = [];
  try {
    dueTitles = db.prepare(`SELECT DISTINCT kp.title FROM review_queue rq JOIN questions q ON q.id=rq.question_id
      ...
      WHERE q.exam_id IN ${_sc} AND q.flagged=0 AND rq.due_date <= ? AND kp.title IS NOT NULL LIMIT 12`).all(todayStr()).map((r) => r.title);
  } catch {}
