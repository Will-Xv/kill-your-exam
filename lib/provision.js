...
import { autoAdjustExamUi } from "@/lib/uiPlacement";

// 立即返回:把考试置为“生成中”,后台跑 runProvision(不 await)。
export function startProvision(exam, user, opts = {}) {
  db.prepare("UPDATE exams SET setup_state='generating', setup_progress=? WHERE id=?").run("准备中…", exam.id);
...
async function buildDossier(exam, user, opts) {
  ...
  // 【强制读主人在资料页手写的说明】这段以前从没进过建考试链路:它存在 exam.checklist 的 {kind:"qa",answer:正文} 里,
  // 而 buildDossier 只用 opts.* + 联网搜索 + 父考试档案 → 主人在资料页写了一整份大纲,建出来的知识树却是通用起点。
  // 上传的文件本来就会被 buildKnowledgeTree 的 materialParts/chunks 读到,缺的只有这个自由文本框,这里补上。
  try {
    const cl = JSON.parse(exam.checklist || "[]");
    const filled = cl.filter((c) => c && c.answer && String(c.answer).trim())
      .map((c) => `${c.item}:${String(c.answer).trim().slice(0, 1200)}`);
    if (filled.length) lines.push(`主人在资料页手写的说明(务必按它限定范围/重点,别给通用起点):\n${filled.join("\n")}`);
  } catch {}
...
const setProg = (examId, txt) => { try { db.prepare("UPDATE exams SET setup_progress=? WHERE id=?").run(txt, examId); } catch {} };
async function runProvision(examId, user, opts) {
  ...
  if (role === "mother") {
    ...
    setProg(examId, "规划考试蓝图…");
    try { const bp = await generateBlueprint(exam, user, opts.notes || ""); saveBlueprint(exam.id, bp); } catch {}
    if (opts.timeBudgetMin) { setProg(examId, "制定复习计划…"); try { await writeReviewPlan(exam, user, opts); } catch {} }
  }

  setProg(examId, "按内容整理栏目…");
  try { await autoAdjustExamUi(exam, user); } catch {}   // 建考试时按内容智能删/凸显栏目(失败不阻塞)
  db.prepare("UPDATE exams SET setup_state=NULL, setup_progress=NULL, assess_status='done' WHERE id=?").run(examId);
}
