...
function buildSeed(exam, scope) {
  ...
  if (scope === "wrong") {
    const rows = db.prepare(
      `SELECT q.body, q.answer FROM attempts a JOIN questions q ON q.id=a.question_id
...
- 每一回合你的回复分两部分:先是【叙事/角色台词】,最后【单独一行】输出机器状态,格式严格为:@@STATE {"meter":数字,"done":true或false,"win":true或false}。这一行考生看不到,别加别的解释。
