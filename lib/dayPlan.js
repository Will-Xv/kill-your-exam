...
export function planDays(userId, { title, units, startDate, perDay } = {}) {
  ...
  const items = list.map((u, i) => ({
    ...
    href: u.href ? String(u.href).slice(0, 200) : null,
...
// 【按绝对日期合并进排期】用于 syllabus:传入带 date(YYYY-MM-DD)的有序 items,换算成相对 startDate 的 day 偏移(过期的 day 为负→视图里进"现在该做·含顺延")后【追加】到现有排期(没有就新建,startDate=今天)。replace=true 则整体替换。
function _between(a, b) { const p = (x) => { const [y, m, d] = String(x).split("-").map(Number); return new Date(y, m - 1, d).getTime(); }; return Math.round((p(b) - p(a)) / 86400000); }
export function addDayPlanItems(userId, { title, items, replace } = {}) {
  const today = todayStr();
  let plan = getDayPlan(userId);
  if (replace || !plan) plan = { title: (title || (plan && plan.title) || "学习排期"), startDate: today, perDay: (plan && plan.perDay) || 1, items: [], createdAt: Date.now() };
  const start = plan.startDate;
  let seq = plan.items.reduce((m, x) => Math.max(m, x.seq), -1);
  const list = (Array.isArray(items) ? items : []).map((u) => (typeof u === "string" ? { title: u } : u)).filter((u) => u && u.title != null);
  for (const u of list) {
    const date = /^\d{4}-\d{2}-\d{2}$/.test(u.date || "") ? u.date : today;
    plan.items.push({ seq: ++seq, day: _between(start, date), title: String(u.title).slice(0, 240), examId: u.examId != null ? Number(u.examId) : null, taskId: u.taskId != null ? Number(u.taskId) : null, href: u.href ? String(u.href).slice(0, 200) : null, done: false, doneAt: null });
  }
  save(userId, plan);
  return plan;
}
