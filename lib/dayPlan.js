...
export function addDayPlanItems(userId, { title, items, replace } = {}) {
  ...
  for (const u of list) {
    ...
    plan.items.push({ seq: ++seq, day: _between(start, date), title: String(u.title).slice(0, 240), examId: u.examId != null ? Number(u.examId) : null, taskId: u.taskId != null ? Number(u.taskId) : null, done: false, doneAt: null });
  }
  save(userId, plan);
  return plan;
}
