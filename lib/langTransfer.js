...
${items}` + langInstruction(user.lang),
  ...
  for (const it of (out.items || [])) {
    ...
    const ch = insT.run(exam.id, r.kp_id || null, r.attempt_id, it.source, it.from_lang || "", bg.target || exam.name, it.note || "");
    if (ch.changes) n++;
  }
...
// A2 实时归因:对【单个】刚判错的语言题,当场后台判断迁移来源并入库(attempt_id UNIQUE 天然去重)。
// 只在语言类考试 + 已设语言背景时跑;不阻塞答题响应。
export function classifyTransferBg(user, exam, { attemptId, kpId, stem, correctAnswer, userAnswer }) {
  if (!exam || exam.exam_type !== "language" || !attemptId) return;
  const bg = getLangBackground(user.id);
  if (!bg.native && !(bg.known || []).length) return; // 背景都没设,判不了迁移
  Promise.resolve().then(async () => {
    try {
      const exists = db.prepare("SELECT 1 FROM lang_transfer WHERE attempt_id=?").get(attemptId);
      if (exists) return;
      const bgStr = `母语=${bg.native || "未知"};已会外语=${(bg.known || []).join("、") || "无"};目标语=${bg.target || exam.name}`;
      const out = await generateJson(
        `考生语言背景:${bgStr}。这是一道${bg.target || exam.name}的题,考生答错了。
题:${String(stem || "").slice(0, 300)}
正解:${String(correctAnswer || "").slice(0, 200)}
考生答:${String(userAnswer || "(空)").slice(0, 200)}
判断这个错误最可能的来源:l1_negative(母语负迁移)/l2_negative(已会的其他外语负迁移)/target_internal(目标语内部规则没掌握)/careless(粗心笔误)。
给 {source, from_lang(迁移来源语言,如中文/英语;target_internal/careless 留空), note(一句话:错在哪、怎么迁移的), contrast(可选,一条值得记的对照点 {concept,native,l2,target,pitfall,kind:negative|positive},没有就不填)}。` + langInstruction(user.lang),
        { type: "object", properties: {
          source: { type: "string", enum: SOURCES }, from_lang: { type: "string" }, note: { type: "string" },
          contrast: { type: "object", properties: { concept: { type: "string" }, native: { type: "string" }, l2: { type: "string" }, target: { type: "string" }, pitfall: { type: "string" }, kind: { type: "string", enum: ["negative", "positive"] } } },
        }, required: ["source"] }
      );
      db.prepare("INSERT OR IGNORE INTO lang_transfer(exam_id,kp_id,attempt_id,source,from_lang,to_lang,note) VALUES(?,?,?,?,?,?,?)")
        .run(exam.id, kpId || null, attemptId, out.source, out.from_lang || "", bg.target || exam.name, out.note || "");
      const c = out.contrast;
      if (c && c.concept) {
        const scSql = scopeSql(examScope(exam.id));
        const exist = db.prepare(`SELECT 1 FROM lang_contrast WHERE exam_id IN ${scSql} AND concept=?`).get(c.concept.trim());
        if (!exist) db.prepare("INSERT INTO lang_contrast(exam_id,concept,native,l2,target,pitfall,kind) VALUES(?,?,?,?,?,?,?)").run(exam.id, c.concept, c.native || "", c.l2 || "", c.target || "", c.pitfall || "", c.kind || "negative");
      }
    } catch {}
  });
}
