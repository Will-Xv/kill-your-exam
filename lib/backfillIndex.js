...
// 设计要点:
//  · 幂等:只处理【当前 chunks 数为 0】的资料,重复跑不会重复建、也不会重复花钱;
...
export function missingSummary() {
  ...
  return { total: rows.length, pdf: pdf.length, image: img.length, bytes,
    users: [...new Set(rows.map((r) => r.user_id))].length };
}

let running = false;
export function backfillState() { return { running }; }
