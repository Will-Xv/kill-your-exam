...
import { maybeExtractQuestions } from "@/lib/paperExtract";
    ...
    for (const r of rows) {
      try {
        const n = await runAsUser(r.user_id, async () => {
          if (r.kind === "image") { const n = await indexImageMaterial(r.id, r.exam_id, r.mime, r.lang); await maybeExtractQuestions(r.id, r.exam_id, r.lang); return n; }
          const n2 = r.size > HUGE_PDF_BYTES
            ? await indexBigPdf(r.id, r.exam_id, r.lang)
            : await indexPdfOutline(r.id, r.exam_id, r.lang);
          await maybeExtractQuestions(r.id, r.exam_id, r.lang);   // 存量卷子也一并把题抄进题库
          return n2;
        });
        made += n || 0;
        if (!n) failed++;
