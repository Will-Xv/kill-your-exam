...
export function onAnswer(userId, examId, { correct }) {
  ...
  const hasDiffTrig = trigs && trigs.some((t) => t.action === "difficulty_down" || t.action === "difficulty_up");
  if (!hasDiffTrig) { // 没有难度触发器(模式被删/停用)-> 自动复位难度档位,恢复正常
    if (getDifficultyPref(examId) != null) { try { setSetting(dkey(examId), ""); } catch {} return [{ action: "difficulty_reset" }]; }
    return null;
  }
  const applied = [];
