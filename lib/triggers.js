...
function hasRecentSelfClaim(userId, examId, days) { try { return db.prepare("SELECT COUNT(*) c FROM memory_facts WHERE user_id=? AND exam_id=? AND kind='self_assessment' AND active=1 AND created_at >= datetime('now','-'||?||' day')").get(userId, examId, Math.max(1, days || 3)).c > 0; } catch { return false; } }
...
function runAction(userId, examId, kpId, questionId, t, applied, pending) {
  ...
  if (/^difficulty_/.test(a)) doDifficulty(examId, a, Math.max(1, parseInt(t.step, 10) || 1), applied);
    ...
    try {
      const n = db.prepare("UPDATE memory_facts SET weight = MAX(0.2, COALESCE(weight,1.0)*0.4) WHERE user_id=? AND exam_id=? AND kind='self_assessment' AND active=1 AND created_at >= datetime('now','-7 day')").run(userId, examId).changes;
