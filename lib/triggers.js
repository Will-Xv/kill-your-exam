...
function userTz(userId) { try { const t = db.prepare("SELECT timezone FROM users WHERE id=?").get(userId)?.timezone; return (t && /\//.test(t)) ? t : "UTC"; } catch { return "UTC"; } }
function localToday(tz) { try { return new Intl.DateTimeFormat("en-CA", { timeZone: tz }).format(new Date()); } catch { return new Date().toISOString().slice(0, 10); } }
function localDow(tz) { try { const wd = new Intl.DateTimeFormat("en-US", { timeZone: tz, weekday: "short" }).format(new Date()); return { Sun: 0, Mon: 1, Tue: 2, Wed: 3, Thu: 4, Fri: 5, Sat: 6 }[wd]; } catch { return new Date().getDay(); } }
...
export async function onSession(userId, examId) {
  ...
  const today = localToday(tz);   // 用户本地日期(YYYY-MM-DD)
  const dow = localDow(tz);        // 用户本地周几(0=周日)
  const applied = [], pending = [];
  for (const t of trigs) {
    let hit = false;
