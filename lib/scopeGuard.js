...
//   判据是【超没超出课纲范围】,不是【是不是同一个学科】——沾边不等于越界,超范围才算越界;\n//   所以"用 Python 解数学题"放进一门纯证明/计算的数学课算 out(课纲里没有编程这块);\n//   但若这门课是交叉学科、课纲里本就有编程条目,那它就是 in,不能因为"看着像另一个学科"误杀。
...
export async function checkInSyllabus(exam, text, lang) {
  ...
  try {
    const out = await generateJson(
      ...
      `- out:【明显不在】这份课纲范围内。★判据是【课纲范围】,不是【学科】:哪怕它跟这门课的学科沾边,只要【超出课纲范围】就算 out;反过来,只要课纲里确实有这一块(比如交叉学科的课纲本来就含编程),那就是 in——别因为"看起来像另一个学科"就判 out。例:课纲全是纯数学的证明与计算,却要求"写程序/用 Python 实现",那是 out(课纲里没有编程这块);但若课纲里本就列了编程/建模条目,同样的要求就是 in;\n` +
