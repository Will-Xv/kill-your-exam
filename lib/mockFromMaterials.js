...
默认规则:题目要能真正检验理解、不要只考死记;single/multi 给 4 个选项,answer 写字母(多选如 AC);
judge 的 answer 写「对」或「错」;fill 写标准答案;short 写评分要点;explanation 写解析;difficulty 1~3。
...
${basisText}` + langInstruction(user.lang);
  ...
  const mock = db.prepare("INSERT INTO mock_exams(exam_id,config_json) VALUES(?,?)")
    .run(exam.id, JSON.stringify({ questionIds: picked.map((q) => q.id), marks, totalMarks, mode: "materials", materialIds: ids, request: String(request || "").slice(0, 600), createdAt: Date.now() }));
  return { mockId: mock.lastInsertRowid, totalMarks, questions: picked.map((q) => ({ ...q, marks: marks[q.id] })), files: names.map((x) => x.filename) };
}
