...
import { extractedFor, extractQuestionsFromMaterial } from "@/lib/paperExtract";
...
// 【做原题】(2026-08,Will 定;二版:上传即抽题之后的改法)
// 这里【不再自己解析文件】—— 资料一上传就已经把里面的题抄进题库了(见 lib/paperExtract.js)。
// 所以做原题 = 从题库里把这几份资料的题取出来组成一份卷子:零 token、零等待、每次都是同一批题。
// 只有一种情况会现抽:老资料是在"上传即抽题"上线之前传的、库里还没有(兜底;抽完同样入库,下次就有了)。
export async function extractPaperFromMaterials(exam, user, { materialIds, request = "", count = 0 }) {
  const ids = (materialIds || []).map(Number).filter(Boolean);
  if (!ids.length) return { error: "no_material" };
  const ph = ids.map(() => "?").join(",");
  const mats = db.prepare(`SELECT id, filename, kind, mime, status, stored, has_questions, questions_extracted FROM materials
    WHERE id IN (${ph}) AND exam_id IN ${scopeSql(familyScope(exam.id))}`).all(...ids);
  if (!mats.length) return { error: "no_material" };

  const out = [], perFile = [], failed = [];
  for (const m of mats) {
    let rows = extractedFor(exam.id, m.id);
    if (!rows.length && !m.questions_extracted) {
      if (!(m.status === "ready" && m.stored)) { failed.push({ filename: m.filename, why: "not_ready" }); continue; }
      try { await extractQuestionsFromMaterial(m.id, exam.id, user.lang, { request }); } catch {}
      rows = extractedFor(exam.id, m.id);
    }
    if (!rows.length) { failed.push({ filename: m.filename, why: "no_question_found" }); continue; }
    out.push(...rows);
    perFile.push({ filename: m.filename, n: rows.length });
  }
