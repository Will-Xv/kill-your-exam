...
import { buildFigures, docxImages } from "@/lib/figures";
...
export const extractSchema = { type: "object", properties: { questions: { type: "array", items: { type: "object", properties: {
  ...
  figures: { type: "array", description: "这道题【自带的图】(几何图形、电路、图表、地图、照片…)。纯文字题留空数组。",
    items: { type: "object", properties: {
      page: { type: "integer", description: "图在原文件的第几页(从 1 数;文件不是 PDF 就填 0)" },
      bbox: { type: "array", description: "图在那一页上的位置,四个 0~1 之间的小数 [左, 上, 右, 下];左上角是 0,0", items: { type: "number" } },
      index: { type: "integer", description: "Word 文档里:这是文中【从上往下数】第几张图(从 0 数)" },
...
- difficulty: 1~3。
- figures: 【这道题带不带图】。几何图形、函数图像、电路图、表格式插图、地图、实验装置、照片……凡是【离了它这题就没法做】的图,都要报。
  ★纯文字题一定留【空数组】,别为了凑数把页眉、logo、装饰线条报成图。
  · 文件是 PDF:填 page(第几页,从 1 数)和 bbox=[左, 上, 右, 下],四个数都是【相对整页的比例】(0~1),页面左上角是 0,0、右下角是 1,1。
    框要【把整张图连同坐标轴、标注、图题都框进去】,宁可稍微大一点,也不要切掉边角。
  · 文件是图片:page 填 0,bbox 同样按【整张图片】的比例填。
  · 文件是 Word:填 index —— 这是文中从上往下数第几张图(第一张是 0)。
  · alt:一句话说清这张图画的是什么(考生看不到图时的替代说明)。
...
export async function extractQuestionsFromMaterial(materialId, examId, lang, { request = "", force = false, text = "" } = {}) {
  ...
  // 【题目插图】docx 里的图在 zip 的 word/media/ 下,先整份解出来,模型报的 index 直接对号入座。
  // PDF/图片不用预解:那两种是按模型给的页码+框现裁的。
  let docxImgs = null;
  if (/officedocument\.wordprocessingml|\.docx$/i.test((m.mime || "") + " " + (m.filename || ""))) {
    try { docxImgs = docxImages(buf || fs.readFileSync(matPath(materialId))); } catch { docxImgs = null; }
  }
  const figSrc = { materialId, kind: m.kind };
  ...
  for (const q of list) {
    ...
    // 把模型报的图【真的裁出来存成文件】再写进题目;裁不出来就当没有,绝不留一个指不到东西的引用
    let figures = [];
    try { figures = await buildFigures(examId, figSrc, q.figures, `m${materialId}`, docxImgs); } catch {}
