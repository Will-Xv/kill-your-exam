...
export function datedAnchorPlan(userId, exam, today) {
  ...
  // 优先级:根因 > 薄弱(weak)> 未学 > 做得少的
  const pr = (k) => (k.rootCause ? 0 : 100) + (k.level === "weak" ? 0 : 10) + Math.min(9, k.attempts || 0);
...
export function currentDailyItems(userId, exam) {
  ...
  if (practicalMode) {
    ...
    let hasAssign = false;
    try { hasAssign = !!db.prepare(`SELECT 1 FROM practical_tasks WHERE exam_id IN (${(familyScope(exam.id) || []).map(Number).join(",") || "0"}) AND completed_at IS NULL AND (kind='assignment' OR due_date IS NOT NULL) LIMIT 1`).get(); } catch {}
