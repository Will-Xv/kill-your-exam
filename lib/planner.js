...
export function datedAnchorPlan(userId, exam, today) {
  const famIds = (familyScope(exam.id) || []).map(Number);
  if (!famIds.length) return null;
  let mm = [];
  try { mm = masteryMatrix(exam.id); } catch { return null; }
  const notReady = mm.filter((k) => k.level !== "mastered" && k.level !== "ok"); // 只锚未掌握(weak/unlearned)
  const weakByExam = {};
  for (const k of notReady) (weakByExam[k.exam_id] ||= []).push(k);
  const inList = famIds.join(",");
  const assessments = [];
  try {
    for (const e of db.prepare(`SELECT id, name, exam_date FROM exams WHERE id IN (${inList}) AND exam_date IS NOT NULL AND deleted_at IS NULL`).all()) {
      if (String(e.exam_date).slice(0, 10) < today) continue;
      const kps = weakByExam[e.id] || [];
      if (!kps.length) continue;
      assessments.push({ kind: "exam", name: e.name, date: String(e.exam_date).slice(0, 10), daysLeft: daysUntil(e.exam_date, today), kps });
    }
  } catch {}
  try {
    for (const a of db.prepare(`SELECT id, title, due_date, kp_id FROM practical_tasks WHERE exam_id IN (${inList}) AND due_date IS NOT NULL AND completed_at IS NULL AND kp_id IS NOT NULL`).all()) {
      if (String(a.due_date).slice(0, 10) < today) continue;
      const k = mm.find((x) => Number(x.id) === Number(a.kp_id));
      if (!k || k.level === "mastered" || k.level === "ok") continue;
      assessments.push({ kind: "assignment", name: a.title, date: String(a.due_date).slice(0, 10), daysLeft: daysUntil(a.due_date, today), kps: [k] });
    }
  } catch {}
  if (!assessments.length) return null;
  assessments.sort((x, y) => (x.date < y.date ? -1 : x.date > y.date ? 1 : 0));
  const near = assessments[0];
  // 优先级:根因 > 未学(unlearned)> 薄弱(weak)> 做得少的。未学排在薄弱前,避免用户一直在做过的旧薄弱点里打转、新知识点推不进去。
  const pr = (k) => (k.rootCause ? 0 : 100) + (k.level === "unlearned" ? 0 : 10) + Math.min(9, k.attempts || 0);
  const sorted = near.kps.slice().sort((a, b) => pr(a) - pr(b));
  const dl = Math.max(0, near.daysLeft == null ? 0 : near.daysLeft) + 1;
  const perDay = Math.max(1, Math.ceil(sorted.length / dl));
  const todayKps = sorted.slice(0, perDay);
  return {
    anchor: { kind: near.kind, name: near.name, date: near.date, daysLeft: near.daysLeft, remaining: sorted.length },
    kpIds: todayKps.map((k) => Number(k.id)),
    kps: todayKps.map((k) => ({ id: k.id, title: k.title, chapter: k.chapter || null, level: k.level, rootCause: !!k.rootCause })),
  };
}
...
export function currentDailyItems(userId, exam) {
  ...
  try {
    if (exam.parent_exam_id) {
      ...
      it = tasks.map((tk) => tk.type === "review" ? { type: "review" } : tk.type === "kp" ? { type: "kp", kpId: tk.kpId, title: tk.title, weak: true, root: !!tk.root } : { type: "free", target: 10 });
    } else {
      ...
      it = (e && e.tasks ? e.tasks : []).map((tk) => tk.type === "review" ? { type: "review" } : tk.type === "kp" ? { type: "kp", kpId: tk.kpId, title: tk.title, weak: true, root: !!tk.root } : { type: "free", target: 10 });
    }
  } catch {}
  ...
  if (practicalMode) {
    ...
    const hadFree = it.find((x) => x.type === "free");
    it = [review];
    if (oneKp) it.push({ ...oneKp, target: 2, light: true });
    // 有作业(assignment/实践作业)的实践模式:也保留一条自由练习(Will:在有作业的实践模式也要有自由练习)
    let hasAssign = false;
    try { hasAssign = !!db.prepare(`SELECT 1 FROM practical_tasks WHERE exam_id IN (${(familyScope(exam.id) || []).map(Number).join(",") || "0"}) AND completed_at IS NULL LIMIT 1`).get(); } catch {}
    if (hasAssign) it.push(hadFree || { type: "free", target: 5 });
  }
  // 【按任务日期分配知识点 → 严格套在自由练习上】给自由练习绑定今天该锚定练的知识点
  try {
    const anchored = datedAnchorPlan(userId, exam, today);
    if (anchored && anchored.kpIds.length) {
      const fi = it.find((x) => x.type === "free");
      if (fi) { fi.kpIds = anchored.kpIds; fi.anchor = anchored.anchor; fi.anchorKps = anchored.kps; }
    }
  } catch {}
