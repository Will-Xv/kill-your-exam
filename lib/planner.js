...
function summarizeExam(ex) {
  ...
  // 根因知识点排到最前(它薄弱会拖垮一片),并去重合并进 weakKps。
  const rcIds = new Set(rootCauseKps.map((k) => k.id));
  const mergedWeak = [...rootCauseKps.map((k) => ({ id: k.id, title: k.title, rootCause: true })), ...(s.weakKps || []).filter((k) => !rcIds.has(k.id))];
...
function buildTasks(row, allocMinutes, sprint) {
  ...
  for (const kp of (row.weakKps || []).slice(0, sprint ? 2 : 3)) { if (left <= 6) break; const m = Math.min(left, 15); tasks.push({ type: "kp", title: kp.title, kpId: kp.id, minutes: m, root: !!kp.rootCause, href: `/practice?kp=${kp.id}` }); left -= m; }
