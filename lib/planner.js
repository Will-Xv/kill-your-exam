...
// 【考核截止:某个日期之前必须学完什么】给周计划用。
// 和今日任务【同一个引擎、同一套判定】:范围=该考核自己的知识点(按单元顺序),"学完"=做够 NEW_KP_TARGET 题。
// 不做任何"预计哪天学哪个"的推算——只说清客观约束:到这天之前,这些必须学完。
export function cycleDeadlines(exam, today) {
  let mm = [];
  try { mm = masteryMatrix(exam.id); } catch { return []; }
  return upcomingCycles(exam, today).map((c) => {
    const list = mm.filter((k) => Number(k.exam_id) === Number(c.examId));
    const done = list.filter((k) => (k.attempts || 0) >= NEW_KP_TARGET);
    const left = list.filter((k) => (k.attempts || 0) < NEW_KP_TARGET);
    return {
      examId: c.examId, name: c.name, date: c.date, daysLeft: c.daysLeft,
      total: list.length, learned: done.length, remaining: left.length,
      remainingTitles: left.slice(0, 6).map((k) => (k.chapter ? k.chapter + " · " : "") + k.title),
    };
  }).filter((d) => d.total > 0);
}
