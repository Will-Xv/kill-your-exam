...
function daysUntil(dateStr, refDateStr) {
  ...
  const t0 = refDateStr ? new Date(refDateStr + "T00:00:00") : new Date();
  if (!refDateStr) t0.setHours(0, 0, 0, 0);
...
function summarizeExam(ex) {
  ...
  return { id: ex.id, name: ex.name, examDate: ex.exam_date || null, dailyMinutes: ex.daily_minutes || 60,
    ...
    due, gap, weakTitles: s.weak.slice(0, 3), weakKps: s.weakKps || [] };
}
...
function buildTasks(row, allocMinutes, sprint) {
  ...
  for (const kp of (row.weakKps || []).slice(0, sprint ? 2 : 3)) { if (left <= 6) break; const m = Math.min(left, 15); tasks.push({ type: "kp", title: kp.title, kpId: kp.id, minutes: m, href: `/practice?kp=${kp.id}` }); left -= m; }
...
function variantTasks(row, allocMinutes, profile) {
  ...
  for (const kp of (row.weakKps || []).slice(0, profile.weakCap)) { if (left <= 6) break; const m = Math.min(left, 15); tasks.push({ type: "kp", title: kp.title, kpId: kp.id, minutes: m, href: `/practice?kp=${kp.id}` }); left -= m; }
