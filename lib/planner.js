...
function listTopExams(userId) {
  // 顶层考试(parent_exam_id IS NULL),排除已删/未建完(setup_state 非空=草稿/生成中)。子考试由 examSummary 家族聚合。
  try {
    return db.prepare("SELECT id,name,exam_date,daily_minutes FROM exams WHERE user_id=? AND parent_exam_id IS NULL AND deleted_at IS NULL AND (status IS NULL OR status='active') AND (setup_state IS NULL OR setup_state='') ORDER BY id").all(userId);
  } catch {
    try { return db.prepare("SELECT id,name,exam_date,daily_minutes FROM exams WHERE user_id=? AND deleted_at IS NULL ORDER BY id").all(userId); } catch { return []; }
  }
}
