...
import { todayStr, nowMs } from "@/lib/devtime";
...
function daysUntil(dateStr, refDateStr) {
  ...
  const t0 = refDateStr ? new Date(refDateStr + "T00:00:00") : new Date(nowMs());
...
export function currentDailyItems(userId, exam) {
  ...
  // 任务优先模式(vibe coding / 编程 / 动手技能):主要靠做实践任务学,今日任务里【少留题】——只保留复习 + 至多一条轻量练习(目标2),自由练习去掉;实践任务由 /api/daily 的 practical 段作为主线呈现。
