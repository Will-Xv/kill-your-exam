...
export function saveMode(userId, examId, name, rules, { scope = "exam", activate = true, triggers = null } = {}) {
  ...
  if (existing) {
    db.prepare("UPDATE learning_modes SET rules=?, active=?, triggers=?, updated_at=datetime('now') WHERE id=?").run(String(rules || ""), activate ? 1 : 0, triggers ? JSON.stringify(triggers) : null, existing.id);
    return { id: existing.id, name: nm, scope: eid == null ? "global" : "exam", active: !!activate, updated: true };
  }
  const r = db.prepare("INSERT INTO learning_modes(user_id,exam_id,name,rules,active,triggers) VALUES(?,?,?,?,?,?)").run(userId, eid, nm, String(rules || ""), activate ? 1 : 0, triggers ? JSON.stringify(triggers) : null);
...
export function listModes(userId, examId) {
  const eid = g(examId);
  return db.prepare(
    "SELECT id,name,rules,active,exam_id,triggers FROM learning_modes WHERE user_id=? AND (exam_id IS NULL" + (eid == null ? "" : " OR exam_id=?") + ") ORDER BY exam_id IS NULL DESC, updated_at DESC"
  ).all(...(eid == null ? [userId] : [userId, eid]))
    .map((m) => ({ id: m.id, name: m.name, rules: m.rules, active: !!m.active, scope: m.exam_id == null ? "global" : "exam", triggers: m.triggers }));
}
...
// 已激活模式里的【结构化触发器】(供确定性触发引擎读取)
export function activeTriggers(userId, examId) {
  const out = [];
  for (const m of listModes(userId, examId)) {
    if (!m.active || !m.triggers) continue;
    try { const t = JSON.parse(m.triggers); if (Array.isArray(t)) out.push(...t); } catch {}
  }
  return out;
}
