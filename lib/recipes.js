...
export function methodLink(m, kpId) {
  ...
  if (mm === "debate") return { href: `/arena?mode=debate`, label: "辩论", tag: "🎤", count: cnt };
  ...
  return { href: `/practice?kp=${kpId}${cnt != null ? "&n=" + cnt : ""}`, label: "练习", tag: "✍️", count: cnt };
}

// H1:根据活跃配方的各阶段方法,给首页推出一组【为你的学习法准备】的入口(去重、当前阶段方法高亮)。
// 用泛化链接(不绑具体知识点),让用户从入口自己选。curMethod=当前阶段方法,用于高亮。
export function recipeSurface(recipe, curMethod) {
  const phases = (recipe && recipe.spec && recipe.spec.phases) || [];
  const seen = new Set(); const out = [];
  for (const ph of phases) {
    const mt = ph.method && ph.method.type;
    if (!mt || mt === "ai_choose" || seen.has(mt)) continue;
    seen.add(mt);
    let href, label, tag;
    if (mt === "socratic") { href = "/arena?mode=socratic"; label = "苏格拉底式引导"; tag = "🧭"; }
    else if (mt === "debate") { href = "/arena?mode=debate"; label = "辩论"; tag = "🎤"; }
    else if (mt === "explore") { href = "/study?mode=explore"; label = "自由探索"; tag = "🔍"; }
    else if (mt === "custom_mode") { href = "/arena?launch=" + ((ph.method && ph.method.modeId) || ""); label = "自定义考核"; tag = "🎯"; }
    else if (mt === "explain_first") { href = "/study"; label = "先看讲解再练"; tag = "📖"; }
    else { href = "/practice"; label = "练习"; tag = "✍️"; }
    out.push({ method: mt, href, label, tag, current: mt === curMethod });
  }
  return out;
}
