...
import fs from "fs";
import { runAsUser } from "@/lib/reqctx";   // 后台入库产生的 token 归上传者
...
import { indexBigPdf, isHugePdf, indexPdfOutline, indexImageMaterial } from "@/lib/pdfIndex";
import { maybeExtractQuestions, extractQuestionsFromMaterial, looksLikeQuestions } from "@/lib/paperExtract";
...
export async function ingestMaterialBuffer(examId, user, buffer, filename, mimeHint) {
  ...
  try {
    ...
    // 【上传即抽题·文本路】docx/txt 这类已经解析出文字的,过一遍粗筛就地抽题(便宜,不用再读原件)。
    // PDF/图片走的是另一条:它们的页段索引由后台补索引任务建,建完顺带判断有没有题、再抽(见 lib/backfillIndex.js)。
    try { if (looksLikeQuestions(parsedText)) await extractQuestionsFromMaterial(materialId, examId, user.lang, { text: parsedText }); } catch {}
    ...
    runAsUser(user?.id, () => Promise.resolve().then(() => autoResolveOnUpload(user, examId, materialId)).catch(() => {}));
    runAsUser(user?.id, () => Promise.resolve().then(async () => {
      try {
...
export async function ingestMaterialFromChunks(examId, user, uploadId, filename, mimeHint) {
  ...
  runAsUser(user?.id, () => Promise.resolve().then(async () => {
    try {
      ...
      // 【建可检索索引·上传时后台就做】PDF 和图片在 parse.js 里一律 text:""(故意不抽字),
      // 不在这儿用 Gemini 多模态真读一遍建索引的话,它们【一个 chunk 都没有】、RAG 永远搜不到,
      // 于是出题/讲解/追问只能每次强制附整份原件 —— 那才是 token 爆炸的根源。
      // 一次性读一遍换掉此后每次调用的整本重复投喂,立刻回本。
      try {
        if (kind === "pdf") {
          const n = tooBigToRead ? await indexBigPdf(materialId, examId, user.lang)   // 超大:拆片逐片读
                                 : await indexPdfOutline(materialId, examId, user.lang); // 普通:整份读一次、按页分节
          if (n > 0) await afterMaterialsChanged(examId);
        } else if (kind === "image") {
          if (await indexImageMaterial(materialId, examId, mime, user.lang) > 0) await afterMaterialsChanged(examId);
        }
        // 【上传即抽题】建索引时已经判定过这份文件里有没有成套的题(materials.has_questions);
        // 有题的,趁热把题原样抄进题库 —— 用户一传完卷子,「我的题库」当场就有,不必先做一遍原题。
        // 没题的(讲义/笔记)直接跳过,一分钱不花。整段失败也不影响上传本身。
        let gotQ = 0;
        if (kind === "pdf" || kind === "image") {
          gotQ = await maybeExtractQuestions(materialId, examId, user.lang);
        } else if (looksLikeQuestions(parsedText)) {
          // docx/txt 这类已经解析出文字的:直接拿文字抽题,不用再读原件(便宜一个数量级)。
          // 先过一遍粗筛(成串题号/成组选项/分值标注),讲义类连这一次调用都省了。
          gotQ = await extractQuestionsFromMaterial(materialId, examId, user.lang, { text: parsedText });
        }
        if (gotQ > 0) await afterMaterialsChanged(examId);
      } catch {}
      if (!tooBigToRead) {
        try {
          const exRow = db.prepare("SELECT id, name FROM exams WHERE id=?").get(examId);
