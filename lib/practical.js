...
// —— 【作业助手型作业】没有里程碑/自动判分,只有一个能传/贴文件、聊天自动存、实时记掌握度的「作业助手」;标记完成后聊天清空 ——
export async function createAssignment(user, exam, { title, brief, dueDate, kpId } = {}) {
  const ttl = String(title || "作业").slice(0, 160);
  const finalKp = kpId || (await matchKp(exam.id, (title || "") + " " + (brief || "")));
  const info = db.prepare("INSERT INTO practical_tasks(exam_id,kp_id,user_id,title,brief,language,milestones_json,due_date,kind) VALUES(?,?,?,?,?,?,?,?, 'assignment')")
    .run(exam.id, finalKp || null, user.id, ttl, String(brief || "").slice(0, 8000), "", "[]", dueDate || null);
  return { taskId: info.lastInsertRowid, title: ttl, kind: "assignment", dueDate: dueDate || null };
}
// 【只改截止日期·小改动】按标题关键词在本考试家族里找作业(practical 或 assignment),只改它的 due_date,不动别的、不重排/重生成。
export function setTaskDue(exam, { title, dueDate } = {}) {
  const scSql = scopeSql(examScope(exam.id));
  const rows = db.prepare(`SELECT id, title, due_date FROM practical_tasks WHERE exam_id IN ${scSql} ORDER BY id DESC`).all();
  if (!rows.length) return { ok: false, note: "no_tasks" };
  let hit;
  const kw = String(title || "").trim().toLowerCase();
  if (kw) hit = rows.find((r) => String(r.title || "").toLowerCase().includes(kw));
  else if (rows.length === 1) hit = rows[0];
  if (!hit) return { ok: false, note: "not_found", candidates: rows.slice(0, 6).map((r) => r.title) };
  const nd = dueDate && /^\d{4}-\d{2}-\d{2}$/.test(dueDate) ? dueDate : null;
  db.prepare("UPDATE practical_tasks SET due_date=? WHERE id=?").run(nd, hit.id); // 只改这一处
  return { ok: true, taskId: hit.id, title: hit.title, oldDue: hit.due_date || null, dueDate: nd };
}

// 【所有带截止日期、未完成的作业】跨考试:给本周计划表(/plan 周历)按截止日期并进去显示。
// 某考试或它的任一祖先考试已标记完成 → 它名下的作业不再当"逾期/到期"提醒(考试都收工了就别再催)。
function _examOrAncestorCompleted(examId) {
  let cur = Number(examId), guard = 0;
  while (cur && guard++ < 50) {
    const r = db.prepare("SELECT completed_at, parent_exam_id FROM exams WHERE id=?").get(cur);
    if (!r) return false;
    if (r.completed_at) return true;
    cur = r.parent_exam_id ? Number(r.parent_exam_id) : 0;
  }
  return false;
}

export function allDatedTasks(userId) {
  let rows = [];
  try {
    rows = db.prepare(`SELECT pt.id, pt.exam_id, pt.title, pt.due_date, pt.milestones_json, pt.kind, pt.completed_at, e.name AS examName
      FROM practical_tasks pt JOIN exams e ON e.id = pt.exam_id
      WHERE pt.user_id = ? AND pt.due_date IS NOT NULL AND (e.deleted_at IS NULL)`).all(userId);
  } catch { return []; }
  const out = [];
  for (const r of rows) {
    const kind = r.kind || "practical";
    if (_examOrAncestorCompleted(r.exam_id)) continue; // 考试已完成的,别再排/提醒它的作业
    if (kind === "assignment") { if (r.completed_at) continue; }
    else {
      let ms = []; try { ms = JSON.parse(r.milestones_json) || []; } catch {}
      const done = db.prepare("SELECT COUNT(DISTINCT milestone_idx) n FROM task_progress WHERE task_id=? AND status IN ('passed','reviewed')").get(r.id)?.n || 0;
      if (ms.length > 0 && done >= ms.length) continue; // 做完的不显示
    }
    out.push({ taskId: r.id, title: r.title, date: r.due_date, examName: r.examName, kind });
  }
  return out;
}

// 【只更新一个作业助手作业的内容·别动别的】按标题在本考试家族里找那一个 assignment,更新它的 brief(和可选 due),让作业助手知道这份作业的具体要求。
export function updateAssignment(exam, { title, brief, dueDate } = {}) {
  const scSql = scopeSql(examScope(exam.id));
  const rows = db.prepare(`SELECT id, title, brief, due_date FROM practical_tasks WHERE exam_id IN ${scSql} AND kind='assignment' ORDER BY id DESC`).all();
  if (!rows.length) return { ok: false, note: "no_assignment" };
  let hit;
  const kw = String(title || "").trim().toLowerCase();
  if (kw) hit = rows.find((r) => String(r.title || "").toLowerCase().includes(kw)) || rows.find((r) => kw.includes(String(r.title || "").toLowerCase()));
  else if (rows.length === 1) hit = rows[0];
  if (!hit) return { ok: false, note: "not_found", candidates: rows.slice(0, 8).map((r) => r.title) };
  const nb = brief != null ? String(brief).slice(0, 8000) : hit.brief;
  const nd = dueDate && /^\d{4}-\d{2}-\d{2}$/.test(dueDate) ? dueDate : hit.due_date;
  db.prepare("UPDATE practical_tasks SET brief=?, due_date=? WHERE id=?").run(nb, nd, hit.id); // 只改这一条
  return { ok: true, taskId: hit.id, title: hit.title, dueDate: nd };
}

// 【资料页/杀手上传文档时后台自动匹配】:若这份文档是某个已有【作业助手作业】的具体要求说明,就自动更新到【那一个】作业的 brief(只改匹配上的那个)。匹配不上就什么都不做。
export async function autoMatchAssignmentDoc(exam, text, lang) {
  try {
    if (!text || String(text).trim().length < 40) return null;
    const scSql = scopeSql(examScope(exam.id));
    const rows = db.prepare(`SELECT id, title FROM practical_tasks WHERE exam_id IN ${scSql} AND kind='assignment' ORDER BY id DESC`).all();
    if (!rows.length) return null;
    const names = rows.map((r, i) => `${i + 1}. ${r.title}`).join("\n");
    const out = await generateJson(
      `考生给考试「${exam.name}」上传了一份文档。这门考试下已有这些【作业(还没填具体要求)】:\n${names}\n\n这份文档的内容(节选):\n「${String(text).slice(0, 4000)}」\n\n判断:这份文档【是不是上面某一个作业的具体要求/题目说明】(assignment / homework / problem set 的要求)?如果是,它对应【哪一个】(给编号 index),并把这份作业【完整的要求内容】整理成 brief。若不是作业要求、或对不上其中任何一个,就 match=false。【务必对得上才 match=true,拿不准就 false】。` + langInstruction(lang),
      { type: "object", properties: { match: { type: "boolean" }, index: { type: "integer" }, brief: { type: "string" } }, required: ["match"] }
    );
    if (out && out.match && Number.isInteger(out.index) && out.index >= 1 && out.index <= rows.length) {
      const hit = rows[out.index - 1];
      const nb = String(out.brief || "").slice(0, 8000);
      if (nb.trim().length > 20) { db.prepare("UPDATE practical_tasks SET brief=? WHERE id=?").run(nb, hit.id); return { taskId: hit.id, title: hit.title }; }
    }
  } catch {}
  return null;
}

export function markAssignmentDone(taskId, done = true) {
  try {
    db.prepare("UPDATE practical_tasks SET completed_at=? WHERE id=?").run(done ? new Date().toISOString() : null, Number(taskId));
    if (done) clearTaskChat(taskId); // 完成即清空作业助手聊天(观察早已进掌握度)
    return true;
  } catch { return false; }
}
...
export function listTasks(exam) {
  const scSql = scopeSql(examScope(exam.id));
  const rows = db.prepare(`SELECT id, title, brief, language, milestones_json, created_at, due_date, kind, completed_at FROM practical_tasks WHERE exam_id IN ${scSql} ORDER BY id DESC`).all();
  return rows.map((r) => {
    let ms = []; try { ms = JSON.parse(r.milestones_json) || []; } catch {}
    ...
    const kind = r.kind || "practical";
    const complete = kind === "assignment" ? !!r.completed_at : (ms.length > 0 && done >= ms.length);
    return { id: r.id, title: r.title, brief: r.brief, language: r.language, milestoneCount: ms.length, done, created_at: r.created_at, dueDate: r.due_date || null, kind, complete };
  });
}
...
export function getTask(taskId) {
  ...
  return { id: r.id, exam_id: r.exam_id, user_id: r.user_id, kp_id: r.kp_id, title: r.title, brief: r.brief, language: r.language, milestones: ms, progress: progMap, appeals, kind: r.kind || "practical", completed_at: r.completed_at || null };
}
...
export async function taskChatTurn(user, task, message, live = [], attachments = []) {
  ...
  const isAssign = task.kind === "assignment";
  const kpTail = `【每次回复最后另起一行,输出本轮从主人【最新这句】看出的知识点掌握信号(他看不到):@@KP [{"id":${task.kp_id || 0},"kind":"understanding"或"misconception"}]。真正说清/答透=understanding;暴露明确概念错误=misconception;只是在提问/看不出就给 @@KP []。】`;
  const kpHist = kpTitle ? `【这位主人在「${kpTitle}」上的历史(他看不到;据此因材施教)】\n${(() => { try { return learnerKpContext(task.kp_id) || "(暂无)"; } catch { return "(暂无)"; } })()}` : "";
  const system = isAssign ? `你是主人的「作业助手」,陪他完成下面这份作业(assignment)。目标是【帮他自己真正做出来、并学会背后的知识】,【不是替他把整份答案写好交上去】。
【这份作业】${task.title}${task.brief ? "\n内容/要求:\n" + String(task.brief).slice(0, 6000) : ""}
【你能看到什么】主人可以在这个聊天里【上传或粘贴文件】(题目、他的草稿、截图等)——这一轮他附带的文件你能直接读,据此帮他;聊天记录会自动保存,你能延续之前的讨论。
【怎么帮】
- 【先搞清这份作业到底是什么】如果上面『内容/要求』很少、笼统或没有(比如只是从 syllabus 抄了个名字),你对要交什么还没把握,就【第一步先请主人把作业题目/要求原文贴上来或上传文件】,确认清楚再开始帮,【别只凭标题瞎猜】。
- 先弄清他卡在哪、这份作业要交什么、他做到哪一步了;再用【提示/追问/拆步骤/给类比/指方向】把他往前推,能让他自己写出来的就别替他写。
- 前置知识简单就几句话讲清;抽象/易误解就用苏格拉底式反问先确认他真懂。
- 以事实为准,绝不为迎合把错的说成对的;他跑题(问网站功能/建考试/改界面等)就提醒他去「问问杀手」,你只管这份作业。
- 简洁,用${LANGN}回复。
${kpHist}
${kpTail}` : `你是主人的「实践作业助教」,正陪他做这个动手作业。你的目标是【帮他自己把作业做出来、并真正学会所需的知识】,【不是替他把答案/代码整段写好交上去】。
...
【每次回复最后另起一行,输出本轮从主人【最新这句】看出的知识点掌握信号(他看不到):@@KP [{"id":${task.kp_id || 0},"kind":"understanding"或"misconception"}]。真正说清/答透=understanding;暴露明确概念错误=misconception;只是在提问/看不出就给 @@KP []。】`;
  ...
  let _ap = [];
  try { _ap = await attachParts(Array.isArray(attachments) ? attachments.slice(0, 4) : []); } catch {}
  contents.push({ role: "user", parts: [{ text: String(message || "").slice(0, 4000) }, ..._ap] });
...
export function listTaskSubs(examId) {
  ...
  const rows = db.prepare(`SELECT id, title, milestones_json, due_date, kind, completed_at FROM practical_tasks WHERE exam_id IN ${scSql} ORDER BY (due_date IS NULL), due_date ASC, id ASC`).all();
  return rows.map((r) => {
    let ms = []; try { ms = JSON.parse(r.milestones_json) || []; } catch {}
    ...
    const kind = r.kind || "practical";
    const complete = kind === "assignment" ? !!r.completed_at : (ms.length > 0 && done >= ms.length);
    return { taskId: r.id, title: r.title, done, total: ms.length, due: r.due_date || null, complete, kind };
  });
}

// 【别的科目快到期/逾期的作业】跨考试:排除当前考试家族,列出还没完成、且截止在 今天+withinDays 内(含逾期)的实践作业,用于首页提醒。
function _addDays(ymd, n) { const [y, m, d] = String(ymd).split("-").map(Number); const dt = new Date(y, m - 1, d); dt.setDate(dt.getDate() + n); return `${dt.getFullYear()}-${String(dt.getMonth() + 1).padStart(2, "0")}-${String(dt.getDate()).padStart(2, "0")}`; }
export function urgentCrossTasks(userId, currentExamId, withinDays = 3) {
  const today = todayStr();
  const lim = _addDays(today, withinDays);
  const cur = new Set((currentExamId ? examScope(currentExamId) : []).map(Number));
  let rows = [];
  try {
    rows = db.prepare(`SELECT pt.id, pt.title, pt.due_date, pt.milestones_json, pt.exam_id, e.name AS examName
      FROM practical_tasks pt JOIN exams e ON e.id = pt.exam_id
      WHERE pt.user_id = ? AND pt.due_date IS NOT NULL AND pt.due_date <= ? AND (e.deleted_at IS NULL) AND pt.completed_at IS NULL
      ORDER BY pt.due_date ASC`).all(userId, lim);
  } catch { return []; }
  const out = [];
  for (const r of rows) {
    if (cur.has(Number(r.exam_id))) continue;            // 排除当前考试家族(那是"本科目",不算"别的")
    if (_examOrAncestorCompleted(r.exam_id)) continue;   // 已完成的考试:别再催它的作业(P2-14)
    let ms = []; try { ms = JSON.parse(r.milestones_json) || []; } catch {}
    const total = ms.length;
    const done = db.prepare("SELECT COUNT(DISTINCT milestone_idx) n FROM task_progress WHERE task_id=? AND status IN ('passed','reviewed')").get(r.id)?.n || 0;
    if (total > 0 && done >= total) continue;            // 里程碑做完的不提醒(assignment 的完成已在 SQL 用 completed_at 过滤)
    out.push({ taskId: r.id, title: r.title, examName: r.examName, examId: r.exam_id, due: r.due_date, overdue: r.due_date < today, done, total });
  }
