...
- 若 check=run:给 language(如 python/javascript/cpp)、starter、tests(【3~5 个】测试用例 {stdin, expected})。
  ★输入输出约定铁律(最重要,自相矛盾的题就是坏在这):① 里程碑 desc 里【必须用一两句话把输入输出格式说死】——stdin 上到底有几行/几个数、每行是什么、用什么分隔、以及程序该往 stdout 打印什么、什么格式;② 【优先用"固定行数/固定个数"的输入,不要让学生"读到 EOF"】(除非题目本质就是不定长,那也要在 desc 里明说"一直读到 EOF/文件结束");③ starter【必须给出与该约定完全一致的读取骨架】(如 Python 就写好「data=sys.stdin.read().split()」或固定几行「input()」,学生只填核心逻辑),这样学生不用猜怎么读输入;④ 每个用例的 stdin【必须严格符合上面声明的格式】(行数、分隔、结尾),expected 是正确程序对这个 stdin 打印的确切内容;⑤ 别把"读一行"的题配上"多行无换行"的用例,也别让固定行数的题去依赖 EOF——读法和用例必须自洽。
...
- 【代码题怎么读输入,以测试用例的 stdin 实际格式为准】:上面每个用例的 stdin 就是评测时真正喂进去的输入——教他读法时【数清楚那 stdin 有几行/几个数、用什么分隔、要不要读到 EOF】,照它教(比如就两行就用两次 input()、不定长才 sys.stdin 读到底),【别一会儿让他 sys.stdin 一会儿又说不对、前后矛盾】。拿不准就先让他把某个用例的 stdin 贴进【运行程序】的输入框跑一次看实际行为。
...
export function listTaskSubs(examId) {
  ...
  const rows = db.prepare(`SELECT id, title, milestones_json, due_date FROM practical_tasks WHERE exam_id IN ${scSql} ORDER BY id DESC`).all();
  return rows.map((r) => {
    let ms = []; try { ms = JSON.parse(r.milestones_json) || []; } catch {}
