...
export async function taskChatTurn(user, task, message, live = []) {
  ...
  const msDesc = (task.milestones || []).map((m, i) => {
    ...
    if (lv && lv.runOut && !Array.isArray(lv.runOut.results)) {
      const ro = lv.runOut;
      if (ro.stdout || ro.stderr || ro.compile_output) {
        block += `\n   他刚【运行程序】(未测/未提交)${lv.runInput ? "·输入=" + JSON.stringify(String(lv.runInput)).slice(0, 100) : ""}:输出=${JSON.stringify(String(ro.stdout || "")).slice(0, 200)}${ro.stderr ? " / 报错=" + String(ro.stderr).slice(0, 200) : ""}${ro.compile_output ? " / 编译错误=" + String(ro.compile_output).slice(0, 200) : ""}`;
      }
    }
    return block;
  }).join("\n");
