...
- 若 check=run:给 language(如 python/javascript/cpp)、starter(可选、简短的起始骨架)、tests(【3~5 个】测试用例 {stdin, expected})。
  ★测试用例铁律(务必遵守):① 每个用例的输入要【小】、输出要【短】;② expected 必须是你能【100% 手算正确】的标准输出(末尾不要多余空行);③ 【绝对不要】生成几十上百个用例、也不要用会产生超大数字/超长输出的输入(如大数阶乘、大范围循环打印);④ 用例数控制在 5 个以内;⑤ 只有当你无法确定 expected 一定正确时,把这个里程碑改成 check=evidence(让学生交结果+AI审阅),不要硬塞 run 用例。测试要能区分对错但保持简单可验证。
...
整体给 title(任务名)、brief(一段话说明目标)、language(主要语言,没有就空)。任务要具体、可完成、有意义,别空泛。` + langInstruction(user.lang),
  ...
  const milestones = (out.milestones || []).slice(0, 6).map((m) => {
    if (m.check === "run" && Array.isArray(m.tests)) {
      m.tests = m.tests.filter((tc) => tc && String(tc.stdin || "").length <= 400 && String(tc.expected || "").length <= 400).slice(0, 6);
  ...
  const info = db.prepare("INSERT INTO practical_tasks(exam_id,kp_id,user_id,title,brief,language,milestones_json) VALUES(?,?,?,?,?,?,?)")
    .run(exam.id, kpId || null, user.id, out.title || subject, out.brief || "", out.language || "", JSON.stringify(milestones));
