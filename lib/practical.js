...
import { generate, generateJson, langInstruction, embed, cosine } from "@/lib/gemini";
...
export async function taskChatTurn(user, task, message) {
  const kpTitle = task.kp_id ? (db.prepare("SELECT title FROM knowledge_points WHERE id=?").get(task.kp_id)?.title || "") : "";
  const msDesc = (task.milestones || []).map((m, i) => `${i + 1}. ${m.title}${m.desc ? " — " + String(m.desc).slice(0, 120) : ""}`).join("\n");
