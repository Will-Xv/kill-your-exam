...
export async function assessMaterialTopic(exam, { text, buffer, mime, kind }, lang) {
  try {
    ...
    if (text && text.trim().length >= 30) materialDesc = text.slice(0, 1500);
      ...
      try { materialDesc = (await readImage(buffer, mime || "application/pdf", "One sentence: what subject/topic/course is this document about? Output only that one sentence." + langInstruction(lang), { maxOutputTokens: 200 })).slice(0, 300); } catch {}
    }
